...
- ``XY_Addition_MultipleBond``, seeded by the four-centre XY-addition builder. This family belongs
  to RMG's ``halogens`` family set rather than the ``default`` set, so it is only classified — and
  the builder only reached — when that set is enabled for the run, for example
  ``rmg_family_set: 'all'``.
...
are submitted; the number dropped is logged.

Locating the binary: standalone builds are searched for under ``/Local/ce_dana``. Set
``ARC_CREST_STANDALONE_DIR`` to search a different directory, or to an empty string to skip that
search and rely on a conda/mamba ``crest_env`` or ``PATH``.
...
- Constraints pin interatomic distances only. An angle restraint is redundant once the distances
  spanning the reactive core are pinned, and is ill-conditioned near linearity.
