...
CREST is **opt-in only** — it needs its own environment and an external binary, so it is
intentionally absent from the default ``ts_adapters`` list. Install it once with
``make install-crest`` and request it per run:
...
- ``H_Abstraction`` (RMG family reference:
  `H_Abstraction <https://rmg.mit.edu/database/kinetics/families/H_Abstraction/>`_).
- ``XY_Addition_MultipleBond``, seeded by the four-centre XY-addition builder and constrained on all
  six pairwise distances among the ring atoms ``*1``, ``*2``, ``*3`` and ``*4``, so that the
  multiple bond and the ring diagonals cannot relax away from the seed while the forming and
  breaking bonds are held. CREST is the only
  adapter through which that builder is reached, so it never runs unless CREST is requested. This
  family belongs to RMG's ``halogens`` family set rather than the ``default`` set, so it is only
  classified — and the builder only reached — when that set is enabled for the run, i.e. with
  ``rmg_family_set: 'halogens'`` or ``rmg_family_set: 'all'``. A list of family-set labels such as
  ``['default', 'halogens']`` does not work: ``get_all_families()`` does not expand family-set
  labels given in a list, so they are taken for family names and every RMG family is dropped.
- ``carbonyl_based_hydrolysis``, ``ether_hydrolysis`` and ``nitrile_hydrolysis``, constrained on all
  six pairwise distances among the four reactive atoms (electrophilic centre, leaving group, water
  oxygen and the transferring water hydrogen). Four atoms have six internal degrees of freedom, so
  a smaller set would leave the reactive core free to relax away from the saddle. These are
  ARC-native families and are always available.

Each TS seed becomes its own CREST metadynamics job, and a heuristics sweep can produce dozens of
seeds for a single reaction. At most ``MAX_CREST_SEEDS`` of the most geometrically distinct seeds
are submitted; the number dropped is logged. The adapter waits for the jobs of a reaction
synchronously. A job is cancelled once it has been running for more than the smaller of
``MAX_CREST_WALL_TIME_SECONDS`` and the job's ``max_job_time``, counted from the moment it actually
starts executing; time spent waiting in the queue does not count towards that bound. A job that has
not started within ``MAX_CREST_QUEUE_WAIT_SECONDS`` of its submission is cancelled and dropped.

Locating the binary: set ``CREST_PATH`` (the executable) and/or ``CREST_ENV_PATH`` (a shell line
activating the environment that provides it) in your ``~/.arc/settings.py``. When neither is set,
CREST is located when it is first used: under ``CREST_STANDALONE_DIR`` (or the
``ARC_CREST_STANDALONE_DIR`` environment variable), a directory of versioned standalone builds; then
in the common conda/mamba ``crest_env`` locations under the home directory; then on ``PATH``.

Upgrading an existing checkout: ``arc/constants.pxd`` gained ``angstrom_to_bohr``, so a stale
compiled ``arc.constants`` shadows the source and lacks the symbol. Re-run ``make compile`` after
pulling, otherwise the first CREST job fails with an ``AttributeError`` rather than at import.
...
- CREST constraints are implemented for ``H_Abstraction``, ``XY_Addition_MultipleBond`` and the
  three hydrolysis families.
- The ``$constrain`` block restrains the Cartesian positions of the reactive atoms (its ``atoms:``
  line, relative to the ``coords.ref`` reference) and, in addition, the interatomic distances the
  family specification lists (``distance:`` lines; for H-abstraction the A--B separation as well).
  No angle restraint is written: an angle is redundant once the distances spanning the reactive
  core are pinned, and is ill-conditioned near linearity. A completed geometry is accepted only if
  every restrained distance (for H-abstraction, A--H, H--B and A--B) stays within
  ``MAX_RESTRAINED_DISTANCE_DEVIATION`` of the seed; the unrestrained A--H--B angle is not checked.
...
``get_wrapper_constraints(wrapper='crest', ...)`` returns ``None`` when no constraints can be
derived for a seed, and otherwise a dictionary that every family populates with:

- ``atoms``: The zero-based indices of the reactive atoms CREST holds in place.
- ``distance_pairs``: The zero-based atom index pairs whose distances are restrained.

Only the H-abstraction specification additionally carries ``angle_atoms`` (the A, H and B indices,
also available as the ``A``, ``H`` and ``B`` keys); do not rely on those keys for other families.
...
.. code-block:: python
    ...
    for seed in seeds:
        ...
        if crest_constraints is None:
            continue
        reactive_atoms = crest_constraints['atoms']
        restrained_pairs = crest_constraints['distance_pairs']
