...
``year`` key on ``sp_level`` / ``arkane_level_of_theory`` if you need a specific correction year. For
``level_of_theory`` shorthand inputs, you can also use ``level_of_theory_year`` (applies to the SP side only)::

    level_of_theory: 'wb97xd/def2tzvp'
    level_of_theory_year: 2023

``arkane_level_of_theory`` can still be used to explicitly override Arkane behavior.
