...
RUN git clone --filter=blob:none --no-checkout https://github.com/ReactionMechanismGenerator/RMG-Py.git RMG-Py \
 && git -C RMG-Py checkout --detach 6615d10^1  \
...
ENV JULIA_CPU_TARGET="x86-64;sandybridge,-xsaveopt,clone_all;haswell,-rdrnd,base(1)"
# Install RMS
...
RUN micromamba create -y -n rmg_env -f environment.yml \
 ...
 # Verify example
 micromamba run -n rmg_env bash -lc "\
   python-jl /home/rmguser/Code/RMG-Py/rmg.py /home/rmguser/Code/RMG-Py/examples/rmg/minimal/input.py \
 "
...
RUN chmod 644 /etc/profile.d/99-rmg-aliases.sh \
 ...
 && printf '\n# print cheat-sheet once on interactive login\nif [[ \$- == *i* && \$SHLVL -eq 1 ]]; then\n  aliases\nfi\n\n' \
    >> /home/rmguser/.bashrc
