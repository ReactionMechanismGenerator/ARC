...
def extract_sparse_2d_points(results: dict) -> dict:
    ...
    Returns:
        dict: A dictionary with keys ``'x'``, ``'y'``, ``'energy'`` (lists of floats for
              completed isomorphic points with non-None energy), plus ``'failed_points'`` and
              ``'invalid_points'`` (lists of ``[x, y]`` pairs).
    """
    xs, ys, energies = [], [], []
    for key, entry in results.get('directed_scan', {}).items():
        e = entry.get('energy')
        # Non-isomorphic points are tracked in 'invalid_points' and overlaid separately;
        # their energies are physically meaningless here and must not distort the surface.
        if e is not None and entry.get('is_isomorphic', True):
