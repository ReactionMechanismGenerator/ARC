    ...
    def compute_high_p_rate_coefficient(self,
        ...
        if self.output_dict[ts_species.label]['convergence'] or not require_ts_convergence:
            ...
            if not statmech_success:
                ...
                if all(spc.e0 is not None for spc in self.reaction.r_species):
    ...
    def check_arkane_en_corr(self):
        ...
        try:
            ...
            try:
                ...
                return False
        if any(symbol not in atom_energies for symbol in self.reaction.ts_species.get_xyz()['symbols']):
            return False
        return True
