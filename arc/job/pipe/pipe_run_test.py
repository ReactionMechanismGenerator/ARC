...
class TestPipeRunWriteSubmitScript(unittest.TestCase):
    ...
    def test_max_concurrent_zero_is_unthrottled(self):
        """max_concurrent=0 is treated as disabled (no throttle applied)."""
        run = self._make_run('slurm', max_workers=10, n_tasks=10, max_concurrent=0)
        with open(run.write_submit_script()) as f:
            content = f.read()
        array_line = next(line for line in content.splitlines() if '#SBATCH --array=' in line)
        self.assertEqual(array_line.strip(), '#SBATCH --array=1-10')
