...
from arc.species.converter import str_to_xyz, xyz_file_format_to_xyz, xyz_to_str
...
class PyGSM(JobAdapter):
    ...
    def execute(self):
        ...
        Returns:
            ts_dictionary (List[Dict]):
        """
        # self.reactions is a list containing one ARCReaction object
        for arc_reaction in self.reactions:
            # get reactant/s
            reactant_xyz_dict = arc_reaction.get_reactants_xyz(return_format='dict')
            r_all_atoms, r_all = np.array(reactant_xyz_dict['symbols']), np.array(reactant_xyz_dict['coords'])
            xyz_reactant = xyz_to_str(reactant_xyz_dict).split('\n')

            # get product/s
            # get_products_xyz already calls atom_map property so that the product atoms are aligned with reactant atoms
            product_xyz_dict = arc_reaction.get_products_xyz(return_format='dict')
            p_all_atoms, p_all = np.array(product_xyz_dict['symbols']), np.array(product_xyz_dict['coords'])
            xyz_product = xyz_to_str(product_xyz_dict).split('\n')

            r_size = r_all.shape[0]
            ...
            # write the xyz file used for DE GSM to the project directory
            dir_path = os.path.join(self.project_dir, 'TS', arc_reaction.label)
            file_path = os.path.join(dir_path, 'DE_GSM_input.xyz')
            if not os.path.isdir(dir_path):
                os.path.mkdir(dir_path)
            with open(file_path, 'w') as f:
                f.write(xyz)

            #todo: run DE GSM i.e. pass the file path to the pyGSM submit script and run the job

            pyGSM_log = os.path.join(dir_path, 'log')
            lines = _get_lines_from_file(pyGSM_log)
            read = False
            for line in lines:
