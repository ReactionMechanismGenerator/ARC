...
def _load_and_validate_ckpt(ckpt_path: str) -> dict:
    """
    Load the checkpoint at ``ckpt_path``, verifying it is plausibly real.
    ...
    non-Lightning checkpoints). The loaded object is returned so the caller does not have to
    deserialize the (~57 MB) checkpoint a second time.

    Args:
        ckpt_path (str): Path to the checkpoint file.

    Returns:
        dict: The loaded Lightning checkpoint, guaranteed to contain a ``'state_dict'`` key.
    ...
    import torch  # Only importable inside goflow_env; see run_goflow_inference().
...
def run_goflow_inference(input_dict: dict) -> List[dict]:
    ...
    try:
        # Late imports: at run time this whole script executes inside goflow_env, so these are
        # always available here. They are imported inside the function rather than at module
        ...
        ckpt = _load_and_validate_ckpt(input_dict['ckpt_path'])
