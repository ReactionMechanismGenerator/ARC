...
class TestMolproAdapter(unittest.TestCase):
    ...
    def test_set_input_file_memory(self):
        ...
        # The `memory,Total=N,m` card is a PER-PROCESS value (molpro -n cpu_cores), so it must
        # scale as ceil(job_memory_gb * 125 / cpu_cores). 125 MW == 1 GB (1 MW == 8e6 bytes),
        # so input_file_memory * 8e6 * cpu_cores must reproduce ~job_memory_gb of total memory.
        self.assertEqual(self.job_1.job_memory_gb, 14.0)

        self.job_1.input_file_memory = None
        self.job_1.cpu_cores = 48
        self.job_1.set_input_file_memory()
        self.assertEqual(self.job_1.input_file_memory, math.ceil(14.0 * 125 / 48))  # 37
        self.assertAlmostEqual(self.job_1.input_file_memory * 8e6 * 48 / 1e9, 14.0, delta=0.5)

        self.job_1.input_file_memory = None
        self.job_1.cpu_cores = 8
        self.job_1.set_input_file_memory()
        self.assertEqual(self.job_1.input_file_memory, math.ceil(14.0 * 125 / 8))  # 219
        self.assertAlmostEqual(self.job_1.input_file_memory * 8e6 * 8 / 1e9, 14.0, delta=0.5)

        self.job_1.input_file_memory = None
        self.job_1.cpu_cores = 1
        self.job_1.set_input_file_memory()
        self.assertEqual(self.job_1.input_file_memory, math.ceil(14.0 * 125 / 1))  # 1750
        self.assertAlmostEqual(self.job_1.input_file_memory * 8e6 * 1 / 1e9, 14.0, delta=0.5)
