...
* ``linear_utils/linear_invariants_test.py``: production-path invariant
...
from arc.job.adapters.ts.linear_utils.addition import find_split_bonds_by_fragmentation, stretch_bond
from arc.job.adapters.ts.linear_utils.geom_utils import dihedral_deg
...
def _angle_deg(p1: np.ndarray, vertex: np.ndarray, p2: np.ndarray) -> float:
    """Return the angle p1–vertex–p2 in degrees (range [0, 180])."""
    v1 = p1 - vertex
    v2 = p2 - vertex
    ...
    for i in range(n):
        for j in range(i + 1, n):
            ...
            if d < sbl * min_contact_frac:
                test_case.fail(f'H close contact: {symbols[i]}[{i}]-{symbols[j]}[{j}] = {d:.3f} A '
    ...
    for i in range(len(ts_xyzs)):
        for j in range(i + 1, len(ts_xyzs)):
            if almost_equal_coords(ts_xyzs[i], ts_xyzs[j]):
                test_case.fail(f'Guesses {i + 1} and {j + 1} are near-duplicates:\n'
...
class TestLinearAdapter(unittest.TestCase):
    ...
    def test_type_p_uses_product_topology(self):
        ...
        the reactant and product have different bond orders (C–OO vs C–O–O single bonds shift).
        """
...
H       2.05354047   -0.10415729    1.58865243"""
    ...
    def test_family_dispatch_tables_populated(self):
        ...
        self.assertIs(FAMILY_POSTPROCESSORS['Ketoenol'], FAMILY_POSTPROCESSORS['intra_H_migration'])
        self.assertIs(FAMILY_VALIDATORS['Ketoenol'], FAMILY_VALIDATORS['intra_H_migration'])
    ...
    def test_interpolate_isomerization_multi_species_guard(self):
        ...
        self.assertEqual(result, [], msg='interpolate_isomerization should return [] for reactions '
                                         'with more than one reactant or product species.')

    def test_get_rxn_weight(self):
        """Test the get_rxn_weight() function."""
        rxn_1 = ARCReaction(r_species=[ARCSpecies(label='HO2', smiles='[O]O'), ARCSpecies(label='NH', smiles='[NH]')],
                            p_species=[ARCSpecies(label='N', smiles='[N]'), ARCSpecies(label='H2O2', smiles='OO')])
    ...
    def test_get_weight_grid_clips_at_bounds(self):
        ...
        p.e0 = 100.0
        rxn = ARCReaction(r_species=[r], p_species=[p])
        grid = get_weight_grid(rxn, include_hammond=True, hammond_delta=0.35)  # large delta ensures clipping
        self.assertEqual(grid, sorted(grid))
        self.assertTrue(all(0.0 <= w <= 1.0 for w in grid))
    ...
    def test_get_weight_grid_custom_params(self):
        ...
        rxn = ARCReaction(r_species=[r], p_species=[p])
        custom_grid = (0.4, 0.6)
        grid = get_weight_grid(rxn, include_hammond=True, base_grid=custom_grid, hammond_delta=0.05)
        ...
        for w in BASE_WEIGHT_GRID:
            if round(w, 3) not in [round(cw, 3) for cw in custom_grid]:
                w0 = get_rxn_weight(rxn)
                hammond_values = {round(clip01(w0 - 0.05), 3), round(w0, 3), round(clip01(w0 + 0.05), 3)}
...
H      -1.29204440    1.77519606   -0.86803354"""
        ...
        for n_idx in carbene_neighbors:
            ...
            self.assertGreater(d_ts, d_product, msg=f'Forming bond C[{p_carbene}]-C[{n_idx}] not extended: '
 ...
 H                 -0.26777988   -1.39577664   -0.43976256"""
        ...
        for ts in ts_xyzs:
            ...
            d_c4_nearest = min(
                ...
                for j in range(4))
            self.assertGreater(d_c4_nearest, 1.7,msg=f'Carbene C (atom 4) is at product-like distance {d_c4_nearest:.3f} '
                                                     f'Å from the butadiene backbone — this is a bad guess where the '
                                                     f'migrating H is already on the terminal C.')
...
H       2.89130493   -0.59895508   -0.29374226"""
    ...
    def test_interpolate_1_2_shift_c(self):
        ...
        expected_ts = """C      -0.84098792   -1.62746011   -0.65466549
C      -1.38616808    0.31243567    0.41701874
C       0.09289885    0.19281646    0.35695343
C       0.92864438    0.68782411   -0.70819340
C       2.18636908    0.35957487   -0.37255721
...
H       0.24205001    2.21144523    0.08369465"""
        ...
        self.assertGreater(len(ts_xyzs), 0)
        expected_ts = """ C                  1.44144128   -0.30929849   -1.68182421
...
H       1.17124840    2.45844005    1.21692406"""
        ...
        rxn = ARCReaction(r_species=[r], p_species=[p1, p2])
        ts_xyzs = interpolate(rxn)
        expected_ts = """C       3.08090013   -0.41970379   -0.62078809
...
H      -0.43054237    2.12019619    0.25847314"""
        ...
        self.assertGreater(d_cs, 1.7, msg=f'Closest C-S distance {d_cs:.3f} Å is too short for a forming-bond TS')
        self.assertLess(d_cs, 3.5, msg=f'Closest C-S distance {d_cs:.3f} Å is too long for a forming-bond TS')
        expected_ts = """C      -0.52979235   -0.97049747    0.10629971
C      -0.10826701    0.29287574    0.05045664
...
H       0.93760911   -0.05885406   -0.10079043"""
        ...
        rxn = ARCReaction(r_species=[r], p_species=[p1, p2])
        ts_xyzs = interpolate(rxn)
        expected_ts = """C       1.14981017    0.04138987   -0.06722786
C      -0.25415691   -0.17696939    0.46881798
...
H      -1.35146375    0.36042106    2.56558099"""
    ...
    def test_interpolate_1_3_sigmatropic_rearrangement(self):
        ...
        # (2.0-2.5 Å) and at least one in forming-bond range (1.7-2.0 Å).
        has_breaking, has_forming = False, False
        for a in range(5):
            for b in range(a + 1, 5):
                ...
                if 1.7 < d < 2.0:
                    has_forming = True
        self.assertTrue(has_breaking, msg='No heavy-atom pair in breaking-bond range [2.0, 2.5]')
        self.assertTrue(has_forming, msg='No heavy-atom pair in forming-bond range [1.7, 2.0]')
        # All bonded heavy-heavy distances should be > 1.1 Å.
        for a in range(5):
            for b in range(a + 1, 5):
                d = float(np.linalg.norm(coords[a] - coords[b]))
                if d < 2.0:
                    self.assertGreater(d, 1.1, msg=f'Ring bond {a}-{b} at {d:.3f} Å is unphysically short')
        expected_ts = """C      -0.96405208   -0.58870010   -0.35675666
N       0.09948347   -1.35699528   -0.30406609
C       1.38353962   -1.78044479   -0.07760771
C       1.34501329   -0.19807306    0.42260768
N      -0.70083129    0.66320502    0.12207481
H      -1.93870511   -0.87854432   -0.72608823
H       2.38301348   -2.16438007    0.08111116
H       1.81068982    0.68335609    0.83904443
H      -1.36158329    1.42559689    0.18141710"""
        self.assertTrue(any(almost_equal_coords(ts, str_to_xyz(expected_ts)) for ts in ts_xyzs))
...
""", xyz=p_xyz, multiplicity=1)
        rxn = ARCReaction(r_species=[r], p_species=[p])
        ts_xyzs = interpolate(rxn)
        self.assertGreaterEqual(len(ts_xyzs), 1, msg='No TS guess generated.')
...
C      -1.01328204    0.82171103    0.85783337
C      -1.58703673    1.77063602    1.61388634
C      -3.02820483    1.91464515    1.60082626
C      -3.81990455    0.98353285    1.04262686
C      -3.26045636   -0.15876748    0.34604992
C      -2.13813227    0.41485023   -0.06800856
H      -2.38224100   -2.86704541   -0.64276219
H      -0.72266441   -3.54330060   -0.14761540
H       1.15903232   -1.87939413    0.67690806
H       1.00858087    0.84877512    0.21949054
H       0.78637541    0.44003084    1.93700461
H      -1.01189075    2.37605329    2.31444114
...
H      -2.05805178    0.77427910   -1.09392182""")
        ...
        best_rmsd = float('inf')
        for ts_xyz in ts_xyzs:
            self.assertEqual(len(ts_xyz['symbols']), 20, msg=f'TS guess has {len(ts_xyz["symbols"])} atoms, expected 20')
            self.assertEqual(ts_xyz['symbols'][:10], ('C',) * 10, msg='heavy-atom ordering (first 10 = C) not preserved')
            self.assertFalse(colliding_atoms(ts_xyz), msg='TS guess has atom-atom collisions')
            coords = np.array(ts_xyz['coords'], dtype=float)
        ...
        self.assertLess(best_rmsd, 0.35, msg=f'Best TS RMSD vs the expected TS is {best_rmsd:.2f} Å, '
...
H       1.80251143    1.03132880   -1.10238169"""
        ...
        self.assertTrue(len(ts_oo) >= 1, msg=f'No O-O distance in TS range; found {oo_dists}')
...
H      -3.32568190   -0.76196544   -0.01344000
...
H       5.16275840   -2.17786532    0.15880725
H       5.58934956   -0.38386514    0.41300024"""
        self.assertTrue(any(almost_equal_coords(ts, str_to_xyz(expected_ts)) for ts in ts_xyzs))
        ...
        self.assertAlmostEqual(d_C0_H6, d_C1_H6, places=1, msg='Migrating H should be equidistant from donor C0 and acceptor C1')
...
H      -0.53338088   -0.77135867   -0.54806440"""
        ...
        rxn = ARCReaction(r_species=[r], p_species=[p1, p2])
        ts_xyzs = interpolate(rxn)
        expected_ts = """C      -2.49431891    1.11277273    0.66895182
...
12 H u0 p0 c0 {6,S}""", xyz=p_xyz, multiplicity=1)
        ...
        for ts_xyz in ts_xyzs:
            ...
            if all_h_ok:
                good_guesses += 1
        self.assertGreaterEqual(good_guesses, 1, msg='No Cyclopentadiene_scission TS guess has all H atoms properly bonded.')

        # 3-membered-ring TS between C atoms 3, 4, 5 (1-indexed): retained edges still
        # C-C single bonds; breaking edge stretched but shorter than in the product.
        p_coords = np.array(p.get_xyz()['coords'], dtype=float)
        d_p_24 = float(np.linalg.norm(p_coords[2] - p_coords[4]))
        coords_0 = np.array(ts_xyzs[0]['coords'], dtype=float)
        d_23 = float(np.linalg.norm(coords_0[2] - coords_0[3]))
        d_34 = float(np.linalg.norm(coords_0[3] - coords_0[4]))
        d_24 = float(np.linalg.norm(coords_0[2] - coords_0[4]))
        self.assertTrue(1.3 < d_23 < 1.7,
                        msg=f'TS 3-ring edge C3-C4 outside C-C single-bond range: {d_23:.3f} Å')
        self.assertTrue(1.3 < d_34 < 1.7,
                        msg=f'TS 3-ring edge C4-C5 outside C-C single-bond range: {d_34:.3f} Å')
        self.assertGreater(d_24, 1.6,
                           msg=f'TS breaking edge C3-C5 not stretched beyond reactant: {d_24:.3f} Å')
        self.assertLess(d_24, d_p_24,
                        msg=f'TS breaking edge C3-C5 ({d_24:.3f} Å) is not closer than in the '
                            f'product ({d_p_24:.3f} Å); 3-ring TS has not formed')
...
H       1.59082870    1.95894204   -0.79688170"""
        ...
        rxn = ARCReaction(r_species=[r1, r2], p_species=[p])
        ts_xyzs = interpolate(rxn)
        expected_ts = """C       2.60098776   -0.04177774    0.73723478
C       1.20465630    0.10105432    0.20245819
C       0.16278370   -0.55312927    0.74494799
C      -1.24024239   -0.46705077    0.21761600
...
H       4.29227483    0.19395179    1.31850164"""
        ...
        rxn = ARCReaction(r_species=[r1, r2], p_species=[p])
        ts_xyzs = interpolate(rxn)
        expected_ts = """C       2.84332690    1.43980589    0.27287352
C       1.60930875    1.53809494   -0.24535975
C       0.72702818    0.37582523   -0.37603061
C      -0.62707838    0.45548717   -0.45882195
C      -1.37683626    1.71376941   -0.42793406
...
H       0.84348057    0.49385350   -1.31559023"""
        self.assertTrue(any(almost_equal_coords(ts, str_to_xyz(expected_xyz)) for ts in ts_xyzs))
    ...
    def test_interpolate_intra_2_plus_2_cycloaddition_cd(self):
        ...
        self.assertLessEqual(len(ts_xyzs), 2,
                             msg=f'Expected at most 2 TS guesses, got {len(ts_xyzs)}')

        # At least one TS guess must have C1–C4 ≈ 2.21 Å (forming bond) and all four
        # terminal H atoms rotated out of the C0-C1-C2-C3 backbone plane (|dihedral|
        # in (30°, 150°)) so none is wedged between the joining carbons.
        backbone_h_indices = [(4, 0, 1, 2), (5, 0, 1, 2),
                              (8, 3, 2, 1), (9, 3, 2, 1)]

        def _is_good(ts_xyz: dict) -> bool:
            c = np.array(ts_xyz['coords'], dtype=float)
            d_14 = float(np.linalg.norm(c[0] - c[3]))
            if not 2.0 < d_14 < 2.4:
                return False
            for h, a, b, t in backbone_h_indices:
                phi = abs(dihedral_deg(c[h], c[a], c[b], c[t]))
                if phi < 30.0 or phi > 150.0:
                    return False
            return True

        self.assertTrue(any(_is_good(ts) for ts in ts_xyzs),
                        msg='No [2+2] TS guess has C1–C4 ≈ 2.21 Å with all four terminal H atoms '
                            'rotated out of the backbone plane.')
...
12 H u0 p0 c0 {6,S}""", xyz=p_xyz, multiplicity=1)
        rxn = ARCReaction(r_species=[r], p_species=[p])
        ts_xyzs = interpolate_isomerization(rxn)
        expected_ts = """C      -3.03124363    0.21595810   -0.01068883
C      -1.77136356   -0.00875193   -0.22839960
C      -0.63288097   -0.39358006   -0.72438053
C      -1.03044850   -1.09006417   -1.91903749
C      -2.32559961   -0.97969794   -1.89383231
C      -3.03757164   -1.50458547   -2.84397932
H      -3.51150731    1.11248039   -0.40272782
H      -3.62891356   -0.48738065    0.56914327
H       0.29224195   -0.14545380   -0.20411047
H      -0.25872096   -1.53027324   -2.55050607
H      -3.40169423   -2.52887404   -2.76430711
...
H       1.65446241   -1.85341259    0.04404524"""
        ...
        rxn = ARCReaction(r_species=[r], p_species=[p])
        # Gather guesses across the weight grid the linear adapter actually uses.
        ts_xyzs = list()
        for w in (0.35, 0.5, 0.65):
            ts_xyzs.extend(interpolate_isomerization(rxn, weight=w) or list())
        self.assertTrue(ts_xyzs, 'No TS guesses were generated.')
        for ts_xyz in ts_xyzs:
            self.assertEqual(len(ts_xyz['symbols']), 12)
            self.assertFalse(colliding_atoms(ts_xyz))

        # Fulvene's exocyclic =CH2 (atom 0) folds into the C0-C1-C5 cyclopropane; its two H
        # atoms must point outward, not inward. Measured bond-length-independently as the H-bond
        # bisector vs the normalized C0->C1/C0->C5 corridor (reference TS: +0.997).
        def _ch2_bisector_outward(ts: dict) -> float:
            c = np.array(ts['coords'], dtype=float)
            c0, c1, c5, h6, h7 = c[0], c[1], c[5], c[6], c[7]
            b6 = (h6 - c0) / np.linalg.norm(h6 - c0)
            b7 = (h7 - c0) / np.linalg.norm(h7 - c0)
            u1 = (c1 - c0) / np.linalg.norm(c1 - c0)
            u5 = (c5 - c0) / np.linalg.norm(c5 - c0)
            corridor = u1 + u5
            corridor /= np.linalg.norm(corridor)
            bis = b6 + b7
            bis /= np.linalg.norm(bis)
            return float(np.dot(bis, -corridor))
        for ts_xyz in ts_xyzs:
            out = _ch2_bisector_outward(ts_xyz)
            self.assertGreater(out, 0.3,
                               msg=f'CH2 H atoms point inward (bisector.outward={out:.3f}); they '
                                   f'must point outward, away from the ring backbone.')

        # At least one guess shows the CH2 rotated ~90 deg vs the planar reactant: its H atoms
        # straddle the C1-C0-C5 cyclopropane plane on opposite sides.
        def _ch2_straddle(ts: dict) -> bool:
            c = np.array(ts['coords'], dtype=float)
            c0, c1, c5, h6, h7 = c[0], c[1], c[5], c[6], c[7]
            normal = np.cross(c1 - c0, c5 - c0)
            normal /= np.linalg.norm(normal)
            s6 = float(np.dot(h6 - c0, normal) / np.linalg.norm(h6 - c0))
            s7 = float(np.dot(h7 - c0, normal) / np.linalg.norm(h7 - c0))
            return s6 * s7 < -0.1 and min(abs(s6), abs(s7)) > 0.4
        self.assertTrue(any(_ch2_straddle(ts) for ts in ts_xyzs),
                        msg='No TS guess shows the CH2 rotated ~90 deg with H atoms straddling '
                            'the cyclopropane plane.')

        # At least one TS guess must preserve the 5-membered ring scaffold (atoms 1-5).
        ring_bonds = [(1, 2), (2, 3), (3, 4), (4, 5), (5, 1)]
        self.assertTrue(any(all(1.2 < np.linalg.norm(np.array(ts['coords'])[a] - np.array(ts['coords'])[b]) < 2.0
                                for a, b in ring_bonds) for ts in ts_xyzs),
                        'No TS guess preserves the 5-membered ring scaffold.')

        # At least one guess forms the C0-C1-C5 3-ring (the reacting C bent toward its partner).
...
H       0.97222065   -1.40727159   -1.00427440"""
        ...
        rxn = ARCReaction(r_species=[r], p_species=[p])
        ts_xyzs = interpolate_isomerization(rxn)
        expected_ts = """ O                 -2.82721787   -0.50660445    0.17006246
 C                 -1.42423100   -0.54684140   -0.06325185
 C                 -0.72789956    0.80632577    0.00134428
 C                 -1.20268894    1.82056422   -1.03855187
 C                 -1.81843062    1.24444351   -2.26508495
 C                 -2.69466630    0.17606694   -2.00589762
 H                 -2.47473082    0.10908565   -0.69658544
 H                 -0.97373547   -1.20171248    0.68260179
 H                 -1.25248674   -0.96509841   -1.05505071
 H                 -0.88111343    1.23637319    0.99113452
 H                  0.34899946    0.66474936   -0.09000458
 H                 -1.88963161    2.52634334   -0.57155550
 H                 -0.34527752    2.40154848   -1.37826196
 H                 -1.58087719    1.66491783   -3.21937397
 H                 -2.31197868   -0.68229801   -2.53749802
 H                 -3.67931714    0.54985766   -2.24806126"""
        self.assertTrue(any(almost_equal_coords(ts, str_to_xyz(expected_ts)) for ts in ts_xyzs))
...
H       2.88277623   -1.03926992   -1.24983116"""
        ...
        rxn = ARCReaction(r_species=[r], p_species=[p])
        ts_xyzs = interpolate_isomerization(rxn)
        expected_ts = """C       2.37622772    0.13188036   -0.10043648
C       1.09261098   -0.13889783    0.66802441
...
H       6.03724721    2.13554246   -0.93004450"""
    ...
    def test_interpolate_intra_r_add_exo_scission(self):
        ...
        for ts_xyz in ts_xyzs:
            ...
            ch2_h_indices = [i for i, s in enumerate(ts_xyz['symbols']) if s == 'H' and np.linalg.norm(coords[3] - coords[i]) < 1.3]
        ...
        # Find the best TS guess: TS1 preserves the phenyl ring with C3-C4 breaking and C2-C4 forming.
        # Use wider tolerances since the adapter may produce different coordinate frames.
        best_idx = None
        for i, ts_xyz in enumerate(ts_xyzs):
            coords = np.array(ts_xyz['coords'], dtype=float)
            ...
            if ring_ok:
                best_idx = i
                break
        self.assertIsNotNone(best_idx, 'No TS guess preserves the phenyl ring.')
        coords_best = np.array(ts_xyzs[best_idx]['coords'], dtype=float)
        d_24 = float(np.linalg.norm(coords_best[2] - coords_best[4]))
        d_34 = float(np.linalg.norm(coords_best[3] - coords_best[4]))
        # Forming bond (C2-C4) should be contracted toward TS range.
        # Expected ~2.29 Å; the 3-center-shift now contracts it from
        # the reactant distance (~2.9 Å) to ~2.6 Å.
        self.assertLess(d_24, 2.7, msg=f'C2-C4 forming bond {d_24:.3f} Å not contracted  toward TS range (should be < 2.7)')
        # Breaking bond (C3-C4) should be stretched toward TS range.
        # Expected ~2.30 Å; currently at ~1.96 Å.
        self.assertGreater(d_34, 1.8, msg=f'C3-C4 breaking bond {d_34:.3f} Å not stretched  toward TS range (should be > 1.8)')
        # Both bonds should be materially closer to TS-like distances
        # than the reactant values (forming ~2.50, breaking ~1.51).
        self.assertLess(d_24, 2.90, msg=f'C2-C4 forming bond {d_24:.3f} not improved from reactant')
        self.assertGreater(d_34, 1.55, msg=f'C3-C4 breaking bond {d_34:.3f} not improved from reactant')
        expected_ts = """C       4.16628677   -1.17387387    0.42027015
C       3.20983817   -0.46475143    0.27197651
C       2.08026613    0.37563724    0.09737216
C       1.12888754   -0.05066411   -0.65424161
C       0.81218753    1.42065783   -1.91405738
C       1.64631371    1.59084870   -3.02865931
C       1.43302606    2.64455795   -3.91865828
C       0.38802147    3.54057215   -3.70164600
C      -0.44293222    3.38571167   -2.59350729
C      -0.23130881    2.33270646   -1.70172217
H       5.01639747   -1.80193430    0.55250799
H       1.99727579    1.26778525    0.70906190
H       1.35430301   -0.96327946   -1.22054288
H       0.20865888   -0.26481986   -0.09681906
H       2.47003331    0.90233884   -3.20563155
H       2.08451055    2.76729073   -4.77982138
H       0.22310907    4.36126939   -4.39461454
H      -1.25568049    4.08672428   -2.42251918
...
H       0.31648739    2.42027069    0.39137850"""
        ...
        rxn = ARCReaction(r_species=[r], p_species=[p])
        ts_xyzs = interpolate_isomerization(rxn)
        # TS0 should have reasonable forming-bond (C0-C5) and breaking-bond (C0-C1) distances.
        coords_0 = np.array(ts_xyzs[0]['coords'], dtype=float)
        d_forming = float(np.linalg.norm(coords_0[0] - coords_0[5]))  # C0-C5 forming bond
        d_breaking = float(np.linalg.norm(coords_0[0] - coords_0[1]))  # C0-C1 breaking bond
        self.assertTrue(1.4 < d_forming < 2.5, msg=f'C0-C5 forming bond: {d_forming:.3f} (expected 1.4-2.5)')
        self.assertTrue(1.4 < d_breaking < 2.0, msg=f'C0-C1 breaking bond: {d_breaking:.3f} (expected 1.4-2.0)')
 ...
 H                 -1.89603539   -1.07075910   -3.51660759"""
    ...
    def test_interpolate_intra_ene_reaction(self):
        ...
        r_xyz = """ C                 -1.39276735   -1.33348938   -0.25890111
 C                 -2.76706117   -0.99870485   -0.19462596
 C                 -2.90619661    0.39306779    0.02816505
 C                 -1.52807258    1.03130923   -0.18072141
 C                 -0.60270118   -0.11348171    0.22643462
 C                  0.81123003    0.03780639   -0.36475945
 C                  1.55869449    1.27537457    0.16303244
 C                  2.85391625    0.94030146    0.38138684
 C                  2.98170964   -0.59308804    0.29462596
 C                  1.75755007   -1.11068832    0.02802634
 H                 -0.98242769   -2.27141103   -0.57006722
 H                 -3.57632323   -1.69097845   -0.29831871
 H                 -3.81078981    0.90738642    0.27733865
 H                 -1.42259160    1.26300048   -1.21999660
 H                 -1.34437325    1.92216210    0.38278167
 H                 -0.51585685   -0.17672930    1.29102740
 H                  0.72722405    0.09108488   -1.43012530
 H                  1.12633635    2.23881079    0.33553491
 H                  3.64936918    1.62301886    0.59598321
 H                  3.88088011   -1.15487477    0.43880689
 H                  1.49172145   -2.14543548    0.08746937"""
        p_xyz = """ C                  2.43114821    0.87463786   -0.22677059
 C                  2.87673781    0.11863883    0.88437787
 C                  2.24229097   -1.14744769    0.88140596
 C                  1.55099328   -1.31088907   -0.47704843
 C                  1.22647508    0.14046445   -0.82540746
 C                  1.01071951    0.34537058   -2.33638828
 C                  1.32009897    1.46624990   -3.03274198
 C                  1.22907733    1.14520637   -4.53667476
 C                  0.86899664   -0.15401037   -4.67591690
 C                  0.41646135   -0.69220741   -3.30626713
 H                  2.84633872    1.79807486   -0.57286442
 H                  3.58417713    0.45269488    1.61433414
 H                  2.25867333   -1.86613385    1.67394733
 H                  0.68041002   -1.93297003   -0.47729702
 H                  2.25995331   -1.72422253   -1.16365685
 H                  0.34758579    0.47186652   -0.31293670
 H                  1.59846265    2.40898744   -2.61006054
 H                  1.43359982    1.82719143   -5.33540308
 H                  0.90053345   -0.71496352   -5.58654141
 H                 -0.65166569   -0.68039128   -3.24409792
 H                  0.75564524   -1.68866472   -3.11410283"""
        ...
        for ts_xyz in ts_xyzs:
            ...
            self.assertFalse(colliding_atoms(ts_xyz))
        coords_0 = np.array(ts_xyzs[0]['coords'], dtype=float)
        # At minimum, the 5-membered ring backbone should be preserved
...
H       1.55342226   -2.35901745    2.26912698"""
        ...
        rxn = ARCReaction(r_species=[r], p_species=[p])
        ts_xyzs = interpolate_isomerization(rxn)
        self.assertGreaterEqual(len(ts_xyzs), 1, msg='Ketoenol produced no TS guesses')

        # H8 migrates from donor O2 to acceptor C0; bridge atom is C1.
        # The 4-membered TS ring traverses H8 → O2 → C1 → C0 → H8.
        for gi, ts_xyz in enumerate(ts_xyzs):
            c = np.array(ts_xyz['coords'], dtype=float)

            # Distances among the three reactive atoms (donor O, migrating H, acceptor C).
            d_oh = float(np.linalg.norm(c[2] - c[8]))
            d_ch = float(np.linalg.norm(c[0] - c[8]))
            d_co = float(np.linalg.norm(c[0] - c[2]))
            self.assertTrue(1.3 < d_oh < 2.0,
                            msg=f'guess {gi}: O–H breaking distance out of TS range: {d_oh:.3f} Å')
            self.assertTrue(1.4 < d_ch < 2.2,
                            msg=f'guess {gi}: C–H forming distance out of TS range: {d_ch:.3f} Å')
            self.assertTrue(2.0 < d_co < 2.7,
                            msg=f'guess {gi}: C…O cross-ring distance out of TS range: {d_co:.3f} Å')

            # H–C–H angles on C0: static H6/H7 should still be sp²-like; migrating H8
            # should not crowd either of them.
            a_h6_h7 = _angle_deg(c[6], c[0], c[7])
            a_h6_h8 = _angle_deg(c[6], c[0], c[8])
            a_h7_h8 = _angle_deg(c[7], c[0], c[8])
            self.assertTrue(100.0 < a_h6_h7 < 130.0,
                            msg=f'guess {gi}: H–C–H angle between static H atoms is non-sp² '
                                f'({a_h6_h7:.1f}°)')
            self.assertGreater(min(a_h6_h8, a_h7_h8), 50.0,
                               msg=f'guess {gi}: migrating H8 crowds a static H atom on C0 '
                                   f'(H6–C0–H8={a_h6_h8:.1f}°, H7–C0–H8={a_h7_h8:.1f}°)')

            # 4-ring closure: angles sum ≈ 360° and none is degenerately sharp or splayed.
            a_at_h8 = _angle_deg(c[0], c[8], c[2])
            a_at_o2 = _angle_deg(c[8], c[2], c[1])
            a_at_c1 = _angle_deg(c[2], c[1], c[0])
            a_at_c0 = _angle_deg(c[1], c[0], c[8])
            ring_sum = a_at_h8 + a_at_o2 + a_at_c1 + a_at_c0
            self.assertTrue(345.0 < ring_sum < 365.0,
                            msg=f'guess {gi}: 4-ring is non-planar (angle sum {ring_sum:.1f}°, '
                                f'expected ≈360°)')
            for name, a in (('H8', a_at_h8), ('O2', a_at_o2), ('C1', a_at_c1), ('C0', a_at_c0)):
                self.assertTrue(60.0 < a < 130.0,
                                msg=f'guess {gi}: 4-ring angle at {name} = {a:.1f}° outside '
                                    f'[60°, 130°]')
    ...
    def test_interpolate_korcek_step1(self):
        ...
        """
        r_xyz = """ O                  1.46497838    1.17312399    1.00181460
 ...
 H                  0.76017293    0.75570075    1.71499461"""
        p_xyz = """O       2.24879740   -0.60288879   -0.61333491
C       1.57371209    0.04810206    0.45127457
C       0.22294576   -0.58878552    0.64811736
C      -0.61576351    0.24712755   -0.27994372
C      -2.09858764    0.18320384    0.02866078
...
H      -2.48665452   -0.83141064   -0.10098054
H      -2.65275729    0.85777810   -0.63272727"""
        r = ARCSpecies(label='R', smiles='O=CCC(C)OO', xyz=r_xyz)
        p = ARCSpecies(label='P', smiles='OC1CC(C)OO1', xyz=p_xyz)
        rxn = ARCReaction(r_species=[r], p_species=[p])
        ts_xyzs = interpolate_isomerization(rxn)
        self.assertGreaterEqual(len(ts_xyzs), 1)
        for ts_xyz in ts_xyzs:
            self.assertFalse(colliding_atoms(ts_xyz), msg=f'Collision in Korcek_step1 TS:\n{xyz_to_str(ts_xyz)}')
        coords_best = np.array(ts_xyzs[0]['coords'], dtype=float)
        d_oo = float(np.linalg.norm(coords_best[5] - coords_best[6]))
        self.assertGreater(d_oo, 1.3, msg=f'O-O distance {d_oo:.3f} Å is unphysically short (< 1.3 Å) in Korcek_step1 TS')
        ...
        if axis_norm > 1e-6:
            ...
            for h_idx in [11, 12, 13]:
                ...
                self.assertGreater(dot, -0.3, msg=f'H{h_idx} on terminal CH3 appears umbrella-inverted (dot={dot:.3f})')
        expected_ts = """O       2.05151175    1.24690464    0.18729467
C       1.63040685    0.08031738    0.40016878
C       0.49825841   -0.49996593   -0.46765101
C      -0.88832652   -0.09129418    0.06335712
C      -2.09475838   -0.24114070   -0.88198314
O      -0.78370731    0.72655231    1.23172434
O       0.47579796    0.93132565    1.56953249
H       2.06698037   -0.51943632    1.19879201
H       0.56053193   -1.58818432   -0.46599311
H       0.59807432   -0.13188142   -1.48875364
H      -0.75612115    0.91309122   -0.33892602
...
H       1.60421902    1.70916992    1.40821236"""
    ...
    def test_interpolate_korcek_step2(self):
        ...
        The Korcek_step2 RMG family explicitly prescribes a C-to-C hydrogen migration as part of the trioxolane fragmentation
        (the H originally bonded to the methyl-substituted ring carbon ends up on what becomes a methyl group of acetone).
        Korcek_step2 sits outside ARC's *default* RMG family set, so this test also exercises the recipe-revisit fallback
        in ``interpolate_addition`` which retries with the *all* family set when the default scan produces no product_dicts.
        """
...
H      -2.48665452   -0.83141064   -0.10098054
H      -2.65275729    0.85777810   -0.63272727"""
        p1_xyz = """C       1.27586049   -0.20311519   -0.02275731
C       0.00452324    0.57012170   -0.24664672
C      -1.27830828   -0.10541516    0.15623466
O       0.01347981    1.69903580   -0.73503936
H       1.24624439   -1.13132851   -0.59865800
...
H      -1.38833773   -1.04508735   -0.39212641
H      -2.12451706    0.54376791   -0.08169526"""
        p2_xyz = """O       0.80871183   -0.54502054   -0.13494187
C      -0.41707829   -0.03415895    0.05655464
O      -0.60811294    1.16486843    0.13270033
H       1.39322951    0.24033937   -0.18369670
H      -1.17675011   -0.82602831    0.12938360"""
        r = ARCSpecies(label='R', smiles='OC1CC(C)OO1', xyz=r_xyz)
        p1 = ARCSpecies(label='P1', smiles='CC(C)=O', xyz=p1_xyz)
        p2 = ARCSpecies(label='P2', smiles='OC=O', xyz=p2_xyz)
        rxn = ARCReaction(r_species=[r], p_species=[p1, p2])
        ts_xyzs = interpolate(rxn)
        self.assertEqual(rxn.family, 'Korcek_step2', msg=f'Expected Korcek_step2 to be detected via the wider '
        ...
        self.assertTrue(1.6 < d_c1_c2 < 2.4, msg=f'C[1]-C[2] should be stretched at the TS, got {d_c1_c2:.3f}')
        self.assertTrue(1.6 < d_o5_o6 < 2.4, msg=f'O[5]-O[6] peroxide should be stretched at the TS, got {d_o5_o6:.3f}')
        self.assertTrue(1.30 < d_c3_h11 < 1.95, msg=f'C[3]-H[11] (H leaving donor) should be stretched, got {d_c3_h11:.3f}')
        self.assertTrue(1.30 < d_c2_h11 < 1.95, msg=f'C[2]-H[11] (H arriving at acceptor) should be partially formed, got {d_c2_h11:.3f}')
        self.assertLess(abs(d_c3_h11 - d_c2_h11), 0.30, msg=f'donor-H ({d_c3_h11:.3f}) and acceptor-H ({d_c2_h11:.3f}) '
                                                            f'should be near-symmetric in the Korcek_step2 TS')
        expected_ts = """O       2.68197169   -0.52829330   -0.62136664
C       2.00688638    0.12269755    0.44324284
C       0.22294576   -0.58878552    0.64811736
C      -0.61576351    0.24712755   -0.27994372
C      -2.09858764    0.18320384    0.02866078
O      -0.12479259    1.56634922   -0.03969215
O       1.76771677    1.48797525    0.09701521
H       3.54727149   -0.08890978   -0.67544945
...
H       1.10527012   -0.12500938   -0.13651192"""
        ...
        rxn = ARCReaction(r_species=[r1, r2], p_species=[p])
        ts_xyzs = interpolate(rxn)
        expected_ts = """C                 -1.05582353   -0.20017926    0.05937677
C                  0.21933868    0.45983151    0.08061640
C                  0.41289183    2.36774420    0.48559338
O                  0.53245912    3.51169865    0.72806540
H                 -1.13119639   -1.25826023   -0.16835790
H                 -1.95511829    0.36523106    0.27350088
H                  1.10527012   -0.12500938   -0.13651192"""
        self.assertTrue(any(almost_equal_coords(ts, str_to_xyz(expected_ts)) for ts in ts_xyzs))
...
H      -1.52283795   -0.94788470   -0.43997119"""
        ...
        rxn = ARCReaction(r_species=[r], p_species=[p1, p2])
        ts_xyzs = interpolate(rxn)
        expected_ts = """C       2.24216061    0.49951640   -1.22347136
...
H       1.05323163    1.59589194    0.73495846"""
        ...
        rxn = ARCReaction(r_species=[r1, r2], p_species=[p])
        ts_xyzs = interpolate(rxn)
        expected_ts = """C      -3.92872757   -1.47192834    0.63358250
C      -2.94092110   -0.79153032    0.60258970
C      -1.77265412    0.01495047    0.56847031
C      -0.78622480   -0.02895120   -0.56871809
C      -0.77398201    1.22361456   -1.43102540
C      -1.51506692    2.33393107   -1.29713471
C       1.00458963   -0.47345031    0.09234486
C       1.49888248   -1.78070881   -0.04914028
C       2.76535913   -2.12002588    0.43121800
C       3.55238586   -1.16081647    1.06468647
C       3.07346674    0.13774286    1.22237142
C       1.80729139    0.47950705    0.74342457
H      -4.80587828   -2.07557590    0.66228852
H      -1.58412268    0.68341112    1.40170142
H      -1.09297182   -0.83522501   -1.25118213
H      -0.07170966    1.18775849   -2.26433101
H      -2.24816162    2.45558677   -0.50659652
H      -1.40408512    3.15690637   -1.99714322
H       0.89640201   -2.54673168   -0.53289310
H       3.13589338   -3.13499622    0.31356387
H       4.53740591   -1.42637416    1.43904218
H       3.68469729    0.88564612    1.72058619
H       1.44906432    1.49764207    0.88107652"""
        self.assertTrue(any(almost_equal_coords(ts, str_to_xyz(expected_ts)) for ts in ts_xyzs))
...
H       1.51207789    0.08805770   -2.98742858"""
    ...
    def test_interpolate_r_addition_multiplebond_10(self):
        ...
        rxn = ARCReaction(r_species=[r], p_species=[p1, p2])
        ts_xyzs = interpolate(rxn)
        self.assertGreaterEqual(len(ts_xyzs), 1, msg='No TS guesses produced')

        # Breaking bond is C2-O4 (carbonyl C to peroxide O); H8 leaves with the OO
        # fragment and must remain bonded to O5 only.
        coords_0 = np.array(ts_xyzs[0]['coords'], dtype=float)
        d_c2_o4 = float(np.linalg.norm(coords_0[1] - coords_0[3]))
        self.assertAlmostEqual(d_c2_o4, 1.85, delta=0.15,
                               msg=f'C2-O4 breaking bond outside TS range: {d_c2_o4:.3f} Å '
                                   f'(expected ≈ 1.85 Å)')
        d_h8_o5 = float(np.linalg.norm(coords_0[7] - coords_0[4]))
        self.assertLess(d_h8_o5, 1.2,
                        msg=f'H8 is no longer bonded to O5 (d = {d_h8_o5:.3f} Å)')
        for label, j in (('O3', 2), ('O4', 3), ('C1', 0), ('C2', 1)):
            d = float(np.linalg.norm(coords_0[7] - coords_0[j]))
            self.assertGreater(d, 1.5,
                               msg=f'H8 has drifted into proximity of {label} (d = {d:.3f} Å); '
                                   f'it should stay bonded to O5 only')
...
H      -2.39347065   -1.25775412    0.06240301
H       0.06681877   -2.19837465    0.09887848"""
        r = ARCSpecies(label='R', smiles='C=C1C=C[C]C1', xyz=r_xyz, multiplicity=1)
...
H       0.00000000    0.00000000   -0.46000000"""
        ...
        rxn = ARCReaction(r_species=[r], p_species=[p1, p2])
        ts_xyzs = interpolate(rxn)
        expected_ts_0 = """C       0.77428372   -0.04996907   -0.02995927
C      -0.73260318   -0.00572049   -0.04005672
F      -1.35300503   -0.60865979    1.50411482
F      -1.20409588   -0.83436495   -1.00353479
H       1.13143005   -1.07211215    0.12426434
H       1.18016366    0.33816078   -0.96843228
H      -0.17941771   -0.07379486    1.14050470
H      -1.12089093    1.00131310   -0.21289037"""
        expected_ts_1 = """C       0.77428372   -0.04996907   -0.02995927
C      -0.73260318   -0.00572049   -0.04005672
F      -1.20770160   -0.46744624    1.14245662
F      -1.34829653   -1.08779639   -1.29820355
H      -0.18719029   -1.04746517   -0.63051673
H       1.18016366    0.33816078   -0.96843228
H       1.16543618    0.56767746    0.78464237
H      -1.12089093    1.00131310   -0.21289037"""
        self.assertTrue(any(almost_equal_coords(ts, str_to_xyz(expected_ts_0)) for ts in ts_xyzs))
...
3 F u0 p3 c0 {2,S}""", xyz=p2_xyz, multiplicity=1)
        rxn = ARCReaction(r_species=[r], p_species=[p1, p2])
        ts_xyzs = interpolate(rxn)
        expected_ts = """F       1.95327962    1.03034032    0.80612159
C       1.35370769    0.07563009    0.05919636
F       2.11827091   -0.84347074   -0.57290547
C      -0.59971907    0.01196808   -0.08801457
F      -0.98882913   -0.48874727   -1.29265530
F      -1.14261278    1.25777426   -0.00729830
Cl     -1.28169420   -0.99746584    1.20199401"""
...
""", xyz=r_xyz, multiplicity=1)
        ...
        rxn = ARCReaction(r_species=[r1, r2], p_species=[p])
        ts_xyzs = interpolate(rxn)
        expected_ts = """F       2.03553319   -0.25885199   -1.05586477
C       1.25503616    0.30553443   -0.11724258
C      -0.64154676   -0.15618245    0.05993180
F      -1.42204376    0.40820397    0.99855402
H       1.80182660    1.01146404    0.48901400
H      -1.18833722   -0.86211206   -0.54632476"""
        self.assertTrue(any(almost_equal_coords(ts, str_to_xyz(expected_ts)) for ts in ts_xyzs))

    def test_interpolate_intra_h_migration_cco(self):
        """Test the interpolate_isomerization() function for intra H migration: CCO (4-membered ring): [CH2]CO <=> CC[O]"""
    ...
    def test_interpolate_intra_h_migration_ccoo(self):  # TODO: some could converge, H's incorrect
        ...
        for ts_xyz in ts_xyzs:
            ...
            for h_idx, sym in enumerate(ts_xyz['symbols']):
                ...
                # Nearest non-H atom overall (excluding self and reactive atoms).
                # Other H atoms can legitimately be closer (geminal H's).
                heavy_dists = [(j, float(np.linalg.norm(coords_arr[h_idx] - coords_arr[j])))
                               for j in range(len(ts_xyz['symbols']))
                               if j != h_idx and j not in (0, 3) and ts_xyz['symbols'][j] != 'H']
                if heavy_dists:
                    nearest_idx, nearest_dist = min(heavy_dists, key=lambda x: x[1])
                    self.assertLessEqual(
                        d_reactive_min, nearest_dist + 0.3,
                        msg=f'Migrating H[{h_idx}] is closer to heavy atom [{nearest_idx}] '
...
H       0.76371340   -0.19234475   -0.25650067"""
 ...
 H                  0.82596935    1.24150783    3.22846954"""
        expected_ts_2 = """O       1.77136558   -0.91790626    0.88650594
 N       1.34754589   -0.18857388   -0.01862669
 O       1.86645005   -0.03906737   -1.13182045
 ...
 H       0.95830534    2.46465213    0.41941724"""
        self.assertGreaterEqual(len(ts_xyzs), 1)
        matched = any(almost_equal_coords(ts, str_to_xyz(expected_ts_1)) for ts in ts_xyzs) or \
                  any(almost_equal_coords(ts, str_to_xyz(expected_ts_2)) for ts in ts_xyzs)
        self.assertTrue(matched, msg='No TS guess matched either expected geometry for intra_NO2_ONO_conversion')
...
H      -1.03086141    1.13813060    0.58426610"""
        ...
        for ts_xyz in ts_xyzs:
            ...
            if not any(1.7 < d < 2.2 for d in oo_dists):
                continue  # the breaking O-O must be stretched
            if not any(0.90 < d < 1.10 for d in oh_dists):
                continue  # the migrating H must still be near-bonded to an O
            # The forming C-O bond must be moderately long *and* the hydrogens already on
            # that carbon must be swung clear of it (angle H-C-O > 75°). Without the angle
            # the distance criteria alone are also satisfied by a geometry where the
            # incoming O is eclipsing a C-H, which is not an early TS.
            for ci in c_idx:
                for oi in o_idx:
                    co = coords[oi] - coords[ci]
                    co_d = float(np.linalg.norm(co))
                    if not 1.8 < co_d < 2.5:
                        continue
                    ch_vectors = [coords[hi] - coords[ci] for hi in h_idx
                                  if np.linalg.norm(coords[hi] - coords[ci]) < 1.2]
                    if not ch_vectors:
                        continue
                    angles = [float(np.degrees(np.arccos(np.clip(
                        float(np.dot(ch, co)) / (float(np.linalg.norm(ch)) * co_d), -1, 1))))
                        for ch in ch_vectors]
                    if all(angle > 75.0 for angle in angles):
                        found_good = True
                        break
                if found_good:
                    break
        ...
        self.assertTrue(found_good, msg='No TS guess has early-TS OH-migration characteristics: '
                                        ...
                                        'near-bonded migrating H on O (0.90–1.10 Å), and every H on the '
                                        'accepting carbon oriented > 75° away from the forming C-O bond')
...
H       0.08803888    0.04052072    2.16412333"""
        ...
        for a, b in zip(ring_atoms, ring_atoms[1:] + [ring_atoms[0]]):
            ...
            if ts_xyzs[0]['symbols'][a] == 'F' or ts_xyzs[0]['symbols'][b] == 'F':
                ...
                self.assertAlmostEqual(d, 2.04, delta=0.15, msg=f'F migration bond {a}-{b}: {d:.3f} Å')
            else:
                # C-C backbone bonds should be near typical single bond (~1.54 Å).
                self.assertAlmostEqual(d, 1.52, delta=0.10, msg=f'C-C backbone bond {a}-{b}: {d:.3f} Å')
...
H      -1.01192637   -0.87148015   -0.75982286"""
        ...
        # Intramolecular S↔CH3 swap: S2-C3 partly broken (~2.27 Å), C3-C1 partly formed
        # (~2.24 Å), and H4/H5 rotated out of the migration plane (D(C3-S2-C1-H4) ≈ -116°,
        # D(C3-S2-C1-H5) ≈ 83°) so they don't crowd the incoming CH3.
        def _is_good(ts_xyz: dict) -> bool:
            c = np.array(ts_xyz['coords'], dtype=float)
            d_s2_c3 = float(np.linalg.norm(c[1] - c[2]))
            d_c3_c1 = float(np.linalg.norm(c[2] - c[0]))
            if not (2.27 - 0.15) < d_s2_c3 < (2.27 + 0.15):
                return False
            if not (2.24 - 0.15) < d_c3_c1 < (2.24 + 0.15):
                return False
            d_3_2_1_4 = dihedral_deg(c[2], c[1], c[0], c[3])
            d_3_2_1_5 = dihedral_deg(c[2], c[1], c[0], c[4])
            if abs(d_3_2_1_4 - (-116.0)) > 10.0:
                return False
            if abs(d_3_2_1_5 - 83.0) > 10.0:
                return False
            return True

        self.assertTrue(any(_is_good(ts) for ts in ts_xyzs),
                        msg='No Intra_substitutionCS TS guess has S2–C3 ≈ 2.27 Å, C3–C1 ≈ 2.24 Å, '
                            'and H4/H5 dihedrals ≈ -116° / 83° relative to C3–S2–C1.')
...
H      -0.17919466    0.39223015   -3.14806886"""
        ...
        p_1 = ARCSpecies(label='P', smiles='CC1CCCS1', xyz=p_xyz)
        p_2 = ARCSpecies(label='CH3', smiles='[CH3]')
        rxn = ARCReaction(r_species=[r], p_species=[p_1, p_2])
        ts_xyzs = interpolate(rxn)
        expected_ts = """C      -3.33147467   -0.41330899    0.93687068
C      -2.13790604   -0.63163510    0.07402846
C      -2.32541520   -0.95568346   -1.37476606
C      -1.06085115   -1.56859001   -1.98320159
C      -0.23423695   -2.30848989   -0.93378281
S      -1.27846911   -2.93377209    0.42408327
C      -1.12284774   -5.15994032    1.50591366
H      -4.06811269   -1.20351865    0.79184272
H      -3.03783795   -0.41558441    1.98657173
H      -3.79273663    0.54822349    0.71149835
H      -1.17281129   -0.26391747    0.42256235
H      -3.15585133   -1.65279179   -1.48664013
H      -2.58784748   -0.04203749   -1.90813305
H      -1.34012369   -2.25895727   -2.77912865
H      -0.46428816   -0.77677958   -2.43624558
H       0.31913209   -3.13486455   -1.37986311
H       0.49767613   -1.62926693   -0.49667939
H      -2.05675652   -5.70950218    1.38803166
H      -0.32577456   -5.73032885    1.02901611
...
H       1.61593633   -0.33730052   -2.83543977"""
        ...
        for ts_xyz in ts_xyzs:
            ...
            self.assertFalse(colliding_atoms(ts_xyz))
        coords = np.array(ts_xyzs[0]['coords'], dtype=float)
        d_s1s2 = float(np.linalg.norm(coords[1] - coords[2]))
        d_c0s2 = float(np.linalg.norm(coords[0] - coords[2]))
        d_s2c3 = float(np.linalg.norm(coords[2] - coords[3]))
        self.assertGreater(d_s1s2, 2.2, msg=f'S1-S2 breaking bond {d_s1s2:.3f} Å not stretched toward TS (should be > 2.2)')
        self.assertLess(d_c0s2, 2.85, msg=f'C0-S2 forming bond {d_c0s2:.3f} Å not contracted toward TS (should be < 2.85)')
        self.assertAlmostEqual(d_s2c3, 1.81, delta=0.15, msg=f'S2-C3 spectator bond {d_s2c3:.3f} Å not preserved (should be ~1.81)')
    ...
    def test_interpolate_lone_electron_pair_bond(self):
        ...
        self.assertAlmostEqual(d_cn, 1.47, delta=0.15, msg=f'C-N distance {d_cn:.3f} Å deviates from expected ~1.47 Å')
        self.assertGreater(d_no, 1.4, msg=f'N-O distance {d_no:.3f} Å is too short for a forming-bond TS')
        self.assertLess(d_no, 2.5, msg=f'N-O distance {d_no:.3f} Å is too long for a forming-bond TS')
        expected_ts = """C      -0.76163009    0.03218892    0.04040734
N       0.71373651   -0.06236679   -0.03561989
O       1.51355990    1.60910947   -0.19701889
H      -1.12830607   -0.99127244    0.13234169
H      -0.99890388    0.62885765    0.92286051
H      -1.10010632    0.50278772   -0.88425834
H       1.03593116   -0.57425306    0.80820000
H       0.94413319   -0.68860760   -0.83098698"""
        self.assertTrue(any(almost_equal_coords(ts, str_to_xyz(expected_ts)) for ts in ts_xyzs))
    ...
    def test_interpolate_isomerization_ring_opening_no_crash(self):
        ...
        for ts_xyz in ts_xyzs:
            self.assertEqual(len(ts_xyz['symbols']), n_atoms, msg='TS guess has wrong atom count.')
            self.assertFalse(colliding_atoms(ts_xyz),
                msg=f'Collision detected in Cyclopentadiene_scission TS guess:\n{xyz_to_str(ts_xyz)}')
        ...
        for i in range(len(ts_xyzs)):
            for j in range(i + 1, len(ts_xyzs)):
                self.assertFalse(
                    ...
                    msg=f'Duplicate TS guesses at indices {i} and {j}: the almost_equal_coords dedup pass left a redundant entry.')

    def test_interpolate_isomerization_linear_alkyne_no_nan(self):
        """Integration test 3: sp-hybridized (linear) groups do not produce NaN or crash.
...
H       0.97222065   -1.40727159   -1.00427440"""
        ...
        for atom in r.mol.atoms:
            if atom.element.symbol == 'C':
                heavy = sum(1 for n in atom.bonds.keys() if n.element.symbol != 'H')
                hcount = sum(1 for n in atom.bonds.keys() if n.element.symbol == 'H')
                if heavy == 2 and hcount == 2:
                    internal_ch2_atoms.add(atom_to_idx[atom])
        self.assertGreater(len(internal_ch2_atoms), 0, msg='reactant OCCCC=C should expose internal CH₂ atoms '
        ...
        self.assertGreater(
            ...
            msg='Wiring did not invoke apply_reactive_center_cleanup even though the detector was forced to return True')
        ...
        self.assertTrue(
            any_call_has_ch2,
            msg='Wiring did not forward an internal CH₂ atom to the orchestrator under a forced-True detector '
                f'(reactive_centers per call: {[c["reactive_centers"] for c in orchestrator_calls]}, '
        ...
        self.assertGreater(len(ts_xyzs), 0, msg='Wiring caused the isomerization pipeline to '
                                                'produce zero TS guesses — the cleanup pass must be non-destructive.')
...
H       0.97222065   -1.40727159   -1.00427440"""
        ...
        with patch('arc.job.adapters.ts.linear.is_internal_reactive_ch2_misoriented', return_value=True), \
                patch('arc.job.adapters.ts.linear.apply_reactive_center_cleanup', side_effect=_spy_apply):
            interpolate_isomerization(rxn)

        self.assertGreater(len(captured), 0, msg='no orchestrator calls were captured — the wiring '
        ...
        for call in captured:
            ...
            for idx in range(n_atoms):
                if idx in allowed_indices:
                    continue
                self.assertTrue(np.allclose(call['in'][idx], call['out'][idx], atol=1e-9),
                                msg=f'atom {idx} was mutated by the cleanup pass even though it is outside the immediate '
                                    f'first shell of the reactive centers {reactive}')
...
H       0.97222065   -1.40727159   -1.00427440"""
        ...
        def _spy_detector(xyz, center_idx, heavy_nbr_indices, h_indices, **kwargs):
            natural_detector_calls.append(int(center_idx))
            return real_detector(xyz, center_idx, heavy_nbr_indices, h_indices, **kwargs)
        ...
        with patch('arc.job.adapters.ts.linear.is_internal_reactive_ch2_misoriented', side_effect=_spy_detector):
            ts_xyzs = interpolate_isomerization(rxn)

        # Sanity: the pipeline still produces guesses.
        self.assertGreater(len(ts_xyzs), 0, msg=' wiring caused the isomerization pipeline to produce zero TS guesses')
        ...
        ch2_natural_calls = [idx for idx in natural_detector_calls if idx in internal_ch2_atoms]
        self.assertGreater(len(ch2_natural_calls), 0,
                           msg=' wiring did not naturally invoke the internal-CH₂ detector for the OCCCC=C reaction — the '
                               'production-path wiring is not being exercised (natural detector invocations on any center: '
                               f'{natural_detector_calls}, expected internal CH₂ atoms: {internal_ch2_atoms})')
    ...
    def test_already_good_isomerization_geometry_unchanged_by_late_cleanup(self):
        ...
        terminal_cs = [atom_to_idx[atom] for atom in sp.mol.atoms
            if atom.element.symbol == 'C' and sum(1 for n in atom.bonds.keys() if n.element.symbol != 'H') == 1]
        middle_c = next(atom_to_idx[atom] for atom in sp.mol.atoms
            if atom.element.symbol == 'C' and sum(1 for n in atom.bonds.keys() if n.element.symbol != 'H') == 2)
...
class TestCleanupAfterExistingHMigration(unittest.TestCase):
    """Tests for ``cleanup_after_existing_h_migration``.

    Contract under test: the helper composes ``orient_h_away_from_axis``
    and ``apply_reactive_center_cleanup(..., migrations=None, ...)`` so
    the migrating H is *not* re-triangulated, and donor / acceptor shells
    receive only the orient + regularize + symmetry passes."""

    @classmethod
    def setUpClass(cls):
        # Propane (11 atoms: C-C-C plus 8 H) — same fixture as the
        # late-cleanup tests above. The terminal C atoms are the stand-in
        # donor and acceptor; one of the terminal H atoms is the stand-in
        # migrating H. The helper does not validate chemistry — only that
        # indices are in range and the molecule has the expected bonds.
        cls.sp = ARCSpecies(label='propane', smiles='CCC')
        cls.xyz = cls.sp.get_xyz()
        atom_to_idx = {a: i for i, a in enumerate(cls.sp.mol.atoms)}
        cls.terminal_cs = sorted(
            atom_to_idx[atom] for atom in cls.sp.mol.atoms
            if atom.element.symbol == 'C'
            and sum(1 for n in atom.bonds if n.element.symbol != 'H') == 1)
        # Pick one H bonded to terminal_cs[0] as the migrating H.
        c0 = cls.sp.mol.atoms[cls.terminal_cs[0]]
        cls.h_on_c0 = next(atom_to_idx[n] for n in c0.bonds
                           if n.element.symbol == 'H')

    def test_empty_migration_records_is_noop(self):
        """No migrations → byte-for-byte unchanged xyz (trivial case)."""
        before = np.asarray(self.xyz['coords'], dtype=float).copy()
        out = cleanup_after_existing_h_migration(self.xyz, self.sp.mol, [])
        after = np.asarray(out['coords'], dtype=float)
        self.assertEqual(before.shape, after.shape)
        for i in range(before.shape[0]):
            self.assertTrue(np.allclose(before[i], after[i], atol=1e-12),
                            msg=f'atom {i} moved on an empty-migrations call')

    def test_preserves_atom_count_and_symbols(self):
        """A single migration record returns a valid xyz (normal case)."""
        rec = {'h_idx': self.h_on_c0,
               'donor': self.terminal_cs[0],
               'acceptor': self.terminal_cs[1]}
        out = cleanup_after_existing_h_migration(self.xyz, self.sp.mol, [rec])
        self.assertEqual(out['symbols'], self.xyz['symbols'])
        self.assertEqual(len(out['coords']), len(self.xyz['coords']))

    def test_migrating_h_position_preserved(self):
        """The migrating H is exempt from cleanup → its position is preserved."""
        rec = {'h_idx': self.h_on_c0,
               'donor': self.terminal_cs[0],
               'acceptor': self.terminal_cs[1]}
        h_before = np.asarray(self.xyz['coords'][self.h_on_c0], dtype=float)
        out = cleanup_after_existing_h_migration(self.xyz, self.sp.mol, [rec])
        h_after = np.asarray(out['coords'][self.h_on_c0], dtype=float)
        self.assertTrue(np.allclose(h_before, h_after, atol=1e-12),
                        msg='migrating H moved despite being in exempt_h_indices')

    def test_does_not_re_triangulate_h(self):
        """``migrations=None`` invariant: a manifestly off-axis migrating H is
        left in place (the helper must not call ``clean_migrating_h`` to snap
        it back to the donor-acceptor triangulation point — that's the whole
        reason the wrapper exists)."""
        # Move the migrating H far from any sensible TS position.
        coords = [list(c) for c in self.xyz['coords']]
        coords[self.h_on_c0] = [10.0, 10.0, 10.0]
        wild_xyz = {'symbols': self.xyz['symbols'],
                    'isotopes': self.xyz['isotopes'],
                    'coords': tuple(tuple(c) for c in coords)}
        rec = {'h_idx': self.h_on_c0,
               'donor': self.terminal_cs[0],
               'acceptor': self.terminal_cs[1]}
        out = cleanup_after_existing_h_migration(wild_xyz, self.sp.mol, [rec])
        h_after = np.asarray(out['coords'][self.h_on_c0], dtype=float)
        self.assertTrue(np.allclose(h_after, [10.0, 10.0, 10.0], atol=1e-12),
                        msg='migrating H was re-triangulated; helper should '
                            'pass migrations=None to avoid clean_migrating_h')

    def test_multiple_migrations_all_h_preserved(self):
        """Two migration records: every migrating H position is preserved (extreme case)."""
        atom_to_idx = {a: i for i, a in enumerate(self.sp.mol.atoms)}
        c1 = self.sp.mol.atoms[self.terminal_cs[1]]
        h_on_c1 = next(atom_to_idx[n] for n in c1.bonds
                       if n.element.symbol == 'H')
        recs = [{'h_idx': self.h_on_c0,
                 'donor': self.terminal_cs[0],
                 'acceptor': self.terminal_cs[1]},
                {'h_idx': h_on_c1,
                 'donor': self.terminal_cs[1],
                 'acceptor': self.terminal_cs[0]}]
        h0_before = np.asarray(self.xyz['coords'][self.h_on_c0], dtype=float)
        h1_before = np.asarray(self.xyz['coords'][h_on_c1], dtype=float)
        out = cleanup_after_existing_h_migration(self.xyz, self.sp.mol, recs)
        h0_after = np.asarray(out['coords'][self.h_on_c0], dtype=float)
        h1_after = np.asarray(out['coords'][h_on_c1], dtype=float)
        self.assertTrue(np.allclose(h0_before, h0_after, atol=1e-12),
                        msg='first migrating H moved')
        self.assertTrue(np.allclose(h1_before, h1_after, atol=1e-12),
                        msg='second migrating H moved')
