...
def _save_debug_geometries(ts_xyzs, rxn, out_dir: Optional[str] = None) -> None:
    """Optionally dump current reaction TS guesses for visual inspection.

    The helper activates when EITHER:

    * the environment variable ``ARC_LINEAR_DEBUG_DUMP`` is set to a
    ...
    * the output directory (default ``~/Desktop/xyz/linear/``) already
    ...
    Args:
        ts_xyzs: TS guess XYZ dicts to dump.
        rxn: Reaction whose reactants/products are also dumped.
        out_dir: Optional output directory.  If ``None`` (default),
            ``~/Desktop/xyz/linear`` is used.  Tests pass an explicit
            path so they can verify the inertness contract against a
            temp directory without mocks.
    """
    if out_dir is None:
        out_dir = os.path.expanduser('~/Desktop/xyz/linear')
    enabled_raw = os.environ.get(_LINEAR_DEBUG_DUMP_ENV, '')
    enabled_by_env = bool(enabled_raw and enabled_raw.strip() not in ('', '0', 'false', 'False'))
    enabled_by_dir = os.path.isdir(out_dir)
    if not enabled_by_env and not enabled_by_dir:
        return  # Inert in committed test runs.
    try:
        ...
        # The helper must NEVER make a test fail.
        pass
...
H       1.65446241   -1.85341259    0.04404524"""
        ...
        # Verify the first TS guess has reasonable forming-bond distance
        coords_0 = np.array(ts_xyzs[0]['coords'], dtype=float)
...
H       0.31648739    2.42027069    0.39137850"""
 ...
 H                  0.75564524   -1.68866472   -3.11410283"""
        ...
        expected_ts = """C       0.25747366    0.56171008    1.87708985
...
H       0.94413319   -0.68860760   -0.83098698"""
    ...
    def test_interpolate_isomerization_ring_opening_no_crash(self):
        ...
        _save_debug_geometries(ts_xyzs, self.rxn_1)
        self.assertIsNotNone(ts_xyzs)
        self.assertIsInstance(ts_xyzs, list)
