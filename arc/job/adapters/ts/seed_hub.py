...
The XY-addition constants describe the four-center ring of ``XY_Addition_MultipleBond``:
``XY_ADDITION_REACTIVE_LABELS`` are its RMG atom labels (``*1`` and ``*2`` the multiple-bond termini, ``*3`` and ``*4``
the X and Y ends of the adding bond), and ``XY_ADDITION_DISTANCE_LABEL_PAIRS`` are all six pairwise separations among
them, which determine that ring rigidly up to a reflection in the same way.
"""
...
XY_ADDITION_REACTIVE_LABELS = ('*1', '*2', '*3', '*4')

XY_ADDITION_DISTANCE_LABEL_PAIRS = (('*1', '*2'),
                                    ('*1', '*3'),
                                    ('*1', '*4'),
                                    ('*2', '*3'),
                                    ('*2', '*4'),
...
def _get_crest_constraints(reaction: 'ARCReaction', seed: dict) -> Optional[dict]:
    ...
    ``XY_Addition_MultipleBond`` pins all six pairwise distances among the four ring atoms
    ``*1``, ``*2``, ``*3`` and ``*4``, which are read from the seed metadata's ``reactive_atoms``
    mapping. ``None`` is returned when that mapping is absent or does not resolve to four distinct,
    in-range atoms.
    ...
    if family == 'XY_Addition_MultipleBond':
        if _is_valid_xy_atom_assignment(xyz=xyz, atoms=explicit_atoms):
            return {
                'atoms': tuple(explicit_atoms[label] for label in XY_ADDITION_REACTIVE_LABELS),
                'distance_pairs': tuple((explicit_atoms[label_1], explicit_atoms[label_2])
                                        for label_1, label_2 in XY_ADDITION_DISTANCE_LABEL_PAIRS),
            }
        if explicit_atoms is not None:
...
def _is_valid_xy_atom_assignment(xyz: dict, atoms: Optional[Dict[str, int]]) -> bool:
    ...
    symbols = xyz.get('symbols') if isinstance(xyz, dict) else None
    if not symbols or not isinstance(atoms, dict) or set(atoms) != set(XY_ADDITION_REACTIVE_LABELS):
        return False
    indices = tuple(atoms[label] for label in XY_ADDITION_REACTIVE_LABELS)
