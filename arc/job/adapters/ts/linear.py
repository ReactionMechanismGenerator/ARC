...
def _reposition_migrating_atom(xyz: dict,
    ...
    # Identify the fragment bonded to mig_idx from the molecular graph,
    # blocking donor and acceptor so they don't move.
    adj = mol_to_adjacency(mol)
...
def interpolate_addition(rxn: 'ARCReaction',
    ...
    # The bespoke builder constructs the ring TS directly from the
    # template bb/fb, bypassing the generic stretch_bond() path.
...
def _validate_addition_builder_geometry(xyz: dict,
    ...
    """
    return validate_addition_guess(
        xyz=xyz, uni_mol=uni_mol, forming_bonds=list(forming_bonds or []),
...
def _strategy_3center_shift(ctx: _PathContext) -> _StrategyResult:
    ...
    if n_heavy_pivot >= 3:
        ...
        # Step 1: Stretch the bb_other-pivot bond: move bb_other (and
        # its H's) AWAY from pivot.
        ts_3c = _build_ring_scission_ts(
            ctx.r_xyz, breaking_bonds=[(bb_other, pivot)],
            weight=ctx.weight, stretch_factor=1.6, mol=ctx.r_mol)
        # Step 2: Contract the fb_other-pivot bond: move fb_other (and
        # its fragment excluding the pivot) TOWARD pivot.  Without this
        # step the forming bond stays at the reactant distance, making
        # the 3-membered-ring TS incomplete.
        if ts_3c is not None:
            ...
            if d_fp > 1e-6:
                ...
                if d_fp > d_target_form:
                    ...
                    # Identify the fragment connected to fb_other
                    # (excluding pivot AND bb_other) via BFS on the
                    # molecular graph.  Excluding bb_other prevents
                    # the contraction from dragging the breaking-bond
                    # partner along.
                    ...
                    ts_3c = {
                        'symbols': ts_3c['symbols'],
                        'isotopes': ts_3c['isotopes'],
                        'coords': tuple(tuple(float(v) for v in row)
                                        for row in coords_3c),
                    }
    else:
...
def _try_bespoke_family_fallbacks(
    ...
    if ts_carbene is not None and not colliding_atoms(ts_carbene):
        ...
        out.append(GuessRecord(
            xyz=ts_carbene, bb=[], fb=[],
...
def interpolate_isomerization(rxn: 'ARCReaction',
    ...
    * **Type P** (product-topology): Symmetric to Type R.  A Z-matrix is built from the
    ...
    for i, product_dict in enumerate(rxn.product_dicts):
        ...
        # Bespoke route for Intra_Retro_Diels_alder_bicyclic.
        # The per-path atom map (map_rxn) hangs for bicyclic ring-bond
        # cuts because scissors() calls generate_conformers() on a large
        # monocyclic fragment.  Bypass the atom map entirely: stretch
        # both breaking bonds directly from the reactant geometry.
        if path_family == 'Intra_Retro_Diels_alder_bicyclic' and bb and not fb:
            ...
            if ts_da is not None and not colliding_atoms(ts_da):
                logger.debug(f'Linear (rxn={rxn.label}, path={i}): used bespoke '
                             f'Intra_Retro_Diels_alder_bicyclic builder.')
                ts_xyzs.append(GuessRecord(
                    xyz=ts_da,
                    bb=list(bb), fb=list(fb),
                    family=path_family,
            ...
            continue  # Skip map_rxn entirely for this family
    ...
    # Fast-exit bespoke family fallback: when the strategy pipeline has not
    # produced any guesses yet (typically because atom mapping failed and
    # every path was skipped), try the narrow family-specific builders so
    # the pipeline can recover without going through the generic plumbing.
    # A second invocation after post-processing catches the complementary
    # failure mode where the strategy pipeline DID produce guesses that
    # were subsequently dropped by path-spec validation.
    if not ts_xyzs:
    ...
    _has_bespoke = any(getattr(g, 'strategy', '').startswith('bespoke_')
                       for g in ts_xyzs)
    # Only compute the global atom map if it may be needed (no bespoke
    # guesses).  For large bicyclic molecules, rxn.atom_map computation
    # can hang due to expensive conformer generation in scissors().
    if not _has_bespoke:
        _ensure_global_atom_map()
    if not ts_xyzs or (initial_atom_map is None and not _has_bespoke):
        ...
        if r_xyz['symbols'] == p_xyz['symbols']:
            ...
            try:
                ...
                # Always restore original atom_map to avoid cross-reaction side effects,
                # even if order_xyz_by_atom_map or any other call above raises.
    ...
    # Last-resort bespoke fallback: if post-processing (notably the
    # path-spec wrapper at step 6) rejected every generic guess — e.g.,
    # the Z-matrix chimera leaves a breaking bond at equilibrium length
    # because the "reactive" flag did not propagate to the right var —
    # try the family-specific builders.  They carry no path_spec, so the
    # wrapper cannot reject them on the same criterion; and they produce
    # a geometry designed directly from the recipe, not an average.
    if not unique:
