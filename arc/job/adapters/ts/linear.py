...
def _apply_internal_ch2_cleanup_to_isomerization_record(
    ...
    First, a dedicated reacting-terminal-CH₂ pass reseats the H atoms of any
    terminal =CH₂ reactant carbon that gains an internal (cyclopropane-like)
    character through a forming heavy–heavy bond, using
    ``[reactant_neighbor, forming_partner]`` as its heavy frame. This is needed
    because such a center keeps a reactant heavy-degree of 1 and is therefore
    invisible to the spectator-expansion pass below.

    The spectator-expansion pass is strictly opt-in and runs only when ALL of:
    ...
    for bond_pair in (rec.fb or []):
        ep_a, ep_b = int(bond_pair[0]), int(bond_pair[1])
        for center, partner in ((ep_a, ep_b), (ep_b, ep_a)):
            if not (0 <= center < len(r_mol.atoms) and 0 <= partner < n_atoms_iso):
                continue
            if symbols_iso[center] == 'H' or symbols_iso[partner] == 'H':
                continue
            graph_heavy = [atom_to_idx_iso[nbr] for nbr in r_mol.atoms[center].bonds.keys()
                           if nbr.element.symbol != 'H']
            ch2_h = [atom_to_idx_iso[nbr] for nbr in r_mol.atoms[center].bonds.keys()
                     if nbr.element.symbol == 'H']
            if len(graph_heavy) != 1 or len(ch2_h) != 2 or partner in graph_heavy:
                continue
            heavy_pair = [graph_heavy[0], partner]
            if is_internal_reactive_ch2_misoriented(rec.xyz, center_idx=center,
                                                    heavy_nbr_indices=heavy_pair, h_indices=ch2_h):
                rec.xyz = repair_internal_reactive_ch2(rec.xyz, center_idx=center,
                                                       heavy_nbr_indices=heavy_pair, h_indices=ch2_h)
...
def cleanup_after_existing_h_migration(ts_xyz: dict,
                                       uni_mol: Molecule,
                                       migration_records: list[dict],
                                       ) -> dict:
    """
    Reactive-center cleanup when the migrating H is already triangulated upstream.

    Both callers (the template-guided post-migrate pass and the frag-fallback
    post-migrate pass) have already had :func:`migrate_verified_atoms` /
    :func:`migrate_h_between_fragments` place each migrating H at its
    donor-acceptor TS position. This helper orients spectator H atoms around
    each (donor → acceptor) axis, regularizes terminal H bond lengths, and
    restores terminal CH₂/CH₃ symmetry on the donor / acceptor shells —
    without re-triangulating the migrating H (``migrations=None`` so
    :func:`clean_migrating_h` is not invoked a second time).

    Args:
        ts_xyz: TS guess XYZ; the migrating H is already at its TS position.
        uni_mol: Reactant RMG Molecule.
        migration_records: List of ``{'h_idx', 'donor', 'acceptor'}`` dicts
            from :func:`identify_h_migration_pairs` or
            :func:`infer_frag_fallback_h_migration`. May be empty (no-op).

    Returns:
        ``ts_xyz`` with per-shell cleanup applied, or unchanged when ``migration_records`` is empty.
    """
    centers: set[int] = set()
    exempt_hs: set[int] = set()
    for rec in migration_records:
        h_idx = int(rec['h_idx'])
        donor = int(rec['donor'])
        acceptor = int(rec['acceptor'])
        ts_xyz = orient_h_away_from_axis(
            ts_xyz, uni_mol, donor, acceptor, exclude_h={h_idx})
        centers.add(donor)
        centers.add(acceptor)
        exempt_hs.add(h_idx)
    if not centers:
        return ts_xyz
    return apply_reactive_center_cleanup(
        ts_xyz, uni_mol,
        migrations=None,
        reactive_centers=centers,
        exempt_h_indices=exempt_hs,
        restore_symmetry=True,
    )
...
def interpolate_addition(rxn: ARCReaction,
        ...
        if frag_map is not None:
            ...
            for base_xyz in bases:
                ...
                if ts_xyz is not None and migrating_atoms:
                    ...
                    ts_xyz = cleanup_after_existing_h_migration(
                        ts_xyz, uni_mol, migration_records)
    ...
    for cut in isomorphism_verified:
        comp_key = _frag_composition_key(uni_mol, uni_symbols, cut)
    ...
    for cut in cut_lists:
        ...
        for ring_xyz in ring_xyzs:
            ...
            ts_xyz = cleanup_after_existing_h_migration(
                ts_xyz, uni_mol, frag_migrations)
    ...
    if entry is not None and ctx.bb and ctx.fb:
        ...
        if ts is not None and not colliding_atoms(ts):
            ...
            return _StrategyResult(
                ...
                halt=False)
    if ctx.family == 'Singlet_Carbene_Intra_Disproportionation':
        ts = build_singlet_carbene_intra_disproportionation_ts(r_xyz=ctx.r_xyz, r_mol=ctx.r_mol)
        if ts is not None and not colliding_atoms(ts):
            logger.debug(f'Linear ({ctx.label}): used bespoke '
                         f'Singlet_Carbene_Intra_Disproportionation builder.')
            return _StrategyResult(guesses=[GuessRecord(xyz=ts, bb=[], fb=[], family=ctx.family,
                                                        strategy='bespoke_singlet_carbene')], halt=False)
    return _StrategyResult()
