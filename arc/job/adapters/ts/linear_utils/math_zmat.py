    ...
    for part in parts[1:]:
        for idx_str in part.split('|'):
            try:
                ...
                logger.debug(f"Skipping non-integer token '{idx_str}' while parsing Z-matrix variable '{var_name}'.")
    return atoms
...
def get_r_constraints(expected_breaking_bonds: list[tuple[int, int]],
    ...
    Returns a *minimal* canonical R-constraint set: at most one constraint per
    reactive atom, picked greedily in descending frequency order (most-frequent
    atom first, atom index as deterministic tiebreaker). For each atom we keep
    the first bond that does not share an endpoint with an already-served
    higher-frequency atom — so an atom appearing in multiple breaking/forming
    bonds (e.g. a migrating H in both an O–H break and a C–H form) contributes
    one constraint, not several. This avoids over-constraining the optimizer.
    ...
    atoms_sorted_by_frequency = [k for k, _ in sorted(atom_occurrences.items(), key=lambda item: (-item[1], item[0]))]
...
def average_zmat_params(zmat_1: dict,
                        zmat_2: dict,
                        weight: float = 0.5,
                        reactive_xyz_indices: set[int] | None = None,
                        ) -> dict | None:
    """
    ...
    * **All values** are cast to ``float`` before arithmetic to prevent silent integer
    ...
    Args:
        ...
        reactive_xyz_indices (set[int] | None): XYZ atom indices of atoms that participate
    ...
    Returns:
        dict | None: The weighted average ZMat, or ``None`` if the inputs are
...
def get_rxn_weight(rxn: ARCReaction,
                   ...
                   reorg_energy: float | tuple[float, float] | None = None,
                   ) -> float | None:
    """
