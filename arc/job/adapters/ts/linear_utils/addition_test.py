...
MASS_NUMBER = {'H': 1, 'C': 12, 'N': 14, 'O': 16, 'S': 32}
...
class TestStretchBond(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.ethane_xyz = {'symbols': tuple(symbols),
                          'isotopes': tuple(MASS_NUMBER[a.symbol] for a in cls.ethane_mol.atoms),
    ...
    def test_stretch_bond_increases_distance(self):
        ...
        split_bond = (c0, c1)
        d_before = np.linalg.norm(np.array(self.ethane_xyz['coords'][c0]) - np.array(self.ethane_xyz['coords'][c1]))

        result = stretch_bond(self.ethane_xyz, self.ethane_mol, [split_bond], weight=0.5, label='test')
        self.assertIsNotNone(result)
    ...
    def test_stretch_bond_preserves_symbols(self):
        ...
        c0, c1 = self.ethane_c_indices
        result = stretch_bond(self.ethane_xyz, self.ethane_mol, [(c0, c1)], weight=0.5, label='test')
        self.assertIsNotNone(result)
...
class TestDetectIntraFragRingBonds(unittest.TestCase):
    ...
    def test_detect_ring_bonds_butadiene_cyclobutene(self):
        ...
        xyz = {'symbols': tuple(symbols),
               'isotopes': tuple(MASS_NUMBER[a.symbol] for a in mol.atoms),
               'coords': tuple(tuple(c) for c in coords)}
...
class TestBuildConcertedTs(unittest.TestCase):
    ...
    def test_concerted_preserves_symbols(self):
        ...
        # TODO: pair with a happy-path fixture that actually produces a TS.
        self.assertIsNone(result)
