...
def _xyz_distance(xyz: dict, i: int, j: int) -> float | None:
...
def _coords_distance(coords: np.ndarray, i: int, j: int) -> float | None:
    """
    ...
    Returns:
        float | None: The distance in Å, or ``None`` if either index is
            out of range.
    """
...
def _is_changed_bond_frontier_exempt(bi: int,
                                     bj: int,
                                     bo_r_local: float | None,
                                     bo_p_local: float | None,
    ...
    Args:
        ...
        bo_r_local (float | None): Reactant-side bond order; ``None``
            disables the exemption.
        bo_p_local (float | None): Product-side bond order; ``None``
            disables the exemption.
...
class ReactionPathSpec:
    ...
    *what* changes between reactant and product for one specific path, in a
    ...
    bond_order_r: dict[CanonicalBond, float | None] = field(default_factory=dict)
    bond_order_p: dict[CanonicalBond, float | None] = field(default_factory=dict)

    # Per-bond reference distances (None if not available).
    ref_dist_r: dict[CanonicalBond, float | None] = field(default_factory=dict)
    ref_dist_p: dict[CanonicalBond, float | None] = field(default_factory=dict)

    @classmethod
    def build(cls,
              r_mol: Molecule,
              mapped_p_mol: Molecule | None,
              breaking_bonds,
              forming_bonds,
              r_xyz: dict | None = None,
              op_xyz: dict | None = None,
              weight: float = 0.5,
              family: str | None = None,
              ) -> ReactionPathSpec:
        """
        ...
        bond_order_r: dict[CanonicalBond, float | None] = {}
        bond_order_p: dict[CanonicalBond, float | None] = {}
        ref_dist_r: dict[CanonicalBond, float | None] = {}
        ref_dist_p: dict[CanonicalBond, float | None] = {}
...
def insertion_ring_extra_stretch(family: str | None) -> float:
    """
...
def has_recipe_channel_mismatch(path_spec: ReactionPathSpec,
    ...
    Conservative, element-aware checks. The breaking-bond floor and forming-bond ceiling scale with
    ``get_single_bond_length(symbols[i], symbols[j])`` so the same gate works across X–H, heavy–heavy,
    and long-bond pairs:

    1. **Failed to break** — any bond in ``path_spec.breaking_bonds`` whose distance is below
       ``0.85 × sbl(i, j)`` (significantly compressed, indicating no stretch happened or the bond actually got shorter).
    2. **Failed to form** — any bond in ``path_spec.forming_bonds`` whose distance exceeds
      ``2.0 × sbl(i, j)`` (atoms too far apart for any partial-bond character).
    3. **Snapped spectator** — any bond in ``path_spec.unchanged_near_core_bonds``
       whose distance exceeds ``1.25 × sbl(i, j)``.
    ...
    # 1. Failed to break: bond significantly compressed below equilibrium.
    for i, j in path_spec.breaking_bonds:
        sbl = get_single_bond_length(symbols[i], symbols[j]) or 1.5
        floor = 0.85 * sbl
        d = float(np.linalg.norm(coords[i] - coords[j]))
        if d < floor:
            return True, f'failed-to-break: {symbols[i]}{i}-{symbols[j]}{j}={d:.3f} < {floor:.3f}'

    # 2. Failed to form: atoms further than 2× equilibrium for that element pair.
    for i, j in path_spec.forming_bonds:
        sbl = get_single_bond_length(symbols[i], symbols[j]) or 1.5
        ceiling = 2.0 * sbl
        d = float(np.linalg.norm(coords[i] - coords[j]))
        if d > ceiling:
            return True, f'failed-to-form: {symbols[i]}{i}-{symbols[j]}{j}={d:.3f} > {ceiling:.3f}'
    ...
    for i, j in path_spec.unchanged_near_core_bonds:
        ...
        d = float(np.linalg.norm(coords[i] - coords[j]))
        if d > limit:
            return True, f'snapped-spectator: {symbols[i]}{i}-{symbols[j]}{j}={d:.3f} > {limit:.3f}'
...
def score_guess_against_path_spec(path_spec: ReactionPathSpec,
                                  ...
                                  r_mol: Molecule | None,
                                  symbols: tuple[str, ...],
                                  chemistry: PathChemistry | None = None,
                                  ) -> float:
    """
...
def validate_guess_against_path_spec(xyz: dict,
                                     ...
                                     migrating_hs: set[int] | None = None,
                                     label: str = '',
