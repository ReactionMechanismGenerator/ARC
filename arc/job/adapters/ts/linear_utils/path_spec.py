...
def _build_adjacency(mol: 'Molecule') -> Dict[int, Set[int]]:
    """Build a {atom_index -> {neighbor_index, ...}} adjacency dict.

    Delegates to :func:`geom_utils.mol_to_adjacency`.
    """
    return mol_to_adjacency(mol)
    ...
    *what* changes between reactant and product for one specific path, in a
    ...
    def build(cls,
        ...
        if mapped_p_mol is not None:
            ...
            # Without a mapped product molecule, changed-bond inference
            # is unreliable (comparing reactant against itself would
            # always yield []).  Use an empty list explicitly.
            changed = []
    ...
    * Reverse-template ``R_Addition_MultipleBond``: a wrong radical
    ...
    for ep in sorted(heavy_reactive_endpoints):
        ...
        for k in range(n_atoms):
            ...
            if symbols[k] == 'H':
                continue
            # Exempt existing multiple bonds (bo >= 2.0) in the
            # reactant graph.  Their equilibrium distances are
            # intrinsically shorter than 0.85 × sbl(single) and are
            # not a sign of committed spectator contact:
            #   C≡O ~1.13 Å  (bo=3)  vs 0.85×sbl(C,O) = 1.22 Å
            #   C=C ~1.30 Å  (bo=2)  vs 0.85×sbl(C,C) = 1.31 Å
            # Single bonds (bo < 2) are NOT exempt — a short single
            # bond between a spectator and a reactive endpoint is a
            # genuine committed-spectator signal.
