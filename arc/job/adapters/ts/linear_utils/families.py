...
def build_intra_oh_migration_ts(
        r_xyz: dict,
        r_mol: 'Molecule',
        breaking_bonds: List[Tuple[int, int]],
        forming_bonds: List[Tuple[int, int]],
    ...
    # Identify the forming C-O bond and breaking O-O bond.
    forming_co = None
    for a, b in fb:
    ...
    # The migrating O must be in both the forming and breaking bonds.
    ...
    # Step 1: Use ring_closure_xyz to fold the chain.
    # Calibrated early-TS C-O forming distance: ~2.08 Å
    # (larger than Pauling sbl+delta=1.85 because this is an early TS).
    ...
    # Step 2: Stretch the O-O breaking bond to calibrated TS distance.
    # Expected: ~1.90 Å (sbl(O,O) + PAULING_DELTA ≈ 1.48 + 0.42).
    sbl_oo = get_single_bond_length('O', 'O') or 1.48
    ...
    if oo_dist > 1e-6 and oo_dist < d_oo_target:
        ...
        # Move o_migrating and its H-only fragment away from o_other.
    ...
    # Step 3: Orient spectator H atoms on c_radical perpendicular to
    # the C→O_migrating forming-bond axis.  This prevents them from
    # leaning toward the approaching OH.
    ...
    if co_dist > 1e-6:
        ...
        for nbr in r_mol.atoms[c_radical].bonds.keys():
            ...
            if symbols[ni] == 'H':
                h_on_c.append(ni)
        if len(h_on_c) >= 2:
            # Find the other heavy neighbor of c_radical (the backbone C).
            backbone_c = None
            for nbr in r_mol.atoms[c_radical].bonds.keys():
                ni = atom_to_idx[nbr]
            ...
            if backbone_c is not None:
                # Place H atoms in a cone perpendicular to co_hat,
                # centered on the c_radical → backbone direction.
                cb_vec = coords[backbone_c] - coords[c_radical]
                cb_hat = cb_vec / max(float(np.linalg.norm(cb_vec)), 1e-10)
                # Use the original H positions' perpendicular components
                # relative to co_hat, then ensure they're at ~90° from
                # co_hat (perpendicular to the forming bond).
                sbl_ch = get_single_bond_length('C', 'H') or 1.09
                for hi in h_on_c:
                    ...
                    if h_dist < 1e-6:
                        continue
                    # Remove the co_hat component.
                    h_perp = h_vec - co_hat * np.dot(h_vec, co_hat)
                    h_perp_norm = float(np.linalg.norm(h_perp))
                    if h_perp_norm > 1e-8:
                        h_perp_hat = h_perp / h_perp_norm
                    else:
                        # Fallback: use an arbitrary perpendicular.
                        arb = np.array([1.0, 0.0, 0.0])
                        if abs(float(np.dot(co_hat, arb))) > 0.9:
                            arb = np.array([0.0, 1.0, 0.0])
                        h_perp_hat = np.cross(co_hat, arb)
                        h_perp_hat /= max(float(np.linalg.norm(h_perp_hat)), 1e-10)
                    # Place H at sbl_ch from c_radical, perpendicular
                    # to co_hat with a slight tilt away (toward backbone).
                    # tetrahedral angle component: cos(109.5°) ≈ -0.33
                    tilt = -0.10  # slight tilt away from forming bond
...
def build_intra_substitution_s_isomerization_ts(
        r_xyz: dict,
        r_mol: 'Molecule',
        breaking_bonds: List[Tuple[int, int]],
        forming_bonds: List[Tuple[int, int]],
    ...
    # Identify the rigid fragment bonded to the migrating atom,
    # excluding both the breaking and forming partners.
    ...
    # Breaking bond stretches significantly in the TS.
    d_break_target = sbl_break + PAULING_DELTA  # ~2.47 for S-S
    # Forming bond contracts toward a Pauling TS distance.
    d_form_target = sbl_form + PAULING_DELTA  # ~2.23 for C-S
    ...
    # Compute the displacement for the migrating atom.
    # Move it AWAY from the breaking partner and TOWARD the forming
    # partner, each proportionally to the gap.
    displacement = np.zeros(3)

    # Breaking: push migrating away from breaking_partner.
    ...
    # Forming: pull migrating toward forming_partner.
    ...
    # Apply displacement to the entire rigid fragment.
    for k in frag:
...
def build_retroene_ts(
    ...
    In a Retroene reaction, an ester fragments through a concerted
    6-membered ring TS::
    ...
    # Identify the migrating H: present in one bb AND in fb.
    mig_h = None
    donor = None
    other_bb_idx = None
    ...
    # Identify leaving_c (bonded to donor) and ester_o.
    leaving_c = None
    ester_o = None
    ...
    # The carbonyl C: non-H, non-leaving-C neighbor of ester_o.
    carbonyl_c = None
    ...
    # The carbonyl O: double-bonded O on carbonyl_c (not ester_o).
    carbonyl_o = None
    ...
    # =====================================================================
    # Direct construction of the 6-membered ring TS.
    #
    # The acid fragment (O3, C1, O2, C0 + H's) and the donor (C5) stay
    # at their reactant positions.  C4 is repositioned via two-sphere
    # triangulation to simultaneously satisfy:
    #   - d(O3, C4) ≈ 2.5 Å  (breaking sigma bond, stretched)
    #   - d(C5, C4) ≈ 1.40 Å (forming pi bond, contracted)
    # Then the ring is closed by ring_closure_xyz to bring O2 toward C4.
    # Finally, H13 is placed between C5 and O3 (which are still near
    # each other since only C4 moved).
    # =====================================================================

    # Step 1: Place C4 at two-sphere intersection of O3 and C5 so that
    # d(O3, C4) stretches and d(C5, C4) contracts simultaneously.
    d_break = 2.5   # O3-C4 target (breaking)
    d_pi = 1.40     # C5-C4 target (forming pi bond)
    ...
    # Perpendicular direction: use C4's current offset from O3→C5 axis.
    ...
    # Move C4 and its direct H substituents (H11, H12) — NOT C5.
    c4_moving = {leaving_c}
    ...
    # Step 2: ring_closure_xyz to fold O2 toward C4's new position.
    ...
    # C5 and O3 are still near each other (only C4 moved in Step 1,
    # and ring_closure may have adjusted angles but O3/C5 stay close).
    sbl_ch = get_single_bond_length('C', 'H') or 1.09
    sbl_oh = get_single_bond_length('O', 'H') or 0.97
    d_ch = sbl_ch + PAULING_DELTA  # ~1.51
    d_oh = sbl_oh + PAULING_DELTA  # ~1.39
    d_pos = coords[donor]
