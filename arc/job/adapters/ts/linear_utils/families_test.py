...
class TestBuild13SigmatropicRearrangementTS(unittest.TestCase):
    ...
    def test_success_imidazole_rearrangement(self):
        ...
        self.assertAlmostEqual(d_break, 2.24, delta=0.10, msg=f'd(C3-N4)={d_break:.3f} should be ~2.24')
        self.assertAlmostEqual(d_form, 1.85, delta=0.10, msg=f'd(N1-C3)={d_form:.3f} should be ~1.85')
        ...
        self.assertLess(d_bridge, 2.0, msg=f'd(C2-C3)={d_bridge:.3f} should be < 2.0 '
        ...
        for atom in r.mol.atoms:
            ...
            for nbr in atom.bonds.keys():
                ...
                self.assertGreater(d, 0.9, msg=f'bond {ia}-{ib} collapsed to {d:.3f}')
                self.assertLess(d, 3.5, msg=f'bond {ia}-{ib} stretched to {d:.3f}')
    ...
    def test_returns_none_when_bb_fb_ambiguous(self):
        ...
        ts = build_1_3_sigmatropic_rearrangement_ts(r.get_xyz(), r.mol, breaking_bonds=[(0, 2)], forming_bonds=[(1, 3)])
        self.assertIsNone(ts)
    ...
    def test_returns_none_when_multiple_bb(self):
        ...
        ts = build_1_3_sigmatropic_rearrangement_ts(r.get_xyz(), r.mol, breaking_bonds=[(0, 1), (1, 2)], forming_bonds=[(0, 2)])
        self.assertIsNone(ts)
...
class TestBuildBaeyerVilligerStep2TS(unittest.TestCase):
    ...
    def test_returns_none_without_oo_bond(self):
        """When split_bonds contains no O-O bond, the builder returns None (no peroxide motif identified)."""
        r = ARCSpecies(label='ethane', smiles='CC')
        ts = build_baeyer_villiger_step2_ts(
    ...
    def test_returns_none_without_carbonyl(self):
        """When the O-O bond is present but no adjacent C=O exists, the builder returns None."""
...
H      -0.36583394   -1.89034834    0.81324667""")
    ...
    def test_returns_none_for_non_carbene(self):
        ...
        ts = build_singlet_carbene_intra_disproportionation_ts(ethane.get_xyz(), ethane.mol)
        self.assertIsNone(ts)
...
H       2.46084746    4.10465096    0.46458235""")
    ...
    def test_returns_none_when_no_shared_atom(self):
        ...
        ts = build_intra_substitution_s_isomerization_ts(self.r_xyz, self.r.mol,
