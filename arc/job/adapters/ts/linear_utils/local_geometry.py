...
def is_internal_reactive_ch2_misoriented(xyz: dict,
    ...
    4. **Bisector crowding.** If the unit-bond bisector
       ``(ĥ_0 + ĥ_1)`` (normalized) has ``· ŵ > −0.45``, the H–C–H
       bisector leans into the heavy corridor instead of pointing to its
       back side (a healthy sp³ CH₂ has ``bisector · ŵ ≈ −1``). This
       catches asymmetric inversions where one H is only mildly crowded
       and the other straddles the heavy plane — a pattern that slips
       past Rules 2 and 3.
    ...
    # ---- Rule D: H–C–H bisector crowds the heavy corridor ----
    bis = (h0_vec / r0) + (h1_vec / r1)
    bis_norm = float(np.linalg.norm(bis))
    if bis_norm > 1e-6 and float(np.dot(bis / bis_norm, w_hat)) > -0.45:
        return True
