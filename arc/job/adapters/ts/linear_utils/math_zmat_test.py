...
class TestMathZmat(unittest.TestCase):
    ...
    def test_get_r_constraints_frequency_sorting(self):
        """Atom with the highest frequency in bond changes appears first."""
        result = get_r_constraints([(0, 1), (1, 2)], [(1, 3)])
        constraints = result['R_atom']
        self.assertEqual(constraints[0][0], 1)

    def test_get_r_constraints_deterministic(self):
        """Reordering bonds (within or across the breaking/forming lists) produces
        identical output when each reactive atom participates in at most one bond,
        i.e. when the choice is unambiguous."""
        r1 = get_r_constraints([(1, 2), (5, 6)], [(3, 4)])
        r2 = get_r_constraints([(5, 6), (1, 2)], [(3, 4)])
        r3 = get_r_constraints([(3, 4)], [(1, 2), (5, 6)])
    ...
    def test_average_zmat_params_reactive_filtering(self):
        ...
        zmat_1 = {'vars': {'R_1_0': 1.0, 'R_2_1': 1.5},
                  'coords': [('R_1_0', '', ''), ('R_2_1', '', '')],
                  'symbols': ('C', 'C', 'H'),
                  'isotopes': (12, 12, 1),
                  'map': {0: 0, 1: 1, 2: 2}}
        zmat_2 = {'vars': {'R_1_0': 2.0, 'R_2_1': 3.0},
                  'coords': [('R_1_0', '', ''), ('R_2_1', '', '')],
                  'symbols': ('C', 'C', 'H'),
                  'isotopes': (12, 12, 1),
                  'map': {0: 0, 1: 1, 2: 2}}
        result = average_zmat_params(zmat_1, zmat_2, weight=0.5, reactive_xyz_indices={0, 1})
        self.assertIsNotNone(result)
        self.assertAlmostEqual(result['vars']['R_1_0'], 1.5)
        # R_2_1 references atom 1 (reactive) → INTERPOLATED.
        self.assertAlmostEqual(result['vars']['R_2_1'], 2.25)

    def test_average_zmat_params_symbol_order_mismatch(self):
        """Different symbol orderings make check_ordered_zmats return False."""
        zmat_1 = {'vars': {'R_1_0': 1.0}, 'coords': [('R_1_0', '', '')],
                  'symbols': ('C', 'H'), 'isotopes': (12, 1), 'map': {0: 0, 1: 1}}
        zmat_2 = {'vars': {'R_1_0': 2.0}, 'coords': [('R_1_0', '', '')],
                  'symbols': ('H', 'C'), 'isotopes': (1, 12), 'map': {0: 0, 1: 1}}
        self.assertIsNone(average_zmat_params(zmat_1, zmat_2, weight=0.5))

    def test_average_zmat_params_zmat2_missing_key(self):
        """Returns None when zmat_2 is missing a variable present in zmat_1."""
        zmat_1 = {'vars': {'R_1_0': 1.0, 'A_2_1_0': 105.0},
                  'coords': [('R_1_0', '', ''), ('', 'A_2_1_0', '')],
                  'symbols': ('C', 'H', 'H'), 'isotopes': (12, 1, 1),
                  'map': {0: 0, 1: 1, 2: 2}}
        zmat_2 = {'vars': {'R_1_0': 2.0, 'A_2_1_0': 110.0},
                  'coords': [('R_1_0', '', ''), ('', 'A_2_1_0', '')],
                  'symbols': ('C', 'H', 'H'), 'isotopes': (12, 1, 1),
                  'map': {0: 0, 1: 1, 2: 2}}
        # Matching key-sets pass check_ordered_zmats; now remove one key
        # from zmat_2 but keep the key-set equality check satisfied by
        # also removing it from zmat_1's schema check: instead, mismatch
        # at the loop level by adding an extra key to zmat_1 only.
        zmat_1['vars']['D_3_2_1_0'] = 60.0
        zmat_1['coords'].append(('', '', 'D_3_2_1_0'))
        zmat_1['symbols'] = ('C', 'H', 'H', 'H')
        zmat_1['isotopes'] = (12, 1, 1, 1)
        zmat_1['map'] = {0: 0, 1: 1, 2: 2, 3: 3}
        zmat_2['symbols'] = ('C', 'H', 'H', 'H')
        zmat_2['isotopes'] = (12, 1, 1, 1)
        zmat_2['map'] = {0: 0, 1: 1, 2: 2, 3: 3}
        # Now zmat_1 has D_3_2_1_0 but zmat_2 does not — check_ordered_zmats
        # will fail since keysets differ, and we expect None.
        self.assertIsNone(average_zmat_params(zmat_1, zmat_2, weight=0.5))

    def test_average_zmat_params_dihedral_wraparound_weight_non_half(self):
        """Wraparound at +-180 with weight != 0.5 yields a dihedral near +-180."""
        zmat_1 = {'vars': {'D_3_2_1_0': -179.0},
                  'coords': [('', '', ''), ('', '', ''), ('', '', ''), ('', '', 'D_3_2_1_0')],
                  'symbols': ('C', 'C', 'C', 'H'), 'isotopes': (12, 12, 12, 1),
                  'map': {0: 0, 1: 1, 2: 2, 3: 3}}
        zmat_2 = {'vars': {'D_3_2_1_0': 179.0},
                  'coords': [('', '', ''), ('', '', ''), ('', '', ''), ('', '', 'D_3_2_1_0')],
                  'symbols': ('C', 'C', 'C', 'H'), 'isotopes': (12, 12, 12, 1),
                  'map': {0: 0, 1: 1, 2: 2, 3: 3}}
        result = average_zmat_params(zmat_1, zmat_2, weight=0.7)
        self.assertIsNotNone(result)
        self.assertGreater(abs(result['vars']['D_3_2_1_0']), 178.0)

    def test_average_zmat_params_weight_below_zero(self):
        """Weight below 0 returns None."""
        zmat = {'vars': {'R_1_0': 1.5}, 'coords': [('R_1_0', '', '')],
                'symbols': ('C', 'H'), 'isotopes': (12, 1), 'map': {0: 0, 1: 1}}
        self.assertIsNone(average_zmat_params(zmat, zmat, weight=-0.1))

    def test_average_zmat_params_weight_above_one(self):
        """Weight above 1 returns None."""
        zmat = {'vars': {'R_1_0': 1.5}, 'coords': [('R_1_0', '', '')],
                'symbols': ('C', 'H'), 'isotopes': (12, 1), 'map': {0: 0, 1: 1}}
        self.assertIsNone(average_zmat_params(zmat, zmat, weight=1.1))
    ...
    def test_get_weight_grid_no_hammond(self):
        ...
        spc_p.e0 = None
        spc_p.e_elect = None
        rxn = ARCReaction(reactants=['R'], products=['P'], r_species=[spc_r], p_species=[spc_p])
    ...
    def test_get_weight_grid_with_hammond(self):
        ...
        self.assertGreaterEqual(len(grid), len(BASE_WEIGHT_GRID))
