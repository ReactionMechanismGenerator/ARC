...
class TestGraphHelpers(unittest.TestCase):
    ...
    def test_mol_to_adjacency_propane(self):
        ...
        for ci in c_indices:
            self.assertGreaterEqual(len(adj[ci]), 1)
...
class TestHasRecipeChannelMismatch(unittest.TestCase):
    ...
    # --- Element-aware threshold tests (breaking floor = 0.85 × sbl, forming
    #     ceiling = 2.0 × sbl) ---

    def test_breaking_ch_below_floor_returns_true(self):
        """Breaking C-H at d < 0.85 × sbl(C,H) ≈ 0.93 Å is flagged."""
        sbl = get_single_bond_length('C', 'H')
        spec = ReactionPathSpec(breaking_bonds=[(0, 1)])
        xyz = self._two_atom_xyz('C', 'H', 0.84 * sbl)
        mismatch, reason = has_recipe_channel_mismatch(spec, xyz)
        self.assertTrue(mismatch)
        self.assertIn('failed-to-break', reason)

    def test_breaking_ch_above_floor_passes(self):
        """Breaking C-H at d > 0.85 × sbl(C,H) is not flagged (lenient X-H floor)."""
        sbl = get_single_bond_length('C', 'H')
        spec = ReactionPathSpec(breaking_bonds=[(0, 1)])
        xyz = self._two_atom_xyz('C', 'H', 0.86 * sbl)
        mismatch, _ = has_recipe_channel_mismatch(spec, xyz)
        self.assertFalse(mismatch)

    def test_breaking_floor_scales_with_bond_type(self):
        """The same absolute distance can pass for one bond type and fail for another."""
        # 1.10 Å passes for C-C breaking (1.10 > 0.85 × 1.54 = 1.31? actually 1.10 < 1.31)
        # but for clarity: pick a value that's below C-C floor and above C-H floor.
        sbl_cc = get_single_bond_length('C', 'C')
        sbl_ch = get_single_bond_length('C', 'H')
        # Distance 1.20 Å: below 0.85 × sbl_cc (= 1.309) → fails for C-C; well above 0.85 × sbl_ch (= 0.93) → passes for C-H.
        d = 1.20
        cc_mismatch, _ = has_recipe_channel_mismatch(
            ReactionPathSpec(breaking_bonds=[(0, 1)]),
            self._two_atom_xyz('C', 'C', d))
        ch_mismatch, _ = has_recipe_channel_mismatch(
            ReactionPathSpec(breaking_bonds=[(0, 1)]),
            self._two_atom_xyz('C', 'H', d))
        self.assertTrue(cc_mismatch, msg=f'C-C at {d} should fail (floor 0.85*{sbl_cc:.3f})')
        self.assertFalse(ch_mismatch, msg=f'C-H at {d} should pass (floor 0.85*{sbl_ch:.3f})')

    def test_forming_ch_above_ceiling_returns_true(self):
        """Forming C-H at d > 2.0 × sbl(C,H) ≈ 2.18 Å is flagged."""
        sbl = get_single_bond_length('C', 'H')
        spec = ReactionPathSpec(forming_bonds=[(0, 1)])
        xyz = self._two_atom_xyz('C', 'H', 2.01 * sbl)
        mismatch, reason = has_recipe_channel_mismatch(spec, xyz)
        self.assertTrue(mismatch)
        self.assertIn('failed-to-form', reason)

    def test_forming_ch_below_ceiling_passes(self):
        """Forming C-H at d < 2.0 × sbl(C,H) is not flagged."""
        sbl = get_single_bond_length('C', 'H')
        spec = ReactionPathSpec(forming_bonds=[(0, 1)])
        xyz = self._two_atom_xyz('C', 'H', 1.99 * sbl)
        mismatch, _ = has_recipe_channel_mismatch(spec, xyz)
        self.assertFalse(mismatch)

    def test_forming_ceiling_scales_with_bond_type(self):
        """Long-bond (S-S) forming pairs get a more generous ceiling than C-H."""
        sbl_ss = get_single_bond_length('S', 'S')
        sbl_ch = get_single_bond_length('C', 'H')
        # 2.50 Å is above 2.0 × sbl_ch (= 2.18) → fails for C-H,
        # but well below 2.0 × sbl_ss (= 4.10) → passes for S-S.
        d = 2.50
        ch_mismatch, _ = has_recipe_channel_mismatch(
            ReactionPathSpec(forming_bonds=[(0, 1)]),
            self._two_atom_xyz('C', 'H', d))
        ss_mismatch, _ = has_recipe_channel_mismatch(
            ReactionPathSpec(forming_bonds=[(0, 1)]),
            self._two_atom_xyz('S', 'S', d))
        self.assertTrue(ch_mismatch, msg=f'C-H at {d} should fail (ceiling 2*{sbl_ch:.3f})')
        self.assertFalse(ss_mismatch, msg=f'S-S at {d} should pass (ceiling 2*{sbl_ss:.3f})')
...
class TestHasCommittedSpectatorGroup(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        cls.butane_mol = ARCSpecies(label='butane', smiles='CCCC').mol
    ...
    def test_no_op_for_unrelated_family(self):
        ...
        bad, _ = has_committed_spectator_group(spec, xyz, self.butane_mol, symbols=('C', 'C', 'C', 'C'))
        self.assertFalse(bad)
    ...
    def test_rejects_for_allowlisted_family_when_committed(self):
        ...
        xyz = self._xyz_two_carbons(d_ca=2.0, d_spectator=1.0)
        bad, reason = has_committed_spectator_group(spec, xyz, self.butane_mol, symbols=('C', 'C', 'C', 'C'))
        self.assertTrue(bad)
