...
def _repair_crowded_ch2(xyz: dict,
    ...
    for atom in mol.atoms:
        ...
        if len(h_indices) != 2 or len(non_h_nbrs) != 2:
            continue
        # Check if the two H atoms are crowded (H···H < 1.3 Å) or
        # splayed (H-C-H angle > 125°).
        ...
        # Redistribute: place H atoms in a symmetric tetrahedral
        # arrangement around the bisector of the two non-H neighbor
        # directions.
        heavy_pos = coords[heavy_idx]
        ...
        # The H atoms go on the OPPOSITE side from the bisector (back
        # of the tetrahedron), spread by the plane normal.
        ...
        # Compute the correct tilt angle psi from the actual
        # heavy-neighbor angle, matching fix_crowded_h_atoms.
...
def _orient_donor_h_away_from_acceptor(
        xyz: dict,
        mol: 'Molecule',
        forming_bonds: List[Tuple[int, int]],
    ...
    for ia, ja in forming_bonds:
        # Identify the migrating H and the acceptor.
        ...
        # Find the donor: the non-H heavy atom bonded to the migrating H.
        donor = None
        ...
        # Find the backbone neighbor of the donor (non-H, non-migrating-H).
        backbone = None
        ...
        # Only handle terminal donors (1 non-H neighbor).
        n_heavy_nbr = sum(1 for nbr in mol.atoms[donor].bonds.keys()
                          if symbols[atom_to_idx[nbr]] != 'H')
        if n_heavy_nbr != 1:
            continue

        # Collect the non-migrating H's on the donor.
        donor_hs = []
        for nbr in mol.atoms[donor].bonds.keys():
            ni = atom_to_idx[nbr]
        ...
        # Skip if the donor is also involved in a heavy-heavy forming
        # bond (ring-forming reaction, not a pure H-migration).
        ...
        # Axis: donor → backbone (the H's go on the opposite side).
        ...
        # Build perpendicular basis vectors for the cone.
        # perp1: in the plane containing the axis and the acceptor,
        # pointing AWAY from the acceptor.
        ...
        if da_perp_norm < 1e-6:
            # Acceptor is along the axis; use arbitrary perpendicular.
        ...
        # Place H's at tetrahedral angle from the axis, with the
        # centroid of the H fan pointing along perp1 (away from
        # acceptor).  The dihedral spacing is 120° (= 2π/3) to
        # produce the correct ~109° H-C-H angle in a tetrahedral
        # arrangement (NOT 2π/n_h, which gives 180° for 2 H's
        # and a splayed 141° angle).
        sbl = get_single_bond_length('H', symbols[donor]) or 1.09
        tet_angle = np.radians(109.47)
        n_h = len(donor_hs)
        # Offset the dihedral fan so that both H's are symmetrically
        # away from the acceptor.  For 2 H's at 120° spacing, an
        # offset of -60° places them at ±60° from the anti-acceptor
        # direction, so both point away from the acceptor.
...
def postprocess_h_migration(xyz: dict,
    ...
    # Targeted fixes for terminal-donor H-migration (CH₃ → CH₂ + H).
    # Only fire when the migrating H comes from a terminal center
    # (1 non-H neighbor) to avoid disturbing ring-forming reactions
    # where the trivial fallback assigns family=intra_H_migration.
    _atom_to_idx_hm = {a: i for i, a in enumerate(r_mol.atoms)}
    _has_terminal_donor = False
    for h_mig in migrating_hs:
        # Find the donor: the non-H atom bonded to the migrating H.
        donor = None
