"""
Validation filters, geometry fixers, and family-specific post-processing
handlers for TS-guess geometries.
...
**Geometry fixers** (return new XYZ ``dict``):
    Functions like ``adjust_reactive_bond_distances``,
    ``orient_h_on_reactive_centers``, ``fix_forming_bond_distances``,
    ``fix_crowded_h_atoms``, ``fix_h_nonbonded_clashes``,
    ``stagger_donor_terminal_h``.  These take an XYZ dict and return a
    corrected copy without mutating the input.

**Family-specific post-processing handlers** (return ``(dict, set[int])``):
    Entry point: ``postprocess_ts_guess`` dispatches to
...
FamilyPostprocessor = Callable[
    [dict, 'Molecule', list[tuple[int, int]], list[tuple[int, int]], dict[str, int] | None],
    tuple[dict, set[int]],
]
...
# Populated below, after the handler functions are defined.
...
def adjust_reactive_bond_distances(xyz: dict,
    ...
    """
    Adjust reactive-bond distances toward TS-like values, but only when the
    upstream interpolation appears to have failed to stretch them at all.

    For each **breaking** heavy-atom bond whose current distance is shorter
    than the equilibrium single-bond length (``d_cur < sbl``), stretch both
    endpoints symmetrically toward the TS target ``sbl + PAULING_DELTA``.
    Bonds that interpolation already pushed past ``sbl`` are left alone —
    they are assumed to be on a sane TS trajectory and we don't want to
    over-stretch a well-interpolated guess.

    For each **forming** heavy-atom bond whose current distance is shorter
    than ``0.9 × sbl`` (clearly over-compressed), push the endpoints apart
    toward the same target. Forming bonds in the band ``[0.9·sbl, sbl)`` are
    likewise considered already-acceptable and left untouched.
...
def orient_h_on_reactive_centers(xyz: dict,
    ...
    for heavy_idx, partners in reactive_partners.items():
        ...
        # Restrict to direct graph neighbors; distant partners (e.g. the
        # acceptor in H-migration) must not sway non-migrating H orientation.
        ...
        # Terminal (<=1 heavy nbr): flip on aggregate dot > 0.5.
        # Internal (>=2 heavy nbrs): require ALL H's to point inward —
        # a single misplaced H in a tetrahedral group must not trigger a flip.
        n_heavy_nbr = sum(1 for nbr in mol.atoms[heavy_idx].bonds if symbols[atom_to_idx[nbr]] != 'H')
...
def has_broken_nonreactive_bond(xyz: dict,
    ...
    *max_stretch_ratio* × its single-bond length.
    ...
    atom_to_idx = {atom: idx for idx, atom in enumerate(mol.atoms)}
    # Exempt atoms within 2 hops of reactive center: Z-mat interpolation
    # naturally distorts nearby bonds.
    reactive_atoms = set()
...
def fix_crowded_h_atoms(xyz: dict,
                        mol: Molecule,
                        skip_h_indices: set[int] | None = None,
    ...
    Args:
        ...
        redistribute_ch2: When ``True``, also redistribute CH₂ groups
            ...
            families where hybridization changes (e.g. sp → sp2) make
...
def stagger_donor_terminal_h(xyz: dict,
    ...
    for (i_atom, j_atom) in bonds:
        ...
        dih_mig = dihedral_deg(coords[h_atom], coords[donor_idx],
        ...
        for nbr in mol.atoms[donor_idx].bonds.keys():
            ...
            if other_idx == backbone_idx or other_idx == h_atom or symbols[other_idx] != 'H':
                continue
            dih_h = dihedral_deg(coords[other_idx], coords[donor_idx],
                                  coords[backbone_idx], coords[ref_idx])
...
def _repair_crowded_ch2(xyz: dict,
                        mol: Molecule,
                        skip_h_indices: set[int],
                        ) -> dict:
    """
    Repair CH₂ groups whose H-C-H geometry collapsed or splayed.

    Fires for CH₂ groups (heavy atom with exactly 2 non-H neighbors)
    where EITHER the H···H distance is below 1.3 Å (collapsed) OR the
    H-C-H angle exceeds 125° (splayed).  Redistributes the two H atoms
    at proper tetrahedral angles around the bisector of the two
    heavy-neighbor directions.

    This is a targeted fix for families like ``intra_H_migration`` where
    dihedral changes in the backbone (e.g., COO rotation) cause Z-mat
    interpolation to distort CH₂ H-C-H angles.
    """
    atom_to_idx = {atom: idx for idx, atom in enumerate(mol.atoms)}
    symbols = xyz['symbols']
    coords = np.array(xyz['coords'], dtype=float)
    ...
    for atom in mol.atoms:
        heavy_idx = atom_to_idx[atom]
        if symbols[heavy_idx] == 'H':
            continue
        h_indices, non_h_nbrs = [], []
        for nbr in atom.bonds.keys():
            ni = atom_to_idx[nbr]
            if symbols[ni] == 'H' and ni not in skip_h_indices:
                h_indices.append(ni)
            elif symbols[ni] != 'H':
                non_h_nbrs.append(ni)
        if len(h_indices) != 2 or len(non_h_nbrs) != 2:
            continue
        d_hh = float(np.linalg.norm(coords[h_indices[0]] - coords[h_indices[1]]))
        crowded = d_hh < 1.3
        splayed = False
        if not crowded:
            v1 = coords[h_indices[0]] - coords[heavy_idx]
            v2 = coords[h_indices[1]] - coords[heavy_idx]
            n1 = float(np.linalg.norm(v1))
            n2 = float(np.linalg.norm(v2))
            if n1 > 1e-6 and n2 > 1e-6:
                cos_a = float(np.dot(v1, v2) / (n1 * n2))
                hch_angle = float(np.degrees(np.arccos(np.clip(cos_a, -1, 1))))
                splayed = hch_angle > 125.0
        if not crowded and not splayed:
            continue

        heavy_pos = coords[heavy_idx]
        n1_pos = coords[non_h_nbrs[0]]
        n2_pos = coords[non_h_nbrs[1]]
        v1 = n1_pos - heavy_pos
        v2 = n2_pos - heavy_pos
        ...
        if bis_norm < 1e-6:
            bisector = np.cross(v1_hat, np.array([1.0, 0.0, 0.0]))
            bis_norm = float(np.linalg.norm(bisector))
            if bis_norm < 1e-6:
                bisector = np.cross(v1_hat, np.array([0.0, 1.0, 0.0]))
                bis_norm = float(np.linalg.norm(bisector))
        bis_hat = bisector / bis_norm
        # H atoms sit opposite the bisector (back of tetrahedron), split by plane normal.
        plane_normal = np.cross(v1_hat, v2_hat)
        pn_norm = float(np.linalg.norm(plane_normal))
        if pn_norm < 1e-6:
            continue
        pn_hat = plane_normal / pn_norm
        anti_bisector = -bis_hat
        sbl_ch = get_single_bond_length('H', symbols[heavy_idx]) or 1.09
        # Tilt angle psi derived from the actual heavy-neighbor angle (see fix_crowded_h_atoms).
        cos_bb = float(np.clip(np.dot(v1_hat, v2_hat), -1.0, 1.0))
        beta = np.arccos(cos_bb) / 2.0
        cos_beta = np.cos(beta)
        if cos_beta > 1e-6:
            cos_psi = -np.cos(np.radians(109.47)) / cos_beta
            ...
            psi = np.pi / 2.0
        h_dir_plus = np.cos(psi) * anti_bisector + np.sin(psi) * pn_hat
        h_dir_minus = np.cos(psi) * anti_bisector - np.sin(psi) * pn_hat
        coords[h_indices[0]] = heavy_pos + sbl_ch * h_dir_plus
        coords[h_indices[1]] = heavy_pos + sbl_ch * h_dir_minus
        changed = True
...
def _orient_donor_h_away_from_acceptor(xyz: dict,
                                       mol: Molecule,
                                       forming_bonds: list[tuple[int, int]],
                                       migrating_hs: set[int],
                                       ) -> dict:
    """
    Reconstruct non-migrating H's on the donor so they point away from the acceptor.

    In H-migration TSs, the non-migrating H atoms on the donor center
    should point away from the acceptor (the heavy atom receiving the H).
    Z-mat interpolation can produce H's with the wrong angle (splayed to
    ~141° instead of ~109°) AND the wrong dihedral (facing toward the
    acceptor instead of away).

    This function reconstructs the donor H positions from scratch:
    1. Places them at the correct tetrahedral angle (109.47°) from the
       donor→backbone axis.
    2. Sets the dihedral so the H centroid points opposite to the
       acceptor in the perpendicular plane.

    Only modifies atoms on the donor heavy atom and only when there is
    a single backbone neighbor (terminal CH₃/CH₂ donor).
    """
    atom_to_idx = {atom: idx for idx, atom in enumerate(mol.atoms)}
    symbols = xyz['symbols']
    coords = np.array(xyz['coords'], dtype=float)
    ...
    for ia, ja in forming_bonds:
        if ia in migrating_hs:
            h_mig, acceptor = ia, ja
        elif ja in migrating_hs:
            h_mig, acceptor = ja, ia
        else:
            continue

        donor = None
        for nbr in mol.atoms[h_mig].bonds.keys():
            ni = atom_to_idx[nbr]
            if symbols[ni] != 'H':
                donor = ni
                break
        ...
        backbone = None
        for nbr in mol.atoms[donor].bonds.keys():
            ni = atom_to_idx[nbr]
            if ni != h_mig and symbols[ni] != 'H':
                backbone = ni
                break
        if backbone is None:
            continue

        # Only terminal donors (1 non-H neighbor).
        n_heavy_nbr = sum(1 for nbr in mol.atoms[donor].bonds.keys()
                          if symbols[atom_to_idx[nbr]] != 'H')
        if n_heavy_nbr != 1:
            continue
        ...
        for nbr in mol.atoms[donor].bonds.keys():
            ni = atom_to_idx[nbr]
            if symbols[ni] == 'H' and ni not in migrating_hs:
                donor_hs.append(ni)
        if not donor_hs:
            continue

        # Skip donors also in a heavy-heavy forming bond (ring-forming, not pure H-migration).
        donor_in_heavy_fb = False
        for fa, fb_j in forming_bonds:
            if fa in migrating_hs or fb_j in migrating_hs:
                continue
            if fa == donor or fb_j == donor:
                donor_in_heavy_fb = True
                break
        if donor_in_heavy_fb:
            continue

        da_check = coords[acceptor] - coords[donor]
        da_check_n = float(np.linalg.norm(da_check))
        needs_fix = False
        if da_check_n > 1e-6:
            for hi in donor_hs:
                dh = coords[hi] - coords[donor]
                cos_a = float(np.dot(dh, da_check) / (np.linalg.norm(dh) * da_check_n))
                if cos_a > 0:  # angle < 90°: pointing toward acceptor
                    needs_fix = True
                    break
        if len(donor_hs) >= 2 and not needs_fix:
            v1 = coords[donor_hs[0]] - coords[donor]
            v2 = coords[donor_hs[1]] - coords[donor]
            n1 = float(np.linalg.norm(v1))
            n2 = float(np.linalg.norm(v2))
            if n1 > 1e-6 and n2 > 1e-6:
                cos_hch = float(np.dot(v1, v2) / (n1 * n2))
                hch = float(np.degrees(np.arccos(np.clip(cos_hch, -1, 1))))
                if hch > 125.0 or hch < 90.0:
                    needs_fix = True
        if not needs_fix:
            continue

        # Axis: donor -> backbone; H's go on the opposite side.
        axis = coords[donor] - coords[backbone]
        axis_norm = float(np.linalg.norm(axis))
        if axis_norm < 1e-6:
            continue
        axis_hat = axis / axis_norm

        # perp1: in the axis-acceptor plane, pointing AWAY from the acceptor.
        da = coords[acceptor] - coords[donor]
        da_perp = da - np.dot(da, axis_hat) * axis_hat
        da_perp_norm = float(np.linalg.norm(da_perp))
        if da_perp_norm < 1e-6:
            # Acceptor collinear with axis; pick an arbitrary perpendicular.
            ref = np.array([1.0, 0.0, 0.0])
            if abs(float(np.dot(axis_hat, ref))) > 0.9:
                ref = np.array([0.0, 1.0, 0.0])
            perp1 = np.cross(axis_hat, ref)
            perp1 /= float(np.linalg.norm(perp1))
        else:
            perp1 = -da_perp / da_perp_norm  # AWAY from acceptor
        perp2 = np.cross(axis_hat, perp1)

        # Fixed 120° (= 2π/3) dihedral spacing, NOT 2π/n_h: two H's at 2π/n_h
        # would sit 180° apart giving a splayed ~141° H-C-H angle.
        sbl = get_single_bond_length('H', symbols[donor]) or 1.09
        tet_angle = np.radians(109.47)
        n_h = len(donor_hs)
        # n_h==2: offset -60° places the pair symmetrically at ±60° from the
        # anti-acceptor direction so both point away from the acceptor.
        offset = -np.pi / 3.0 if n_h == 2 else 0.0
        for k, hi in enumerate(donor_hs):
            dih = offset + 2.0 * np.pi * k / 3.0
            h_dir = (np.cos(tet_angle) * axis_hat
                     + np.sin(tet_angle) * (np.cos(dih) * perp1
                                            + np.sin(dih) * perp2))
            coords[hi] = coords[donor] + sbl * h_dir
        changed = True
...
def postprocess_h_migration(xyz: dict,
    ...
    # Gate the terminal-donor fixes (CH3 -> CH2 + H): fire only for a
    # terminal donor so ring-forming reactions misclassified as
    # intra_H_migration by the trivial fallback are not disturbed.
    _atom_to_idx_hm = {a: i for i, a in enumerate(r_mol.atoms)}
    _has_terminal_donor = False
    for h_mig in migrating_hs:
        donor = None
        for nbr in r_mol.atoms[h_mig].bonds.keys():
            ni = _atom_to_idx_hm[nbr]
            if xyz['symbols'][ni] != 'H':
                donor = ni
                break
        if donor is None:
            continue
        n_hvy = sum(1 for n2 in r_mol.atoms[donor].bonds.keys()
                    if xyz['symbols'][_atom_to_idx_hm[n2]] != 'H')
        if n_hvy == 1:
            _has_terminal_donor = True
            break
    if _has_terminal_donor:
        xyz = _repair_crowded_ch2(xyz, r_mol, migrating_hs)
        xyz = _orient_donor_h_away_from_acceptor(
            xyz, r_mol, forming_bonds, migrating_hs)
...
def _postprocess_cc_shift(xyz: dict,
    ...
    """
    Post-processing pipeline for C-C shift families with hybridization changes.
...
# ---------------------------------------------------------------------------
# Family-to-handler registry
# ---------------------------------------------------------------------------
#
# Populated here, after every handler and validator has been defined.
# Note the two asymmetric cases:
#   1,3_sigmatropic_rearrangement and intra_substitutionCS_isomerization use
#       the group-shift *postprocessor* but the H-migration *validator*.
#   6_membered_central_C-C_shift uses its own *postprocessor* but the
#       group-shift *validator*.

_H_MIGRATION_FAMILIES = (
    'intra_H_migration',
    'Ketoenol',
...
_GROUP_SHIFT_POSTPROC_FAMILIES = (
    '1,2_shiftC',
    '1,2_shiftS',
    '1,3_sigmatropic_rearrangement',
    'intra_substitutionCS_isomerization',
)
_GROUP_SHIFT_VALIDATOR_FAMILIES = (
    '1,2_shiftC',
    '1,2_shiftS',
    '6_membered_central_C-C_shift',
)
_H_MIGRATION_VALIDATOR_ONLY_FAMILIES = (
    '1,3_sigmatropic_rearrangement',
    'intra_substitutionCS_isomerization',
)

for _fam in _H_MIGRATION_FAMILIES:
    FAMILY_POSTPROCESSORS[_fam] = postprocess_h_migration
    FAMILY_VALIDATORS[_fam] = validate_h_migration

for _fam in _GROUP_SHIFT_POSTPROC_FAMILIES:
    FAMILY_POSTPROCESSORS[_fam] = _postprocess_group_shift

for _fam in _H_MIGRATION_VALIDATOR_ONLY_FAMILIES:
    FAMILY_VALIDATORS[_fam] = validate_h_migration

for _fam in _GROUP_SHIFT_VALIDATOR_FAMILIES:
...
def validate_ts_guess(xyz: dict,
    ...
    *regardless* of the family-string lookup in :data:`FAMILY_VALIDATORS`.
    ...
    if colliding_atoms(xyz):
        ...
        # H-transfer chemistry always uses the H-migration validator regardless
        # of family string; other chemistries fall back to family-string lookup.
