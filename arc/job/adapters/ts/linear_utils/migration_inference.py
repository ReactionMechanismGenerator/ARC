...
def _heavy_formula_of_fragment(symbols: Sequence[str], fragment: set[int]) -> tuple[tuple[str, int], ...]:
...
def infer_frag_fallback_h_migration(pre_xyz: dict,
    ...
    * **S5 — local donor/acceptor geometry consistency**: in the
    ...
    if len(heavy_nbrs) != 1:
        if logger is not None:
            logger.debug(f'Linear addition ({label}): frag-fallback inference skipped (S2: H{h_idx} '
                         f'has {len(heavy_nbrs)} heavy neighbors).')
        return None
    donor = heavy_nbrs[0]

    # ---- S3: donor and acceptor on different fragments after the cut ----
    fragments = split_mol_at_bonds(uni_mol, list(split_bonds or []))
    if len(fragments) < 2:
        if logger is not None:
            logger.debug(f'Linear addition ({label}): frag-fallback inference skipped (S3: only '
                         f'{len(fragments)} fragment(s) after cut).')
        return None

    donor_frag_idx: int | None = None
    ...
    if donor_frag_idx is None:
        if logger is not None:
            logger.debug(f'Linear addition ({label}): frag-fallback inference skipped (S3: donor {donor} not in any fragment).')
        return None
    ...
    if not acceptor_candidates:
        if logger is not None:
            logger.debug(f'Linear addition ({label}): frag-fallback inference skipped (S3: no heavy atom in any non-donor fragment).')
        return None

    # ---- S4: composition consistency (donor surplus, acceptor deficit) ----
    acceptor_frag_idx: int | None = None
    if multi_species:
        try:
            target_h: list[int] = []
        ...
        if target_heavy and len(target_heavy) == len(fragments):
            ...
            if ok_match:
                ...
                for fi, ti in frag_to_target.items():
                    h_count = sum(1 for idx in fragments[fi] if symbols[idx] == 'H')
                    surplus[fi] = h_count - target_h[ti]
                if surplus.get(donor_frag_idx, 0) <= 0:
                    if logger is not None:
                        logger.debug(
                            f'Linear addition ({label}): frag-fallback inference skipped (S4: donor fragment surplus '
                ...
                if len(deficit_frags) != 1:
                    if logger is not None:
                        logger.debug(
                            f'Linear addition ({label}): frag-fallback inference skipped '
                            f'(S4: {len(deficit_frags)} deficit fragments).')
                    return None
    ...
    if acceptor_frag_idx is not None:
        ...
        if not acceptor_candidates:
            if logger is not None:
                logger.debug(f'Linear addition ({label}): frag-fallback inference skipped '
                             f'(S4: deficit fragment has no heavy atoms).')
            return None
    ...
    if nearest_d > 1.60 * sbl_ah:
        if logger is not None:
            logger.debug(f'Linear addition ({label}): frag-fallback inference skipped '
                         f'(S5: d(A,H)={nearest_d:.3f} > {1.60 * sbl_ah:.3f}).')
        return None

    # ---- S5 rival check: uniqueness within acceptor's fragment ----
    if acceptor_frag_idx is not None:
        rival_atoms = [i for i in fragments[acceptor_frag_idx]
                       if symbols[i] != 'H' and i != acceptor]
    else:
        chosen_frag: set[int] | None = None
    ...
    for rival in rival_atoms:
        ...
        if d_rh < 0.95 * float(sbl_rh):
            if logger is not None:
                logger.debug(f'Linear addition ({label}): frag-fallback inference skipped (S5: rival '
                             f'{symbols[rival]}{rival} at d={d_rh:.3f} < {0.95 * float(sbl_rh):.3f}).')
            return None

    return {'h_idx': int(h_idx),
            'donor': int(donor),
            'acceptor': int(acceptor),
