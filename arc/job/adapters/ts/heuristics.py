...
``BREAKING_BOND_LENGTH`` and ``FORMING_BOND_LENGTH`` are the absolute A--H and H--B distances (in Angstrom) at which
an H_Abstraction TS seed A-H...B is built when the partner atom (A for the breaking bond, B for the forming bond) is
one of ``ABSOLUTE_BOND_LENGTH_PARTNERS``. Source: the medians of the breaking and forming bond lengths over a dataset
of 1670 DFT-optimized H-abstraction transition states, dominated by C, N and O partner atoms. For any other partner element the respective bond of the reactant or product
well is multiplied by ``DEFAULT_BOND_STRETCH`` instead. The two bonds are treated independently, so one side of a seed
may take the absolute distance while the other takes the multiplicative stretch.
"""
...
BREAKING_BOND_LENGTH = 1.29
FORMING_BOND_LENGTH = 1.36
ABSOLUTE_BOND_LENGTH_PARTNERS = ('C', 'N', 'O')
DEFAULT_BOND_STRETCH = 1.2
            ...
            for method_index, xyz_entry in enumerate(xyzs):
                ...
                if unique:
                    ...
                    save_geo(xyz=xyz,
                             ...
                             format_='xyz',
                             comment=f'{method_label} {method_index}, family: {family}',
                             )

            if len(self.reactions) < 5:
                successes = [tsg for tsg in rxn.ts_species.ts_guesses
                             if tsg.success and 'heuristics' in tsg.method.lower()]
                if successes:
...
def combine_coordinates_with_redundant_atoms(xyz_1: dict[str, Any],
    ...
    zmats will be constructed in the following way::
        ...
        zmat1 = {'symbols': ('C', 'H', 'H', 'H', 'H'),
                 'coords': ((None, None, None),  # 0, atom A
                            ...
                            ('R_4_3', 'A_4_3_2', 'D_4_3_2_1')),  # 4, H1, R_4_3 is the breaking bond
                 'vars': {...},
                 'map': {...}}

        zmat2 = {'symbols': ('H', 'H', 'H', 'H', 'C'),
                 'coords': ((None, None, None),  # H2, redundant H atom, will be united with H1
                            ('R_5_4', a2 (B-H-A) = 'A_5_4_0', d2 (B-H-A-C) = 'D_5_4_0_1'),  # 5, B, forming bond
    ...
    Args:
        ...
        r1_stretch (float | None, optional): The factor by which to multiply (stretch/shrink) the bond length to the
                                            terminal atom ``h1`` in ``xyz1`` (bond A-H1). If ``None``, the bond is set
                                            to the absolute ``BREAKING_BOND_LENGTH`` if atom A is one of
                                            ``ABSOLUTE_BOND_LENGTH_PARTNERS``, and is multiplied by
                                            ``DEFAULT_BOND_STRETCH`` otherwise.
        r2_stretch (float | None, optional): The factor by which to multiply (stretch/shrink) the bond length to the
                                            terminal atom ``h2`` in ``xyz2`` (bond B-H2). If ``None``, the bond is set
                                            to the absolute ``FORMING_BOND_LENGTH`` if atom B is one of
                                            ``ABSOLUTE_BOND_LENGTH_PARTNERS``, and is multiplied by
                                            ``DEFAULT_BOND_STRETCH`` otherwise.
    ...
    set_reactive_bond(zmat=zmat_1,
                      indices=(h1, a),
                      partner_symbol=xyz_1['symbols'][a],
                      length=BREAKING_BOND_LENGTH,
                      stretch=r1_stretch,
                      )
    set_reactive_bond(zmat=zmat_2,
                      indices=(b, h2),
                      partner_symbol=xyz_2['symbols'][b],
                      length=FORMING_BOND_LENGTH,
                      stretch=r2_stretch,
                      )
...
def set_reactive_bond(zmat: dict,
                      indices: tuple[int, int],
                      partner_symbol: str,
                      length: float,
    ...
    """
    Set a reactive bond of an H_Abstraction TS seed in a zmat.

    With an explicit ``stretch``, the bond length is multiplied by it. Otherwise the bond is set to the absolute
    ``length`` if ``partner_symbol`` is one of ``ABSOLUTE_BOND_LENGTH_PARTNERS``, and is multiplied by
    ``DEFAULT_BOND_STRETCH`` for any other element.

    Args:
        zmat (dict): The zmat to process.
        indices (tuple): A length 2 tuple with the 0-indices of the xyz (not zmat) atoms representing the bond to set.
        partner_symbol (str): The element symbol of the heavy-atom (or H) partner of the transferred hydrogen in
                              this bond.
        length (float): The absolute bond length to set, in Angstrom.
        stretch (float | None, optional): An explicit factor by which to multiply the bond length instead.
    """
    if stretch is not None:
        stretch_zmat_bond(zmat=zmat, indices=indices, stretch=stretch)
    elif partner_symbol in ABSOLUTE_BOND_LENGTH_PARTNERS:
        set_zmat_bond(zmat=zmat, indices=indices, length=length)
    else:
        stretch_zmat_bond(zmat=zmat, indices=indices, stretch=DEFAULT_BOND_STRETCH)
...
def h_abstraction(reaction: ARCReaction,
    ...
    Args:
        ...
        r1_stretch (float | None, optional): The factor by which to multiply (stretch/shrink) the bond length to the
                                            ...
                                            well. If ``None``, the bond is set to the absolute
                                            ``BREAKING_BOND_LENGTH`` if atom A is one of
                                            ``ABSOLUTE_BOND_LENGTH_PARTNERS``, and is multiplied by
                                            ``DEFAULT_BOND_STRETCH`` otherwise.
        r2_stretch (float | None, optional): The factor by which to multiply (stretch/shrink) the bond length to the
                                            terminal atom ``h2`` in ``xyz2`` (bond B-H2) relative to the respective
                                            well. If ``None``, the bond is set to the absolute
                                            ``FORMING_BOND_LENGTH`` if atom B is one of
                                            ``ABSOLUTE_BOND_LENGTH_PARTNERS``, and is multiplied by
                                            ``DEFAULT_BOND_STRETCH`` otherwise.
        a2 (float, optional): The angle (in degrees) in the combined structure between atoms B-H-A (angle B-H-A).
...
def stretch_ab_bond(
    ...
    if a_before_b_zmat != a_before_b_xyz:
