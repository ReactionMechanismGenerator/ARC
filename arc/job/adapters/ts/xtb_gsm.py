...
# hundred small files; anything far beyond that is a malformed or hostile
# archive rather than a run artifact, and gzip expands cheaply enough that an
# unbounded extract is a disk-exhaustion vector.
GSM_ARCHIVE_MAX_MEMBERS = 5000
GSM_ARCHIVE_MAX_UNCOMPRESSED_BYTES = 512 * 1024 * 1024
    ...
    def _extract_gsm_evidence_archive(self) -> bool:
        ...
        try:
            with tarfile.open(self.gsm_evidence_archive_path, mode='r:gz') as archive:
                ...
                if len(members) > GSM_ARCHIVE_MAX_MEMBERS:
                    raise ValueError(f'GSM evidence archive holds {len(members)} members, '
                                     f'more than the {GSM_ARCHIVE_MAX_MEMBERS} expected.')
                total_size = sum(member.size for member in members)
                if total_size > GSM_ARCHIVE_MAX_UNCOMPRESSED_BYTES:
                    raise ValueError(f'GSM evidence archive expands to {total_size} bytes, '
                                     f'more than the {GSM_ARCHIVE_MAX_UNCOMPRESSED_BYTES}-byte limit.')
                ...
                # ``filter='data'`` is the safe extraction filter: it strips
                # absolute paths, ``..`` components, links, and special files.
                # The allowlist above already rejects those, but passing it
                # explicitly keeps behavior identical across Python versions —
                # it is a DeprecationWarning to omit it on 3.12 and the default
                # changes on 3.14.
                archive.extractall(path=self.local_path, members=members, filter='data')
            return True
