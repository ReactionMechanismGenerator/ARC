"""
An adapter for executing CFour (Coupled-Cluster techniques for Computational Chemistry) jobs

http://slater.chemie.uni-mainz.de/cfour/index.php
...
${zmat}
*CFOUR(CALC=${method},BASIS=${basis},CHARGE=${charge},MULTIPLICITY=${multiplicity},MEMORY_SIZE=${memory}
${keywords})
${block}
...
class CFourAdapter(JobAdapter):
    ...
    def __init__(self,
        ...
        self.job_adapter = 'cfour'
        self.execution_type = execution_type or 'queue'
        self.command = 'xcfour'
        self.url = 'http://slater.chemie.uni-mainz.de/cfour/index.php'

        if species is None:
            raise ValueError('Cannot execute CFOUR without an ARCSpecies object.')
    ...
    def write_input_file(self) -> None:
        ...
        input_dict['zmat'] = zmat_to_str(zmat=zmat_from_xyz(xyz=self.xyz,
                                                            mol=self.species[0].mol,
                                                            consolidate=True,
                                                            is_ts=self.species[0].is_ts,
                                                            ),
                                         zmat_format='cfour',
                                         )
        input_dict['block'] = ''
    ...
    def set_input_file_memory(self) -> None:
        ...
        """
        # CFour's memory is per cpu core and in integer words (1 MW ~= 8 MB; 1 GB = 128 MW)
        self.input_file_memory = math.ceil(self.job_memory_gb * 128 * 1e6 / self.cpu_cores)
