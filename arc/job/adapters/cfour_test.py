...
class TestCFourQueueSubmissionSharesAConnection(unittest.TestCase):
    ...
    def _adapter(server):
        ...
        A real adapter is not built here because constructing one runs the whole adapter
        initialization for a test about which connection the submission uses. Naming the
        attributes explicitly also states what ``execute_queue()`` is allowed to touch, so a
        method that starts reading something else fails here rather than passing on a mock
        that answers to anything.
        """
