...
"""
How often the transport sends a keepalive, in seconds, as ``paramiko.Transport.set_keepalive``
takes it. Long lived pooled connections are otherwise dropped by an idle timeout on the server
or on a firewall between it and the client.
"""
...
def _default_factory(server: str) -> SSHClient:
    ...
    client.connect()
    set_keepalive(client, KEEPALIVE_INTERVAL_SECONDS)
    return client


def set_keepalive(client: SSHClient, interval: int = KEEPALIVE_INTERVAL_SECONDS) -> bool:
    """
    ...
    The interval is also recorded on the client, which re-applies it to every transport it
    opens afterwards: a keepalive belongs to a Transport, and ``check_connections`` replaces the
    transport under a client it reconnects in place, which would otherwise leave a pooled client
    without one for the rest of the run.
    ...
    if transport is None:
        server_name = getattr(client, 'server', client)
        logger.debug(f'ssh_pool: no transport to set a keepalive on for {server_name}')
        return False
    client._keepalive_interval = interval
...
class SSHConnectionPool:
    ...
    def __init__(self, factory: SSHClientFactory = _default_factory):
        ...
        The ``opens`` and ``borrows`` counters expose pool behaviour without a caller having to
        peek at internals or hook the factory.

        Args:
            factory (SSHClientFactory, optional): Builds a connected client for a server name.
        """
    ...
    @contextmanager
    def borrow(self, server: str) -> Iterator[SSHClient]:
        """
        Lease the pool's SSHClient for ``server``.

        Exiting the context does not close the client, which the pool retains ownership of. The
        borrowed client is transient by contract; do not stash it past the ``with`` block.

        Concurrent borrows of the same server are not safe today. ARC's scheduler is
        single-threaded, so this has not bitten; a parallel scheduler would need a per-server
        lock around the yield, or a stack of free clients rather than a single one.
        ...
        if client is None or not _is_alive(client):
            ...
            self.opens += 1
            logger.debug(f'ssh_pool: opened SSHClient for {server} (total opens={self.opens})')
        else:
            logger.debug(f'ssh_pool: reusing SSHClient for {server}')
        yield client
...
def _is_alive(client: SSHClient) -> bool:
    ...
    This does not roundtrip to the server, which the SSHClient method's own
    ``check_connections`` decorator does on the next call. It is just enough to skip the obvious
    case of a connection reset between jobs, so that a known-dead handle is not handed out.

    Args:
        client (SSHClient): The client to check.

    Returns: bool
        Whether the client has a transport that reports itself active.
    """
    underlying = getattr(client, '_ssh', None)
...
def _close_quietly(client: SSHClient, context: str) -> None:
    """
    Close a client, reporting rather than propagating a failure to do so.

    Pool teardown must not propagate a close error, since ARC's main path is already past the
    work that needed the connection, so a failure is reported at the debug level instead.

    Args:
        client (SSHClient): The client to close.
        context (str): What was being done, named in the report.
    """
    try:
        client.close()
    except Exception:
        logger.debug(f'ssh_pool: close errored {context}', exc_info=True)


_default_pool: SSHConnectionPool | None = None
"""
The process-global pool, instantiated lazily by :func:`get_default_pool` and discarded between
ARC runs, and between tests, by :func:`reset_default_pool`.
"""
...
def set_default_pool(pool: SSHConnectionPool | None) -> None:
    """
    Replace the process-global pool.

    Mainly for tests that inject a stub-factory pool without monkeypatching the module.

    Args:
        pool (SSHConnectionPool | None): The pool to install, or ``None`` to clear it.
    """
    global _default_pool
    _default_pool = pool
