...
class TestSetKeepalive(unittest.TestCase):
    ...
    def test_the_transport_is_asked_to_send_keepalives(self):
        ...
        self.assertEqual(client._ssh.get_transport().keepalive_interval, KEEPALIVE_INTERVAL_SECONDS)
    ...
    def test_the_interval_is_recorded_on_the_client(self):
        """
        A keepalive is a property of a paramiko Transport, so it is lost when a reconnect opens a
        new one. Recording it on the client is what lets SSHClient.connect() re-apply it, which
        is the case a pooled connection held for a whole run actually meets.
        """
        client = _SSHClientStub('server2')
        set_keepalive(client, 11)
        self.assertEqual(client._keepalive_interval, 11)
