...
def build_output_document(project_directory: str,
    ...
    Four helpers are replaced: the three that shell out to the RMG conda environment
    (point groups, per-species corrections, and the run-level correction tables), and
    the Arkane provenance resolver, which otherwise reads whatever ``RMG_PATH`` the
    machine running the tests happens to export and yields ``ARKANE_PROVENANCE``
    instead. Every other field in the returned document is built by the production
    code paths.
    """
