...
IMPLEMENTED_FAMILIES = ['H_Abstraction']  # Todo: Enum
...
class HeuristicsAdapter(TSAdapter):
    ...
    def __init__(self, user_guesses: list = None,
        ...
        Args:
            user_guesses (list): TS user guesses, not used in the HeuristicsAdapter class.
            arc_reaction (ARCReaction, optional): The ARC Reaction object.
            dihedral_increment (float, optional): The scan dihedral increment to use when generating guesses.
        """
        if arc_reaction is not None and not isinstance(arc_reaction, ARCReaction):
            raise TypeError(f'arc_reaction must be an ARCReaction instance, got\n'
                            f'{arc_reaction}\n'
                            f'which is a {type(arc_reaction)}.')
    ...
    def __repr__(self) -> str:
        ...
        """
        return f"HeuristicsAdapter(arc_reaction={self.arc_reaction}, dihedral_increment={self.dihedral_increment})"
    ...
    def generate_guesses(self) -> list:
        ...
        if self.arc_reaction is None:
            return results
        if self.arc_reaction.family is None:
            logger.error('Cannot generate a TS guess for a reaction using heuristics without an RMG family attribute.')
            return results
        if self.arc_reaction.family.label not in IMPLEMENTED_FAMILIES:
            logger.error(f'Heuristics-based TS guesses generation for reaction family '
                         f'{self.arc_reaction.family} is not implemented yet.')
            return results

        self.arc_reaction.arc_species_from_rmg_reaction()
        reactant_mol_combinations = list(
            itertools.product(*list(reactant.mol_list for reactant in self.arc_reaction.r_species)))
        product_mol_combinations = list(
            itertools.product(*list(product.mol_list for product in self.arc_reaction.p_species)))
        reaction_list = list()
        for reactants in list(reactant_mol_combinations):
            for products in list(product_mol_combinations):
                reaction = label_molecules(list(reactants), list(products), self.arc_reaction.family)
                if reaction is not None:
                    reaction_list.append(reaction)

        if self.arc_reaction.family.label == 'H_Abstraction':
            results = h_abstraction(self.arc_reaction, reaction_list, dihedral_increment=self.dihedral_increment)
        return results
...
def label_molecules(reactants: list,
    ...
    # reaction_copy = Reaction(reactants=[mol.copy(deep=True) for mol in reactants],
