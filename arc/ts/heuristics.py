...
def combine_coordinates_with_redundant_atoms(xyz1, xyz2, mol1, mol2, h1, h2, c=None, d=None,
    ...
    Raises:
        TSError: If crucial parameters were not specified or could not be determined.
    ...
    if len(mol1.atoms) == 1 or len(mol2.atoms) == 1:
        raise TSError(f'The molecule arguments to combine_coordinates_with_redundant_atoms must each have more than 1 '
    ...
    if not is_a2_linear and len(mol1.atoms) > 2 and d2 is None:
        raise TSError('The d2 parameter (the B-H-A-C dihedral) must be given if the a2 angle (B-H-A) is not close '
    ...
    if len(mol1.atoms) > 2 and c is None:
        raise TSError('The c parameter (the index of atom C in xyz1) must be given if mol1 has 3 or more atoms, '
                      'got None.')
    if len(mol2.atoms) > 2 and d is None:
        raise TSError('The d parameter (the index of atom D in xyz2) must be given if mol2 has 3 or more atoms, '
                      'got None.')
    if len(mol2.atoms) > 2 and d3 is None:
        raise TSError('The d3 parameter (dihedral D-B-H-C) must be given if mol2 has 3 or more atoms, got None.')
    ...
    if c is not None and c == a:
        raise TSError(f'The value for c ({c}) is invalid (it represents atom A, not atom C)')
    if c is not None and d == b:
        raise TSError(f'The value for d ({d}) is invalid (it represents atom B, not atom D)')
    ...
    for i, coords in enumerate(combined_zmat['coords']):
        if i > 2 and None in coords:
            raise TSError(f'Could not combine zmats, got a None parameter above the 3rd row:\n{combined_zmat}')
...
def h_abstraction(arc_reaction, rmg_reactions, r1_stretch=1.2, r2_stretch=1.2, a2=180, dihedral_increment=20):
    ...
    for rmg_reaction in rmg_reactions:
        ...
        for d2, d3 in d2_d3_product:
            ...
            if not colliding_atoms(xyz_guess):
...
- eventually this module needs to be applied for all H's connected to the same heavy atom,
  summing up the rates appropriately like we did for imipramine
