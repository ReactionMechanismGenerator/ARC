...
class TestMappingEngine(unittest.TestCase):
    ...
    def test_make_bond_changes(self):
        ...
        print(f'\n\nfamily: {rxn.family}')
        r_bdes, _ = find_all_breaking_bonds(rxn, True), find_all_breaking_bonds(rxn, False)
    ...
    def test_glue_maps(self):
        ...
        atoms_r = [atom for atom in self.r_1.mol.copy(deep=True).atoms] + [atom for atom in
                                                                           self.r_2.mol.copy(deep=True).atoms]
        atoms_p = [atom for atom in self.p_1.mol.copy(deep=True).atoms] + [atom for atom in
                                                                           self.p_2.mol.copy(deep=True).atoms]
    ...
    def test_cut_species_based_on_atom_indices(self):
        ...
        label_species_atoms(reactants), label_species_atoms(products)
        # product_dicts = get_reaction_family_products(self.arc_reaction_2)
        # product_dict = product_dicts[0]
        r_bdes = find_all_breaking_bonds(rxn=self.arc_reaction_2, r_direction=True)
        print(f'product atoms of rxn2: 0: {self.arc_reaction_2.p_species[0].mol.atoms}, 1: {self.arc_reaction_2.p_species[1].mol.atoms}')
        p_bdes = find_all_breaking_bonds(rxn=self.arc_reaction_2, r_direction=False)
        print(f'r_bdes: {r_bdes}, p_bdes: {p_bdes}')
        print(f'p atoms: 0: {products[0].mol.atoms}, 1: {products[1].mol.atoms}')
        r_cuts = cut_species_based_on_atom_indices(reactants, r_bdes)
        p_cuts = cut_species_based_on_atom_indices(products, p_bdes)
        print(f'r_cuts: {[r.copy().mol.to_smiles() for r in r_cuts]}, p_cuts: {[r.copy().mol.to_smiles() for r in p_cuts]}')
        ...
        cuts = cut_species_based_on_atom_indices(spcs, [(1, 2), (3, 4), (4, 10)])
        self.assertEqual(len(cuts), 4)

        spcs = [ARCSpecies(label="ring", smiles = 'CC1CCOCC1[N+](=O)([O-])',
        ...
        label_species_atoms(spcs)
        cuts = cut_species_based_on_atom_indices(spcs, [(1, 2), (4, 5)])
        self.assertEqual(len(cuts), 2)
    ...
    def test_find_all_breaking_bonds(self):
        ...
        print(f'p atoms: 0: {self.arc_reaction_2.p_species[0].mol.atoms}, 1: {self.arc_reaction_2.p_species[1].mol.atoms}')  # todo: fix product BDE
