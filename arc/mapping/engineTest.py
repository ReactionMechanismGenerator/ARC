...
class TestConverter(unittest.TestCase):
    ...
    def test_pairing_reactants_and_products_for_mapping(self):
        ...
        for pair in pairs_of_reactant_and_products:
            ...
            self.assertTrue(str(pair[0].mol.copy(deep=True).smiles) in smiles)
            smiles.remove(pair[0].mol.copy(deep=True).smiles)
    

    def test_r_cut_p_cuts_isomorphic(self):
        self.assertTrue(self.spc1,self.spc2)
    ...
    def test_map_pairs(self):
        ...
        for Map in maps:
            if len(Map) == 1:
                ...
                self.assertTrue(len(np.unique(Map))== 4)
            else:
                self.assertIn(Map,[[0, 2, 1, 3, 5, 4, 9, 8, 7, 6],[0, 1, 2, 4, 3, 5, 7, 6, 8, 9]])
                self.assertTrue(len(np.unique(Map)) == len(Map))
