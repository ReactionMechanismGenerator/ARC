...
class TestConverter(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.smiles = ['CC(C)F', '[CH3]', 'C[CH](C)', 'CF']
        
        cls.r_1_1_xyz = {'symbols': ('C', 'C', 'C', 'F', 'H', 'H', 'H', 'H', 'H', 'H', 'H'),
        ...
        cls.p_1_1_xyz = {'symbols': ('C', 'C', 'C', 'H', 'H', 'H', 'H', 'H', 'H', 'H'),
        ...
        cls.p_2_1_xyz = {'symbols': ('C', 'F', 'H', 'H', 'H'),
        ...
        cls.r_1 = ARCSpecies(label='r1', smiles=cls.smiles[0], xyz=cls.r_1_1_xyz )
        cls.r_2 = ARCSpecies(label='r2', smiles=cls.smiles[1], xyz=cls.r_2_1_xyz)
        cls.p_1 = ARCSpecies(label='p1', smiles=cls.smiles[2], xyz=cls.p_1_1_xyz)
        cls.p_2 = ARCSpecies(label='p2', smiles=cls.smiles[3], xyz=cls.p_2_1_xyz)
    ...
    def test_cut_species_for_mapping(self):
        ...
        self.assertIn("C[CH]C", [r_cut.mol.smiles for r_cut in r_cuts])
        self.assertIn("[F]", [r_cut.mol.smiles for r_cut in r_cuts])
        self.assertIn("[CH3]", [r_cut.mol.smiles for r_cut in r_cuts])
        self.assertIn("C[CH]C", [p_cut.mol.smiles for p_cut in p_cuts])
        self.assertIn("[F]", [p_cut.mol.smiles for p_cut in p_cuts])
        self.assertIn("[CH3]", [p_cut.mol.smiles for p_cut in p_cuts])


    def test_r_cut_p_cuts_isomorphic(self):
        rxn_1_test = ARCReaction(r_species=[self.r_1, self.r_2], p_species=[self.p_1, self.p_2])
        rxn_1_test.determine_family(self.db)
        reactants, products, loc_r, loc_p = prepare_reactants_and_products_for_scissors(rxn_1_test,self.r_label_dict_rxn_1,self.p_label_dict_rxn_1)
        label_species_atoms(reactants)
        ...
        self.assertTrue(r_cut_p_cuts_isomorphic(self.spc1, self.spc2))
        for r_cut in r_cuts:
            for p_cut in p_cuts:
                if r_cut_p_cuts_isomorphic(r_cut, p_cut):
                    p_cuts.remove(p_cut)
                    break
    ...
    def test_pairing_reactants_and_products_for_mapping(self):
        ...
        for pair in pairs_of_reactant_and_products:
            ...
            self.assertIn(str(pair[0].mol.copy(deep=True).smiles), smiles)
            smiles.remove(pair[0].mol.copy(deep=True).smiles)
    

    def test_map_pairs(self):
        r_1 = ARCSpecies(label='r1', smiles=self.smiles[0], xyz=self.r_1_1_xyz )
        r_2 = ARCSpecies(label='r2', smiles=self.smiles[1], xyz=self.r_2_1_xyz)
        p_1 = ARCSpecies(label='p1', smiles=self.smiles[2], xyz=self.p_1_1_xyz)
        p_2 = ARCSpecies(label='p2', smiles=self.smiles[3], xyz=self.p_2_1_xyz)
        rxn_1_test = ARCReaction(r_species=[r_1, r_2], p_species=[p_1, p_2])
        rxn_1_test.determine_family(self.db)
        ...
        label_species_atoms(products)
        r_cuts, p_cuts = cut_species_for_mapping(reactants, products,loc_r,loc_p)
        make_bond_changes(rxn_1_test, r_cuts, r_label_dict)
        pairs_of_reactant_and_products = pairing_reactants_and_products_for_mapping(r_cuts, p_cuts)
        ...
        for Map in maps:
            if len(Map) == 1:
                ...
                self.assertEqual(len(np.unique(Map)), 4)
            else:
                self.assertIn(Map,[[0, 2, 1, 3, 5, 4, 9, 8, 7, 6],[0, 1, 2, 4, 3, 5, 7, 6, 8, 9], [0, 1, 2, 4, 3, 5, 6, 9, 8, 7]])
    ...
    def test_glue_maps(self):
        ...
        self.assertEqual(atom_map, [0, 1, 2, 11, 4, 3, 5, 6, 9, 8, 7, 10, 12, 13, 14])
    ...
    def test_fix_dihedrals_by_backbone_mapping(self):
        ...
        self.assertAlmostEqual(new_dihedrals_1[2], 121.23138918560065)
        self.assertAlmostEqual(new_dihedrals_2[2], 121.23139082897347)
    ...
    def test_check_atom_map(self):
        """tests the check_atom_map function"""
                # C3H6O + C4H9O <=> C3H5O + C4H10O
        r_1 = ARCSpecies(label='C3H6O', smiles='CCC=O', xyz=self.c3h6o_xyz)
        r_2 = ARCSpecies(label='C4H9O', smiles='[CH2]C(C)CO', xyz=self.c4h9o_xyz)
        p_1 = ARCSpecies(label='C3H5O', smiles='C[CH]C=O', xyz=self.c3h5o_xyz)
        ...
        rxn_2.determine_family(self.db)
        rxn_2.atom_map
        self.assertTrue(check_atom_map(rxn))
