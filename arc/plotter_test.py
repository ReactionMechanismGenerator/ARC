...
class TestSparse2DPlotting(unittest.TestCase):
    ...
    def test_extract_sparse_2d_points_excludes_non_isomorphic(self):
        """Test that energies of non-isomorphic points are excluded from the surface."""
        results = self._make_sparse_results(15)
        n_valid = len(plotter.extract_sparse_2d_points(results)['x'])
        results['directed_scan'][('-30.00', '150.00')] = {'energy': 1e6, 'xyz': {},
                                                          'is_isomorphic': False, 'trsh': []}
        data = plotter.extract_sparse_2d_points(results)
        self.assertEqual(len(data['x']), n_valid)
        self.assertNotIn(1e6, data['energy'])
        self.assertEqual(len(data['invalid_points']), 1)
