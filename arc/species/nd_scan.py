...
def decrement_running_jobs(rotor_dict: dict,
    ...
    Args:
        rotor_dict (dict): A single entry from ``species.rotors_dict``.
        resubmitted (bool, optional): Whether the terminated job was immediately resubmitted
            (e.g., by ESS troubleshooting). Such a job is still in flight, so the counter must
            not be decremented on its behalf: it will be decremented once it truly terminates.
