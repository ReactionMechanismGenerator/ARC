...
""")
    ...
    def test_translate_to_indices(self):
        """Test the translate_to_indices() method."""
        # vinoxy (spc1): symbols=('C', 'C', 'O', 'H', 'H', 'H')
        indices = [0, 2, 1]
        new_xyz = self.spc1.translate_to_indices(indices)
        self.assertEqual(new_xyz['symbols'], ('C', 'O', 'C'))
        self.assertEqual(len(new_xyz['coords']), 3)
        self.assertEqual(new_xyz['coords'][0], self.spc1.get_xyz()['coords'][0])
        self.assertEqual(new_xyz['coords'][1], self.spc1.get_xyz()['coords'][2])
        self.assertEqual(new_xyz['coords'][2], self.spc1.get_xyz()['coords'][1])

        # Test out of bounds
        with self.assertRaises(SpeciesError):
            self.spc1.translate_to_indices([10])

        # Test duplicate indices
        with self.assertRaises(SpeciesError):
            self.spc1.translate_to_indices([0, 1, 0])
