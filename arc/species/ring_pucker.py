...
def pucker_label_from_phase(ring_size: int, phase_deg: float) -> str:
    """Map a Cremer-Pople phase angle to its equatorial canonical pucker label.

    Uses the same half-open phase binning as ``classify_pucker``, so a label derived here can
    never disagree with the label ``classify_pucker`` would assign to a ring actually built at
    ``phase_deg`` (this is what makes ``canonical_pucker_wheel`` label/phase pairs self-consistent
    by construction). ``_half_open_bin_index`` subtracts a small tolerance before flooring, so a
    phase exactly on a bin boundary (e.g. 15.0 for a 6-ring) always resolves deterministically to
    the lower (previous) bin, never the bin it nominally opens, and never splitting or aliasing
    between the two neighboring bins.

    Args:
        ring_size (int): The number of atoms in the ring, 5 or 6.
        phase_deg (float): The Cremer-Pople phase angle (phi2 for a 6-ring, phi for a 5-ring),
            in degrees.

    Returns:
        str: 'boat' or 'twist-boat' for ring_size=6; 'envelope' or 'twist' for ring_size=5.

    Raises:
        RingPuckerError: If ring_size is not 5 or 6.
    """
    phase = phase_deg % 360.0
    if ring_size == 6:
        k = _half_open_bin_index(phase, 30.0, 12)
        return 'boat' if k % 2 == 0 else 'twist-boat'
    if ring_size == 5:
        k = _half_open_bin_index(phase, 18.0, 20)
        return 'envelope' if k % 2 == 0 else 'twist'
    raise RingPuckerError(f'pucker_label_from_phase only supports 5- and 6-membered rings, got ring_size={ring_size}.')


def canonical_pucker_wheel(ring_size: int) -> List[Tuple[str, Optional[float], int]]:
    """Enumerate the full discrete Cremer-Pople pucker wheel of a given ring size.

    Unlike ``canonical_pucker_states``, which returns one coarse label per canonical family, this
    enumerates every distinct phase-resolved point on the pucker sphere/wheel that
    ``classify_pucker`` can distinguish: for a 6-ring, both chair poles plus all 12 equatorial
    (boat/twist-boat) phase bins around the theta=90 equator; for a 5-ring, all 20 pseudorotation
    phase bins (there is no chair analogue and no pole for an odd ring). Every entry's label is
    derived from its phase via ``pucker_label_from_phase``, so label and phase can never disagree.

    Args:
        ring_size (int): The number of atoms in the ring, 5 or 6.

    Returns:
        List[Tuple[str, Optional[float], int]]: A list of (label, phase_deg, pole_sign) tuples.
            ``phase_deg`` is None for the two chair entries (ring_size=6 only); ``pole_sign`` is
            +1 or -1 for chair entries and always +1 for equatorial entries (dropping the
            equatorial sign loop is justified by amplitude-sign/phase+180 degeneracy).

    Raises:
        RingPuckerError: If ring_size is not 5 or 6.
    """
    if ring_size == 6:
        wheel = [('chair', None, 1), ('chair', None, -1)]
        for phase_deg in range(0, 360, 30):
            wheel.append((pucker_label_from_phase(6, phase_deg), float(phase_deg), 1))
        return wheel
    if ring_size == 5:
        wheel = []
        for phase_deg in range(0, 360, 18):
            wheel.append((pucker_label_from_phase(5, phase_deg), float(phase_deg), 1))
        return wheel
    raise RingPuckerError(f'canonical_pucker_wheel only supports 5- and 6-membered rings, got ring_size={ring_size}.')


def ideal_pucker_geometry(ring_size: int, label: str, amplitude: Optional[float] = None,
                          phase_deg: Optional[float] = None) -> np.ndarray:
    ...
    Args:
        ...
        phase_deg (Optional[float]): The Cremer-Pople phase angle to use for an equatorial label
            (boat/twist-boat/envelope/twist). If None, the label's single canonical phase is used
            (exact prior behavior). Ignored for the 6-ring 'chair' label, whose pole is instead
            selected via the sign of ``amplitude``.

    Returns:
        np.ndarray: An (N x 3) array of idealized ring atom coordinates.

    Raises:
        RingPuckerError: If ``label`` is not a valid pucker label for ``ring_size``, or if
            ``phase_deg`` is given for an equatorial label but corresponds (via
            ``pucker_label_from_phase``) to a different label than the one requested.
    """
    ...
    if phase_deg is not None and label != 'chair':
        derived_label = pucker_label_from_phase(ring_size, phase_deg)
        if derived_label != label:
            raise RingPuckerError(
                f"phase_deg={phase_deg} corresponds to pucker label '{derived_label}' for "
                f"ring_size={ring_size}, not the requested label '{label}'.")
    ...
    if ring_size == 6:
        if label == 'chair':
            ...
            q2, phi2_deg, q_half = q_amplitude, (phase_deg if phase_deg is not None else 0.0), 0.0
        elif label == 'twist-boat':
            q2, phi2_deg, q_half = q_amplitude, (phase_deg if phase_deg is not None else 30.0), 0.0
    else:  # ring_size == 5
        if label == 'envelope':
            q2, phi2_deg = q_amplitude, (phase_deg if phase_deg is not None else 0.0)
        elif label == 'twist':
            q2, phi2_deg = q_amplitude, (phase_deg if phase_deg is not None else 18.0)
