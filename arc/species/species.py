...
class ARCSpecies(object):
    ...
    Attributes:
        ...
        derived_stability_verdict (dict): The structured verdict of a wavefunction stability analysis, as parsed from
                                          the ESS log. Defaults to None, and is never inputted by the user: it is a
                                          measured property of one SCF solution, so it steers only the r vs. u
                                          reference decision and never the molecular graph perception that
                                          number_of_radicals feeds. A declared number_of_radicals overrides it.
