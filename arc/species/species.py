...
class ARCSpecies(object):
    ...
    def check_multiplicity_parity(self):
        ...
        This is an inviolable parity relation, so a mismatch always indicates a genuinely
        impossible specification (zero false positives). A mismatch is logged as a warning
        rather than raised, so constructing internally- or restart-derived species (which may
        be transiently inconsistent) is never aborted. The check is skipped gracefully when
        either the multiplicity or the electron count is not yet known (e.g. a TS or an
        xyz-less species defined only by descriptors that failed to perceive a Molecule).
        """
