...
class ARCSpecies(object):
    ...
    def get_symmetry_number(self):
        ...
        """
        if not len(self.mol_list):
            self.mol_list = generate_resonance_structures(self.mol)
        if self.symmetry_number is None:
            if len(self.mol_list):
                ...
                try:
                    ...
                    for index, mol in enumerate(self.mol_list):
    ...
    def get_resonance_hybrid(self):
        ...
        """
        molecules = [mol for mol in self.mol_list if mol.reactive]
