...
def find_smart_anchors(mol: 'Molecule',
    ...
    # If anchor1 is sp-hybridised (exactly 2 neighbours), the triple
    # anchor0–anchor1–anchor2 would be collinear (~180°) regardless of
    # which atom is chosen for anchor2.  Drop anchor2 in this case so
    # that xyz_to_zmat never receives three collinear anchor atoms.
    if anchor2 is not None and anchor1 is not None and len(connectivity[anchor1]) == 2:
        anchor2 = None
...
def get_atom_order(xyz: Optional[Dict[str, tuple]] = None,
    ...
    if mol is not None:
        ...
        for fragment in fragments:
            sequence = get_atom_order_from_xyz(xyz, fragment=fragment)
            for i in sequence:
                if i not in atom_order:
                    atom_order.append(i)
    return atom_order
