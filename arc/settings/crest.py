...
DEFAULT_STANDALONE_DIR = "/Local/ce_dana"
...
def find_crest_executable() -> Tuple[Optional[str], Optional[str]]:
    ...
    Locations are searched in this order: a standalone build under the directory named by the
    ``ARC_CREST_STANDALONE_DIR`` environment variable, defaulting to ``/Local/ce_dana`` and
    disabled by setting that variable to an empty string; the active Python environment and the
    ...
    """
    standalone_dir = os.getenv(STANDALONE_DIR_ENV_VAR, DEFAULT_STANDALONE_DIR)
...
def get_crest_paths() -> Tuple[Optional[str], Optional[str]]:
    """
    Return the cached result of :func:`find_crest_executable`.

    The filesystem is only searched on the first call. Call ``get_crest_paths.cache_clear()``
    to force a new search.

    Returns:
        Tuple[Optional[str], Optional[str]]: The crest executable path and its environment activation command.
    """
    return find_crest_executable()
