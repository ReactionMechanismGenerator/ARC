...
class TestCrestSettingsUtils(unittest.TestCase):
    ...
    def test_find_crest_executable_standalone_dir_argument_and_environment(self):
        """Test that the standalone directory is the argument, else the environment variable, else nothing"""
        with tempfile.TemporaryDirectory() as td:
            fake_home = os.path.join(td, "home")
            os.makedirs(fake_home)
            cases = [(None, None, None),
                     (None, "/env/builds", "/env/builds"),
                     ("/arg/builds", "/env/builds", "/arg/builds"),
                     ("/arg/builds", None, "/arg/builds"),
                     ("", "/env/builds", None),
                     ]
            for standalone_dir, env_value, expected_probe in cases:
                with self.subTest(standalone_dir=standalone_dir, env_value=env_value):
                    with patch.dict(os.environ, {}, clear=False):
                        if env_value is None:
                            os.environ.pop(STANDALONE_DIR_ENV_VAR, None)
                        else:
                            os.environ[STANDALONE_DIR_ENV_VAR] = env_value
                            ...
                            with patch("arc.settings.crest.sys.executable", os.path.join(td, "python")):
                                with patch("arc.settings.crest.shutil.which", return_value=None):
                                    with patch("arc.settings.crest.find_highest_version_in_directory") as mock_find:
                                        path, env_cmd = find_crest_executable(standalone_dir=standalone_dir)
    ...
    def test_get_crest_paths_is_cached(self):
        """Test that the crest lookup touches the filesystem only once per standalone directory"""
        ...
        with patch("arc.settings.crest.find_crest_executable",
            ...
            third = get_crest_paths(standalone_dir="/opt/builds")
        self.assertEqual(first, ("/opt/crest", ""))
        self.assertEqual(second, first)
        self.assertEqual(third, first)
        self.assertEqual(mock_find.call_count, 2)
        mock_find.assert_called_with(standalone_dir="/opt/builds")
