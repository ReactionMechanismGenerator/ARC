...
def check_bond_changes(expected_breaking_bonds: Optional[List[Tuple[int, int]]],
    ...
    """
    Determine whether a breaking and forming bonds in a reaction correspond to the distance matrices
    generated by perturbing the TS along the normal mode displacement.
    ...
    Returns:
        Optional[bool]: Whether the expected breaking and forming bonds are in agreement with the distance matrices.
    """
    ...
    if any(bond_change is None for bond_change in direction_1 + direction_2):
        return None
    if expected_breaking_bonds is None \
            or all(bond_change == expected_breaking_bonds[0] for bond_change in expected_breaking_bonds):
        if expected_forming_bonds is None \
                or all(bond_change == expected_forming_bonds[0] for bond_change in expected_forming_bonds):
            if expected_breaking_bonds is not None and expected_forming_bonds is not None \
                    and expected_forming_bonds[0] == expected_breaking_bonds[0]:
                return False
