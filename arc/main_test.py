...
class TestRestartRoundTrip(unittest.TestCase):
    ...
    def test_completed_job_records_survives_a_restart(self):
        ...
        The restored records are aggregated to prove they remain consumable by the cost
        metrics, not merely that a list of the right length came back.
        """
        records = [{'job_name': 'opt_a1', 'label': 'spc1', 'job_type': 'opt', 'job_adapter': 'gaussian',
                    'server': 'local', 'cpu_cores': 8, 'run_time_sec': 12.5, 'job_status': 'done'}]
        arc0 = ARC(project='arc_restart_roundtrip',
                   species=[ARCSpecies(label='spc1', smiles='CC', compute_thermo=False)],
                   compute_thermo=False,
        ...
        metrics = _compute_cost_metrics(arc1.completed_job_records, t0=None)
        self.assertEqual(metrics['total_job_count'], 1)
        self.assertEqual(metrics['total_core_hours'], 0.0278)
        self.assertEqual(metrics['total_execution_time_hrs'], 0.0035)
        self.assertEqual(metrics['jobs_missing_time'], 0)
        self.assertEqual(metrics['jobs_missing_cores'], 0)
        self.assertEqual(metrics['per_ess']['gaussian']['job_count'], 1)
