...
def _remove_if_present(path: str) -> None:
    """Delete ``path`` when it is still on disk."""
...
def _constraint(coordinate_type, atom_indices, target_value=None, index_base=1,
                target_value_units='unset'):
    """Build the expected constraint record.

    ``target_value_units`` defaults to what the coordinate type implies, which is the
    same for every ESS: degrees for an angle or dihedral, Angstrom for a length.
    """
    if target_value_units == 'unset':
        target_value_units = 'degree' if coordinate_type in ('angle', 'dihedral') else 'angstrom'
    return {
        'coordinate_type': coordinate_type,
        'atom_indices': atom_indices,
        'index_base': index_base,
        'target_value': target_value,
        'target_value_units': target_value_units,
    }
...
class TestGaussianInputDeckConstraints(unittest.TestCase):
    ...
    def _deck(self, body: str) -> str:
        path = _write(body)
        self.addCleanup(_remove_if_present, path)
        return path
    ...
    def test_a_define_line_supplies_the_value_the_freeze_line_omits(self):
        """This is the exact two-line shape ARC's own Gaussian adapter writes."""
        path = self._deck(
            "B 1 2 =1.45 B\n"
            "B 1 2 F\n"
        )
        self.assertEqual(parse_gaussian_constraints(path), [
            _constraint('distance', [1, 2], 1.45),
        ])

    def test_a_define_line_alone_is_not_a_constraint(self):
        """Defining a coordinate does not hold it fixed."""
        self.assertEqual(parse_gaussian_constraints(self._deck("B 1 2 =1.45 B\n")), [])

    def test_the_freeze_line_keeps_its_own_value_over_an_earlier_definition(self):
        path = self._deck(
            "D 1 2 3 4 =170.0 B\n"
            "D 1 2 3 4 =180.0 F\n"
        )
    ...
    def test_a_definition_for_other_atoms_supplies_nothing(self):
        path = self._deck(
            "B 1 2 =1.45 B\n"
            "B 3 4 F\n"
        )
        self.assertEqual(parse_gaussian_constraints(path), [
            _constraint('distance', [3, 4], None),
        ])

    def test_a_scan_line_does_not_supply_a_value_to_a_later_freeze(self):
        """A scan line's numbers are a step count and step size, not a target."""
        path = self._deck(
            "D 1 2 3 4 S 36 10.0\n"
            "D 1 2 3 4 F\n"
        )
        self.assertEqual(parse_gaussian_constraints(path), [
            _constraint('dihedral', [1, 2, 3, 4], None),
        ])

    def test_each_constraint_carries_its_own_definition(self):
        path = self._deck(
            "B 1 2 =1.45 B\n"
            "B 1 2 F\n"
            "A 1 2 3 =104.5 B\n"
        ...
        self.assertEqual(parse_gaussian_constraints(path), [
            _constraint('distance', [1, 2], 1.45),
            _constraint('angle', [1, 2, 3], 104.5),
        ])


class TestGaussianWrittenDeckRoundTrips(unittest.TestCase):
    """The parser must read back what ARC's own Gaussian adapter writes.

    Each deck is generated by ``GaussianAdapter.write_input_file`` rather than
    typed out here, so these cannot drift from the writer: the adapter emits two
    lines per constraint, a ``=value B`` definition followed by a bare ``F``, and
    the value appears only on the first of them.
    """

    @classmethod
    def setUpClass(cls):
        cls.project_directory = tempfile.mkdtemp(prefix='test_gaussian_constraint_round_trip_')
        cls.addClassCleanup(shutil.rmtree, cls.project_directory, ignore_errors=True)

    def _written_deck(self, constraint) -> str:
        """Return the path of a deck the Gaussian adapter wrote for one constraint."""
        job = GaussianAdapter(
            execution_type='incore',
            job_type='opt',
            level=Level(method='wb97xd', basis='def2-TZVP'),
            project='test',
            project_directory=os.path.join(self.project_directory, str(len(constraint[0]))),
            species=[ARCSpecies(label='spc', smiles='CCO')],
            constraints=[constraint],
            testing=True,
        )
        job.write_input_file()
        return os.path.join(job.local_path, 'input.gjf')

    def test_the_adapter_writes_the_two_line_shape_this_covers(self):
        """Guard the guard: if the writer stops using ``=value B`` + ``F``, fail here."""
        with open(self._written_deck(([1, 2], 1.45)), 'r') as f:
            deck_lines = [line.strip() for line in f]
        self.assertIn('B 1 2 =1.45 B', deck_lines)
        self.assertIn('B 1 2 F', deck_lines)

    def test_a_written_constraint_round_trips_with_its_target_value(self):
        for constraint, expected in ((([1, 2], 1.45), _constraint('distance', [1, 2], 1.45)),
                                     (([1, 2, 3], 104.5), _constraint('angle', [1, 2, 3], 104.5)),
                                     (([1, 2, 3, 4], 180.0), _constraint('dihedral', [1, 2, 3, 4], 180.0))):
            with self.subTest(constraint=constraint):
                parsed = parse_gaussian_constraints(self._written_deck(constraint))
                self.assertEqual(parsed, [expected])
                self.assertIsNotNone(parsed[0]['target_value'])
