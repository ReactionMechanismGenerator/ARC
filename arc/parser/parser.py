...
def scan_angles_to_displacement(angles) -> list[float]:
    ...
    The closing point of a full rotation is reported as ``360 + x`` rather than
    folded back onto the start: continuity across the last step is the fact
    callers need, and folding would make the curve appear to end where it began.
    ...
    if angle_arr.size > 1 and angle_arr[-1] < 2 * (angle_arr[1] - angle_arr[0]):
        angle_arr[-1] += 360.0
    return angle_arr.tolist()
