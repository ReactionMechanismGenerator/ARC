...
class TestGaussianCartesianHessian(unittest.TestCase):
    ...
    def test_malformed_input_orientation_row_returns_none(self):
        """A truncated coordinate row yields no geometry rather than a short one.

        The returned geometry is presented as the complete structure the Hessian
        was evaluated at, so stopping at the first unreadable row and keeping the
        atoms already collected hands the caller a silently partial molecule.
        """
        path = os.path.join(ARC_TESTING_PATH, 'restart', '3_restart_bde', 'calcs', 'Species',
                            'anilino_radical', 'freq_a4345', 'output.out')
        intact_xyz, _ = GaussianParser(path).parse_cartesian_hessian_geometry()
        self.assertGreater(len(intact_xyz['symbols']), 3)

        lines = _get_lines_from_file(path)
        header = next(idx for idx in range(len(lines) - 1, -1, -1)
                      if 'Force constants in Cartesian coordinates:' in lines[idx])
        table = next(idx for idx in range(header, -1, -1)
                     if 'Input orientation:' in lines[idx])
        truncated_row = table + 7
        self.assertEqual(len(lines[truncated_row].split()), 6)
        lines[truncated_row] = ' '.join(lines[truncated_row].split()[:3]) + '\n'

        scratch = tempfile.mkdtemp()
        self.addCleanup(shutil.rmtree, scratch, ignore_errors=True)
        truncated_path = os.path.join(scratch, 'truncated_input_orientation.out')
        with open(truncated_path, 'w') as f:
            f.writelines(lines)

        xyz, frame = GaussianParser(truncated_path).parse_cartesian_hessian_geometry()
        self.assertIsNone(xyz)
        self.assertIsNone(frame)
