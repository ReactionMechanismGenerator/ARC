...
class OrcaParser(ESSAdapter, ABC):
    ...
    def parse_wavefunction_stability(self) -> dict | None:
        ...
        ``followed_to_stable`` reports whether the last of several ended stable, i.e.
        whether ORCA reached a stable solution after following the instability.
        ...
        * For a restricted reference the single unlabelled matrix spans both the
          ...
          A restart that never reached a stable solution measures nothing, and its
          ``<S**2>`` belongs to an SCF that did not converge, so the instability is real
          but its sector is undetermined and the verdict is
        ...
        for line in _get_lines_from_file(self.log_file_path):
            ...
            if not blocks:
                ...
                continue
            match = re.match(r'\s*E\(\s*\d+\)\s*=\s*([-+]?\d*\.?\d+(?:[EeDd][-+]?\d+)?)\s*Eh', line)
            if match is not None:
        ...
        followed_to_stable = len(blocks) > 1 and blocks[-1]['verdict'] == 'stable'
        s_squared_after_follow = None
        if followed_to_stable:
