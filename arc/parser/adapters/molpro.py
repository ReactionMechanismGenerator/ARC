...
class MolproParser(ESSAdapter, ABC):
    ...
    def parse_normal_mode_displacement(self) -> Optional[Tuple['np.ndarray', 'np.ndarray']]:
        """
        Parse frequencies and normal mode displacement from Molpro log file.
    ...
    def parse_zpe_correction(self) -> Optional[float]:
        ...
        """
        electronic_energy_0k, total_energy_0k = None, None
        with open(self.log_file_path, 'r') as f:
            for line in f:
                if 'Electronic Energy at 0 [K]:' in line:
                    try:
                        # Example: Electronic Energy at 0 [K]:   -154.058191
                        electronic_energy_0k = float(line.split()[-1])
                    except (ValueError, IndexError):
                        continue
                elif 'Total Energy at 0 [K]:' in line:
                    try:
                        # Example: Total Energy at 0 [K]:        -154.032781
                        total_energy_0k = float(line.split()[-1])
                    except (ValueError, IndexError):
                        continue
        if electronic_energy_0k is not None and total_energy_0k is not None:
            zpe_hartree = total_energy_0k - electronic_energy_0k
