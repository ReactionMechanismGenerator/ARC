"""Tests for ARC's parser-neutral evidence producer."""
...
GOLDEN_FLOAT_REL_TOL = 1e-12
GOLDEN_FLOAT_ABS_TOL = 1e-8
...
class TestEvidenceProducer(unittest.TestCase):
    ...
    def test_exact_top_level_shape_and_no_timestamp(self):
        ...
        result = build_parser_evidence(output_doc=output, project_directory=self.root, document_id=DOC_ID)
    ...
    def _producer(self, **header):
        """Return the ``producer`` block the builder emits for an output document with ``header``."""
        output = {"schema_version": "1.1", "species": [], "transition_states": [], **header}
        document = build_parser_evidence(
            output_doc=output, project_directory=self.root, document_id=DOC_ID
        )
        return document["producer"]

    def test_producer_names_the_arkane_install_alongside_arc(self):
        """The producer block carries the output document's Arkane version and commit."""
        producer = self._producer(arc_version="1.1.0", arc_git_commit="abc123",
                                  arkane_version="3.2.0", arkane_git_commit="def456")
        self.assertEqual(producer, {"name": "ARC", "version": "1.1.0", "git_commit": "abc123",
                                    "arkane_version": "3.2.0", "arkane_git_commit": "def456"})

    def test_producer_arkane_identity_is_null_when_the_output_document_has_none(self):
        """Both Arkane fields stay present and carry ``None``, each independently of the other."""
        unresolved = self._producer(arc_version="1.1.0", arc_git_commit="abc123",
                                    arkane_version=None, arkane_git_commit=None)
        self.assertIsNone(unresolved["arkane_version"])
        self.assertIsNone(unresolved["arkane_git_commit"])
        no_repository = self._producer(arc_version="1.1.0", arc_git_commit="abc123",
                                       arkane_version="3.2.0", arkane_git_commit=None)
        self.assertEqual(no_repository["arkane_version"], "3.2.0")
        self.assertIsNone(no_repository["arkane_git_commit"])

    def test_producer_tolerates_an_output_document_without_the_arkane_keys(self):
        """An output document predating the Arkane fields still builds, with both emitted as ``None``."""
        producer = self._producer(arc_version="1.1.0", arc_git_commit="abc123")
        self.assertEqual(set(producer),
                         {"name", "version", "git_commit", "arkane_version", "arkane_git_commit"})
        self.assertIsNone(producer["arkane_version"])
        self.assertIsNone(producer["arkane_git_commit"])
        self.assertEqual(producer["version"], "1.1.0")
    ...
    def test_gaussian_hessian_shape_and_triangle(self):
        ...
        self.assertEqual(value["parser_version"], "arc-hessian-1")
    ...
    @patch("arc.parser_evidence.ess_factory")
    @patch("arc.parser_evidence.determine_ess", return_value="gaussian")
    ...
    @patch("arc.parser_evidence.parse_irc_path")
    ...
    @patch("arc.parser_evidence.parse_irc_traj", return_value=[XYZ_DICT])
    @patch("arc.parser_evidence.parse_irc_path", return_value=None)
    ...
    @patch("arc.parser_evidence.parse_gsm_stringfile_energies", return_value=[0.0, 2.0, 0.0])
    @patch("arc.parser_evidence.parse_trajectory", return_value=[XYZ_DICT, XYZ_DICT, XYZ_DICT])
    @patch("arc.parser_evidence.kabsch", return_value=0.25)
    def test_gsm_frames_coordinates_and_selected_point(self, kabsch_mock, trajectory, energies):
        ...
        self.assertEqual(value["parser_version"], "arc-gsm-stringfile-1")
        self.assertEqual(
    ...
    @patch("arc.parser_evidence.parse_gsm_stringfile_energies", return_value=[0.0, 0.0, 0.0])
    @patch("arc.parser_evidence.parse_trajectory")
    ...
    @patch("arc.parser_evidence.parse_gsm_stringfile_energies", return_value=[0.0, 0.0])
    @patch("arc.parser_evidence.parse_trajectory")
    ...
    @patch("arc.parser_evidence.parse_irc_traj", return_value=None)
    @patch("arc.parser_evidence.parse_irc_path", return_value=[{"xyz": None}])
    ...
    @patch("arc.parser_evidence._build_irc", side_effect=RuntimeError("unexpected"))
    @patch("arc.parser_evidence._build_gsm")
    @patch("arc.parser_evidence._build_hessian")
    ...
    @patch("arc.parser_evidence.parse_gsm_stringfile_energies", return_value=[0.0, 0.0, 0.0])
    @patch("arc.parser_evidence.parse_trajectory", return_value=[XYZ_DICT, XYZ_DICT, XYZ_DICT])
    @patch("arc.parser_evidence.kabsch", return_value=0.0)
    def test_gsm_all_zero_comment_energies_are_read_as_the_no_energy_sentinel(
            self, kabsch_mock, trajectory, energies):
        """An all-zero energy column is the xtb_gsm build's "nothing written" marker.

        ``parse_gsm_stringfile_energies`` returns the comment column verbatim and
        leaves the decision to its caller. Distinct geometries along a string
        cannot all be degenerate, so exporting the zeros would hand a consumer a
        flat 0 kcal/mol profile that reads as a measurement. The key is omitted
        from every point instead.
        """
        self._touch("calcs/gsm/stringfile.xyz0000")
        envelope = _build_gsm({"label": "TS0", "gsm_log": "calcs/gsm/stringfile.xyz0000"}, self.root)
        self.assertEqual(envelope["status"], "available")
        for point in envelope["value"]["points"]:
            self.assertNotIn("stringfile_relative_energy_kcal_mol", point)

    @patch("arc.parser_evidence.parse_gsm_stringfile_energies", return_value=[0.0, 1.5, 0.0])
    @patch("arc.parser_evidence.parse_trajectory", return_value=[XYZ_DICT, XYZ_DICT, XYZ_DICT])
    @patch("arc.parser_evidence.kabsch", return_value=0.0)
    def test_gsm_a_real_energy_column_is_exported_zeros_included(
            self, kabsch_mock, trajectory, energies):
        """Only an all-zero column is the sentinel; a column carrying any energy is data,
        and its own zeros -- the first frame's reference among them -- are exported."""
        self._touch("calcs/gsm/stringfile.xyz0000")
        envelope = _build_gsm({"label": "TS0", "gsm_log": "calcs/gsm/stringfile.xyz0000"}, self.root)
        self.assertEqual(
    ...
    def test_atomic_json_format_and_no_temporary_files(self):
        ...
        path = write_parser_evidence_atomic(evidence_doc=document, output_directory=self.root)
        text = path.read_text()
    ...
    def test_atomic_write_fsyncs_the_parent_directory(self):
        ...
        with patch("arc.parser_evidence.os.fsync", side_effect=_recording_fsync):
            write_parser_evidence_atomic(evidence_doc=document, output_directory=self.root)
    ...
    def test_atomic_write_survives_a_platform_that_cannot_fsync_directories(self):
        ...
        with patch("arc.parser_evidence.os.open", side_effect=_refuse_directories):
            path = write_parser_evidence_atomic(evidence_doc=document, output_directory=self.root)
    ...
    def test_non_finite_cannot_be_serialized_and_temp_is_removed(self):
        with self.assertRaises(ValueError):
            write_parser_evidence_atomic(evidence_doc={"bad": math.nan}, output_directory=self.root)
        self.assertFalse(any(item.name.endswith(".tmp") for item in self.root.iterdir()))
    ...
    def test_paths_are_relative_and_operational_fields_do_not_leak(self):
        ...
        document = build_parser_evidence(output_doc=output, project_directory=self.root, document_id=DOC_ID)
    ...
    GOLDEN_STAGE = {
        ...
        "calcs/TSs/TS0/gsm/stringfile.xyz0000": (ARC_TESTING_PATH, "parser_evidence", "gsm",
                                                 "stringfile.xyz0000"),
    }
    GOLDEN_STAGE_TREES = {
        "calcs/TSs/TS0/gsm/gsm_node_outputs": (ARC_TESTING_PATH, "parser_evidence", "gsm", "gsm_node_outputs"),
    }
    ...
    def test_gsm_energies_attach_to_real_frames_by_geometry_on_a_real_run(self):
        ...
        line in this real stringfile carries ``0.000000``, so that column is read as
        the build's "no energy written" sentinel and dropped, leaving the
        attachment as the points' only energy.
        """
        self._stage_golden()
        value = _build_gsm(
    ...
    def _assert_matches_golden(self, actual, expected, path="") -> None:
        ...
        exactly. A float matches when it differs from the golden value by no
        more than ``GOLDEN_FLOAT_REL_TOL`` times that value or by
        ``GOLDEN_FLOAT_ABS_TOL``, whichever of the two is larger, so that a
        near-zero quantity is held to the absolute bound and every other
        quantity to the relative one. A failure reports the path, both values
        ...
        if isinstance(expected, Mapping):
            ...
            delta = max(GOLDEN_FLOAT_REL_TOL * abs(expected), GOLDEN_FLOAT_ABS_TOL)
            self.assertAlmostEqual(
                actual,
                expected,
                delta=delta,
                msg=f"{path}: {actual!r} is not within {delta!r} of {expected!r}",
            )
        else:
    ...
    def test_golden_contract(self):
        ...
        """
        golden_dir = Path(ARC_TESTING_PATH) / "parser_evidence" / "golden"
        output = yaml.safe_load((golden_dir / "output.yml").read_text())
        expected = json.loads((golden_dir / "parser_evidence.json").read_text())
        self._stage_golden()

        actual = build_parser_evidence(
            output_doc=output, project_directory=self.root, document_id=DOC_ID
        )
    ...
    def test_golden_records_are_all_available(self):
        ...
        """
        expected = json.loads(
            (Path(ARC_TESTING_PATH) / "parser_evidence" / "golden" / "parser_evidence.json").read_text()
        )
        kinds = set()
        attached = 0
