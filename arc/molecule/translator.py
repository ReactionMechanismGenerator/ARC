...
def _lookup(mol, identifier, identifier_type):
    ...
    For troublesome SMILES, we look up the adjacency list and parse that.

    Args:
        mol: Molecule class with methods like from_smiles() and from_adjacency_list()
        identifier (str): The identifier string to look up
        identifier_type (str): The type of identifier ('inchi' or 'smi')
...
def _read(mol, identifier, identifier_type, backend, raise_atomtype_exception=True):
    ...
    for option in _get_backend_list(backend):
        ...
        if _check_output(mol, identifier):
            ...
            logger.debug(f'Backend {option} is not able to parse identifier {identifier}')

    raise ValueError(f"Unable to correctly parse {identifier} with backend {backend}.")


def _write(mol, identifier_type, backend):
    """
    Converts the input molecule to the specified identifier type using one or more backends.
    ...
    Raises:
        ValueError: If no backend can produce a valid identifier.
    """
    if not mol.atoms:
        return ''
    ...
    for option in _get_backend_list(backend):
        try:
            ...
            if _check_output(mol, output):
                ...
                logger.debug(f"Backend '{option}' could not produce a valid '{identifier_type}' for:\n{mol.to_adjacency_list()}")

        except ValueError as e:
            logger.debug(f"Backend '{option}' raised ValueError: {e}")
            continue
