...
import arc.molecule.converter as converter
...
from arc.molecule.element import bdes
from arc.molecule.graph import Vertex, Edge, Graph, get_vertex_connectivity_value
...
class Molecule(Graph):
    ...
    def update_atomtypes(self, log_species=True, raise_exception=True):
        ...
        for atom in self.vertices:
            try:
                ...
                if log_species:
                    logger.debug("Could not update atomtypes for this molecule:\n{0}".format(self.to_adjacency_list()))
                if raise_exception:
                    raise
    ...
    def to_adjacency_list(self, label='', remove_h=False, remove_lone_pairs=False):
        """
        Convert the molecular structure to a string adjacency list.
        """
        from arc.molecule.adjlist import to_adjacency_list
        mol_copy = self.copy(deep=True)
        result = to_adjacency_list(mol_copy.vertices, mol_copy.multiplicity, metal=mol_copy.metal, facet=mol_copy.facet,
    ...
    def assign_atom_ids(self):
        ...
        Uses the entire range of cython's integer values to reduce the chance of duplicates
        """
    ...
    def is_identical(self, other, strict=True):
        ...
        The primary use case is tracking atoms in reactions for reaction degeneracy determination.
        ...
        if not isinstance(other, Molecule):
            raise TypeError(f'Got a {other.__class__} object for parameter "other", when a Molecule object is required.')
