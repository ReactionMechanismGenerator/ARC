"""
This module contains functions for kekulization of an aromatic molecule.
...
4. For the remaining bonds, determine whether they can be double bonds.
5. If a clear determination cannot be made, make a heuristic-based assumption.
...
A key part of the algorithm is the usage of degree of freedom (DOF) analysis
to determine the optimal order to solve the system. Rings and bonds with fewer
DOFs have fewer ways to be kekulized and are generally easier to solve.
...
from arc.molecule.graph cimport Graph, Vertex, Edge
from arc.exceptions import KekulizationError, AtomTypeError
from arc.molecule.element import PeriodicSystem
from arc.molecule.molecule cimport Atom, Bond
...
cpdef kekulize(Graph mol):
    ...
    try:
        ...
        logger.debug('Unable to kekulize molecule, final result was invalid:/n{0}'.format(mol.to_adjacency_list()))
