...
def to_rdkit_mol(mol, remove_h=True, return_mapping=False, sanitize=True, save_order=False):
    """
    Convert a molecular structure to a RDKit rdmol object. Uses `RDKit <http://rdkit.org/>`_ to perform the conversion.
    Perceives aromaticity and, unless remove_h is ``False``, removes hydrogen atoms.

    If return_mapping is ``True`` then it also returns a dictionary mapping the atoms to RDKit's atom indices.
    """
...
def debug_rdkit_mol(rdmol, level=10):
    """
