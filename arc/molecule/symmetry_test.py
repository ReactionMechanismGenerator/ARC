...
""")
    ...
    def test_edge_carbon_atom_symmetry_number_allyl(self):
        """
        Test the Molecule.calculate_atom_symmetry_number() for the edge carbons on [CH2]C=C
        """
        spc = ARCSpecies(label='S1', mol=Molecule(smiles='[CH2]C=C'))
        molecule = spc.get_resonance_hybrid()
        for atom in molecule.atoms:
    ...
    def test_bond_symmetry_number_allyl(self):
        ...
        symmetry_number = 1
        for atom1 in molecule.atoms:
            bonded_atoms = list(atom1.bonds.keys())
    ...
    def test_symmetry_number_hydrogen(self):
        ...
        """
        self.assertEqual(ARCSpecies(label='H2', smiles='[H][H]').get_symmetry_number(), 2)
