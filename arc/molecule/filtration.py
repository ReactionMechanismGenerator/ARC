...
def filter_structures(mol_list, mark_unreactive=True, allow_expanded_octet=True, features=None, save_order=True):
    """
...
def get_octet_deviation_list(mol_list, allow_expanded_octet=True):
    """
    Returns a list of octet deviations for a respective list of :class:Molecule objects
    """
...
def get_octet_deviation(mol, allow_expanded_octet=True):
    ...
    for atom in mol.vertices:
        if atom.is_hydrogen() or atom.is_lithium():
            continue
...
def check_reactive(filtered_list):
    ...
    if not any([mol.reactive for mol in filtered_list]):
        logger.info('\n\n')
        logger.error(f'No reactive structures were attributed to species {filtered_list[0].to_smiles()}')
        for mol in filtered_list:
            logger.info(f'Structure: {mol.to_smiles()}\n{mol.to_adjacency_list()}Reactive: {mol.reactive}')
        logger.info('\n')
