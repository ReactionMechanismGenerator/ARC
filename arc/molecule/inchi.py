...
def create_augmented_layers(mol):
    ...
    if mol.get_radical_count() == 0 and not _has_unexpected_lone_pairs(mol):
        ...
        hydrogens = []
        for at in molcopy.atoms:
            if at.number == 1:
                hydrogens.append(at)
        for h in hydrogens:
            molcopy.remove_atom(h)

        atom_indices_original = _parse_n_layer(auxinfo)

        new_atom_indices = []
        for i, atom in enumerate(molcopy.atoms):
            new_atom_indices.append(atom_indices_original.index(i + 1))

        paired_atoms = list(zip(new_atom_indices, molcopy.atoms))
        paired_atoms.sort()  # simple sort avoids lambdas/closures

        molcopy.atoms = []
        for _, atom in paired_atoms:
            molcopy.atoms.append(atom)
...
def _convert_3_atom_2_bond_path(start, mol):
    """
    Searches for 3-atom-2-bond [X=X-X+] paths starting from the parameter atom.
