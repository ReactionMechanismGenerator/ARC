...
class MoleculeDrawer(object):
    ...
    def draw(self, molecule, file_format, target=None) -> Optional[tuple]:
        ...
        if cairo is None:
            logger.error('Cairo not found; molecule will not be drawn.')
            return None
        ...
        if self.molecule.get_formula() == 'CO' and len(atoms_to_remove) == 0:
            ...
            try:
                ...
                logger.error('Error while drawing molecule {0}: {1}'.format(molecule.to_smiles(), e))
                import sys, traceback
                sys.exc_info()
                traceback.print_exc()
                return None, None, None
            except KeyError:
                logger.error('KeyError occured when drawing molecule, likely because'
    ...
    def render(self, cr, offset=None):
        ...
        for cycle in self.cycles:
            ...
            if all([bond.is_benzene() for bond in cycle_bonds]):
                ...
                radius = math.hypot(center[0] - (coordinates[index1, 0] + coordinates[index2, 0]) / 2,
                                    center[1] - (coordinates[index1, 1] + coordinates[index2, 1]) / 2) - 4
