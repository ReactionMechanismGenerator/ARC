...
def generate_resonance_structures_safely(mol,
    ...
    result = None
    try:
        result = generate_resonance_structures(mol=mol.copy(deep=True),
                                               clar_structures=clar_structures,
...
def generate_resonance_structures(mol,
    ...
    try:
        ...
        logger.debug("The following molecule has at least one atom with an undefined atomtype:\n{0}"
    ...
    if features['is_aromatic'] or (features['is_cyclic'] and features['is_radical'] and not features['is_aryl_radical']):
        ...
        if len(new_mol_list) == 0:
            ...
            for new_mol in new_mol_list:
                # Append to the structure list if unique
    ...
    if filter_structures:
        return filtration.filter_structures(mol_list, features=features, save_order=True)
...
def _generate_resonance_structures(mol_list, method_list, keep_isomorphic=False, copy=False,
    ...
    for mol in mol_list[1:]:
        if mol.get_net_charge() != input_charge:
            mol_list.remove(mol)
            logger.debug('Resonance generation created a molecule %s with a net charge of %d '
        ...
        if mol.contains_surface_site():
            for x in [atom for atom in mol.atoms if atom.is_surface_site()]:
                if x.radical_electrons != 0:
                    mol_list.remove(mol)
                    logger.debug('Resonance generation created a molecule %s with %d radicals on %s.\n'
                                 'Removing it from resonance structures', mol.smiles, x.radical_electrons, x.symbol)
                elif x.lone_pairs != 0:
                    mol_list.remove(mol)
                    logger.debug('Resonance generation created a molecule %s with %d lone pairs on %s.\n'
                                 'Removing it from resonance structures', mol.smiles, x.lone_pairs, x.symbol)
                elif x.charge != 0:
                    mol_list.remove(mol)
                    logger.debug('Resonance generation created a molecule %s with a charge of %d on %s.\n'
...
def generate_kekule_structure(mol):
    ...
    If there's an error (e.g., in RDKit) then it just returns an empty list.
    """
...
def _clar_optimization(mol, constraints=None, max_num=None, save_order=False):
    ...
    aromatic_rings.sort(key=_aromatic_ring_sort_key)
    ...
    lpsolve('set_mat', lp, a)                   # set the left-hand side to constraint matrix
    lpsolve('set_rh_vec', lp, [1] * m)          # set the right-hand side to 1 for all constraints
    ...
    # Add constraints to the problem if provided
    if constraints is not None:
        for constraint in constraints:
            try:
                lpsolve('add_constraint', lp, constraint[0], '<=', constraint[1])
            except Exception as e:
                logger.debug('Unable to add constraint: {0} <= {1}'.format(constraint[0], constraint[1]))
                logger.debug(mol.to_adjacency_list())
