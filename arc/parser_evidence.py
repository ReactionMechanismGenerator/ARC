...
EVIDENCE_SCHEMA_NAME = "arc-parser-evidence"
...
EVIDENCE_FILENAME = "parser_evidence.json"
...
def _build_hessian(record: Mapping[str, Any], project_directory: str | Path) -> dict:
    ...
    try:
        ...
        logger.debug("Parser evidence Hessian parse failed for %s", record.get("label"), exc_info=True)
        return _unavailable("parse_failed", paths)
...
def _build_irc(record: Mapping[str, Any], project_directory: str | Path) -> dict:
    ...
    for position, log_path in enumerate(logs):
        ...
        try:
            ...
            logger.debug("Parser evidence IRC source omitted: %s", source_log, exc_info=True)
            omitted.append(source_log)
    if not trajectories:
...
def _build_gsm(record: Mapping[str, Any], project_directory: str | Path) -> dict:
    ...
    ``stringfile_relative_energy_kcal_mol`` carries the stringfile comment line
    verbatim, but only where that column holds energies. A string of more than
    one frame whose every frame reports exactly zero is the "no energy written"
    sentinel of ARC's own xtb_gsm build, not a profile — distinct geometries
    along a path cannot all be degenerate — so the key is omitted from every
    point rather than exported as a flat zero profile.
    ...
    try:
        ...
        if relative is not None and len(relative) > 1 and not any(relative):
            logger.debug("GSM stringfile %s reports a zero relative energy on every one of its "
                         "%d frames; reading the column as the build's 'no energy written' "
                         "sentinel rather than as a flat profile.", source, len(relative))
            relative = None
        ...
        logger.debug("Parser evidence GSM parse failed for %s", record.get("label"), exc_info=True)
        return _unavailable("parse_failed", [source])


def build_parser_evidence(*, output_doc: Mapping[str, Any], project_directory: str | Path, document_id: str) -> dict:
    """Build one complete evidence document, isolating each optional entry.

    The ``producer`` block names both ARC and the Arkane install whose energy
    corrections ``output_doc`` records, under the same key names ``output.yml``
    itself uses for them. All four version and commit fields are always present
    and carry ``null`` wherever ``output_doc`` does not supply a value.
    """
    records = []
        ...
        for source_record in output_doc.get(section) or []:
            ...
            for evidence_kind, builder, source_paths in attempts:
                try:
                    ...
                    logger.debug(
                        "Unexpected parser evidence builder failure kind=%s label=%s",
            ...
            for evidence_kind in ("freq_hessian", "irc", "gsm"):
                ...
                if envelope and envelope.get("status") == "unavailable":
                    logger.debug(
                        "Parser evidence unavailable kind=%s label=%s reason=%s source_paths=%s",
                        evidence_kind,
...
def write_parser_evidence_atomic(*, evidence_doc: Mapping[str, Any], output_directory: str | Path) -> Path:
    ...
    descriptor, temporary = tempfile.mkstemp(dir=output_directory, suffix=".parser_evidence.json.tmp")
