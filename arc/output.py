...
def write_output_yml(
    ...
    species_corrections = _compute_species_corrections(
        species_dict, corrections.aec_key, bac_type, project_directory,
    )
    doc['species'] = []
...
def _discard_stale_evidence(output_directory: str) -> None:
    """Remove a ``tckdb_evidence.json`` left behind by an earlier run.
...
def _build_energy_corrections_for_species(
    ...
    *,
    ...
    the level of theory ARC ran at, the Arkane database key whose parameters
    produced this record's numbers, and the parameter table it actually used.
    Every record reports ``aec_key`` as its ``matched_arkane_key``, which is
    ``None`` when no atom-energy key was matched. The BAC ``parameter_table`` is
    emitted only when ``bac_key`` equals ``aec_key``. Consumers own any
...
def _get_imaginary_freqs(spc) -> list[float] | None:
    ...
    frequencies are used, falling back to the freq job's (empty) result.
