...
class Scheduler(object):
    ...
    def run_sp_job(self,
        ...
        if len(mol.atoms) == 1 and mol.atoms[0].element.symbol == 'H' and level.method_type == 'wavefunction':
            # Run only CCS for an H atom instead or CCSD(T) / MRCI / DLPNO-CCSD(T) / etc.
            level = Level(repr='ccs/vtz', software=level.software, args=level.args)
        if self.job_types['sp']:
            if self.species_dict[label].multi_species:
