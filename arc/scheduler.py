...
class Scheduler(object):
            ...
            for label in self.unique_species_labels:
                ...
                for job_name in job_list:
                    ...
                    if 'tsg' in job_name:
                        ...
                        if not (job.job_id in self.server_job_ids and job.job_id not in self.completed_incore_jobs):
                            jobs_before = set(self.running_jobs.get(label, list()))
                            successful_server_termination = self.end_job(job=job, label=label, job_name=job_name)
                            if successful_server_termination:
                            ...
                            if 'brute_force' in job.directed_scan_type:
                                ...
                                # Don't decrement for a job that troubleshooting just resubmitted,
                                # it is still in flight and will decrement when it truly terminates.
                                resubmitted = bool(set(self.running_jobs.get(label, list())) - jobs_before)
                                all_done = decrement_running_jobs(rotor_dict, resubmitted=resubmitted)
                                if all_done:
    ...
    def check_directed_scan_job(self, label: str, job: JobAdapter):
        ...
        if job.job_status[1]['status'] == 'done':
            xyz = parser.parse_geometry(log_file_path=job.local_path_to_output_file)
            # A TS is not isomorphic with the 2D graph representation of either side of the
            # reaction, so isomorphism must not be checked for it (retained from main).
            if self.species_dict[label].is_ts:
                is_isomorphic = True
            else:
