...
RITS_CKPT_SHA256="2c6fdacc13a304cb8bb4030881ff4198de647d8f8f7fdaa878414e2514be0668"

# ── parse flags ────────────────────────────────────────────────────────────
TEMP=$(getopt -o h --long cpu,cuda:,path:,no-ckpt,no-ckpt-check,help -- "$@")
eval set -- "$TEMP"
while true; do
  case "$1" in
    --cpu)
    ...
    -h|--help)
      cat <<EOF
Usage: $0 [--cpu] [--cuda <variant>] [--path <existing RitS checkout>]
  ...
  --no-ckpt-check   build the env without validating an existing checkpoint —
                    useful for CI smoke installs. The ARC adapter will skip RitS
...
if $SKIP_CKPT_CHECK; then
  echo "ℹ️  --no-ckpt-check: skipping Zenodo checkpoint download AND validation."
elif $SKIP_CKPT; then
  echo "ℹ️  --no-ckpt set, skipping checkpoint download."
elif [[ -f "$CKPT_PATH" ]]; then
  echo "📦  Existing checkpoint found at $CKPT_PATH — verifying SHA-256..."
  if verify_sha256 "$CKPT_PATH" "$RITS_CKPT_SHA256"; then
    echo "✔️  Checkpoint SHA-256 OK ($RITS_CKPT_SHA256)"
  else
    echo "❌  Existing checkpoint does not match the expected SHA-256." >&2
  ...
  if verify_sha256 "$TMP_CKPT" "$RITS_CKPT_SHA256"; then
